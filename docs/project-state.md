...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED + EXP-045/EXP-046/EXP-047 CLOSED/REVIEWED + EXP-048 DEC-128 ONE-RUN AUTHORIZATION SOURCE FROZEN; NO EXP-048 RESULT
**Next milestone:** Merge DEC-128's single guarded EXP-048 historical run authorization. DEC-128 itself dispatches nothing. After merge, freeze a clean-main one-way operator that exposes exactly one dispatch only while no EXP-048 run exists and routes terminal evidence through DEC-127. The first manual-main attempt will consume the slot on any terminal outcome; no automatic retry/replacement. Promotion, shadow/demo execution, broker mutation, live orders, real-money trading, and trading authorization remain locked.
