...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED + EXP-045 CLOSED/REVIEWED + EXP-046 SOURCES/TERMINAL REVIEW FROZEN + DEC-109 SINGLE-RUN AUTHORIZATION SOURCE FROZEN; NO EXP-046 MODEL RESULT
**Next milestone:** Merge DEC-109's one-run authorization source. After merge, re-check current `main` and zero prior manual-main EXP-046 runs before any separate guarded operator/dispatch step. The first attempt consumes the slot on success, failure, cancellation, or timeout; no automatic retry/replacement. Promotion, shadow/demo execution, broker mutation, live orders, real-money trading, and trading authorization remain locked.
