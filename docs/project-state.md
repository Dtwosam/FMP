...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED; direct market-learning track closed by DEC-262; EXP-015 Stage A now has DEC-264 first-run governance with one bounded authoritative slot open but not dispatched; Stage B/C, DEC-042 portfolio selection, and DEC-045 Phase 8A acceptance remain blocked
**Next milestone:** Build a clean-main one-way EXP-015 Stage A operator that can classify the single authoritative slot as missing/in-progress/terminal without dispatching it. A later read-only proof and separately reviewed one-shot executor are required before Stage A can run. Phase 8B and all demo/live/broker/real-money paths remain locked.
