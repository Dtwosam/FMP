...
4. DEC-052 / `EXP-20260922-023` is implemented on PR #144 head `a949e57eeed58a3f96dc3b59932ce8b2e91127ac`: explicit bounded quote-only `capture-segment`, create-only fsynced raw/audit/event journals, same-reader first-preflight transition, fail-closed bridge identity, clean DEC-050 compile/replay closure, and immutable `fmp-phase8b-prospective-segment-v1` evidence; PR tests run `35751759857` passed 977 tests plus YAML/compile and unchanged Phase 3 acceptance run `35751759911` passed;
5. DEC-052 does not aggregate multiple segments or run acceptance. A later separately frozen campaign-close protocol must account for restart gaps, aggregate only clean replay-matching prospective segments, and create the exact DEC-051 `fmp-phase8b-campaign-evidence-v1` artifact;
