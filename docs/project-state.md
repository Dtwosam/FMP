...
7. DEC-051 / `EXP-20260922-022` acceptance compiler is implemented on PR #142 fix head `8628f0810a55353da3bbe299635d0bd89fa4ab94`: exact prospective campaign/spread-reference schemas, immutable-fingerprint-first validation, frozen minimum/operational/spread/financial gates, deterministic outcome precedence, and PASS-only SHADOW_CANDIDATE -> SHADOW_VALIDATED authorization. Initial CI caught the fingerprint-validation-order defect; fixed run `35749385885` passed 973 tests plus YAML/compile and unchanged Phase 3 acceptance `35749385867` passed. No live capture command exists; a later capture/close protocol must create real prospective evidence. All order/broker/real-money/Phase-9 execution paths remain locked.
