...
- DEC-042 / EXP-20260922-014 is the frozen portfolio-selection protocol. The current source-free implementation branch adds the missing guarded execution layer: exact eligible-pool reconstruction from the Phase 7 baseline plus EXP-015 final shortlist, pre-result freezing of every 1–6-strategy set, joint-result metric conversion, mandatory-gate evaluation, deterministic ranking, and a manual workflow that uploads the preflight before selection-period data is downloaded. The actual combination search remains BLOCKED because no EXP-015 historical stages have been dispatched.
...
3. source-free verify and merge the current DEC-042 selection execution layer; it must freeze/upload the exact 2–12 strategy pool and all 1–6-strategy sets before any selection-period source is downloaded, then run only that immutable universe and preserve non-promotion boundaries;
