...
DEC-232 implements the deterministic in-memory EXP-058 fit-regime-floor training core against merged DEC-231. It preserves the repaired EXP-057 six-regressor/reference/financial-gate/temporal-stability/validation/holdout pipeline and adds exactly one score: average the four downside-adjusted lower bounds within each of the three jackknife-excluded two-year fit regimes, then take the minimum of those three regime means. Ranking places this regime floor ahead of the existing lower-tail/breadth/residual-bound/support stack and freezes an eight-part cutoff reused unchanged in forward evaluation. Artifact loading, readiness execution, workflow dispatch, historical result execution, promotion, shadow/demo, broker mutation, live orders, real-money action, and trading remain locked. The next gate is a non-executable EXP-058 artifact/evidence contract only.
