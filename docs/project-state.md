...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED + EXP-045/EXP-046 CLOSED/REVIEWED + EXP-047 DEC-118 ONE-RUN AUTHORIZATION SOURCE FROZEN; NO EXP-047 RESULT
**Next milestone:** Merge DEC-118's single guarded EXP-047 historical run authorization. DEC-118 itself dispatches nothing. After merge, freeze a clean-main one-way operator that exposes exactly one dispatch only while no EXP-047 run exists and routes terminal evidence through DEC-117. The first manual-main attempt will consume the slot on any terminal outcome; no automatic retry/replacement. Promotion, shadow/demo execution, broker mutation, live orders, real-money trading, and trading authorization remain locked.
