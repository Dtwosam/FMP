...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED + EXP-045/EXP-046/EXP-047/EXP-048 CLOSED/REVIEWED + EXP-049 REGIME-UTILITY PROTOCOL FROZEN UNDER DEC-132; NO EXP-049 FIT OR RESULT
**Next milestone:** Merge DEC-132's source-only EXP-049 regime-utility protocol, then implement a separate deterministic training/evaluation core that fits exactly two 0.5-pip net-utility HGB regressors in each of the three frozen fit-regime windows and applies unanimous positive-utility consensus around the unchanged density/stability pipeline. No EXP-049 model fit, historical result execution, workflow, or run is authorized. Promotion, shadow/demo execution, broker mutation, live orders, real-money trading, and trading authorization remain locked.
