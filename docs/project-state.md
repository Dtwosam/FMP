...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED + EXP-045 CLOSED UNDER DEC-102 WITH SUCCESSFUL HISTORICAL RUN BUT NO VALIDATED MODEL CANDIDATE; PHASE 8B REMAINS LOCKED
**Next milestone:** Merge DEC-102 result review/closure. EXP-045 run `35911916239` completed successfully from `6d42a5053c5f2f696071715640dab24973a40517`, but the only selected challenger (GBPUSD 5m / 240m HGB at 0.6 confidence) failed validation; zero cells passed validation and no retrospective holdout candidate exists. The one-run authorization is consumed and the outer execution gate is closed. Any further learned-model research requires a separately predeclared successor experiment that treats EXP-045 as prior information. Phase 8B, promotion, shadow/demo execution, broker mutation, live orders, real-money trading, and trading authorization remain locked.
...
28. DEC-102 reviews successful run `35911916239`: all 18 cells and all 10 expected artifacts were persisted, but 17 cells had no challenger and the only selected GBPUSD 5m / 240m HGB challenger failed validation (83 validation candidates, approximately -1397.5 net pips at 0.5-pip slippage). No cell passed validation or retrospective holdout. EXP-045 is closed without rerun/replacement, and the consumed outer execution gate is set false.
...
Phase 8 is not PASS. Phase 8B source-free preparation is active under DEC-046 through DEC-049. EXP-044 feature and outcome data-preparation workflows completed successfully. One guarded EXP-044 V1 model-training workflow, run `35891605645`, executed under DEC-093 and failed; DEC-094 reviews that failure, records zero persisted model artifacts and no aggregate model result, and closes V1 execution without rerun or replacement authorization. DEC-095 through DEC-101 froze and authorized the one-shot EXP-045 successor path. That path executed exactly once as run `35911916239` from `6d42a5053c5f2f696071715640dab24973a40517` and completed successfully. DEC-102 verifies all 18 cell results and the exact 10-artifact inventory, records aggregate fingerprint `3e0ebac02dbba690b4c03dd10c3fdd30c5eb0d6356b881e38f9a3527f0135c55`, and closes EXP-045 because the sole selected GBPUSD 5m / 240m HGB challenger failed validation and no cell reached a validation/holdout pass. No rerun/replacement is authorized; promotion/shadow/demo/broker/live/real-money/trading paths remain locked. No prospective shadow campaign has begun.
