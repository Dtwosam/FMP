...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED + EXP-045/EXP-046/EXP-047 CLOSED/REVIEWED + EXP-048 REGIME-CONSENSUS PROTOCOL FROZEN UNDER DEC-123; NO EXP-048 FIT OR RESULT
**Next milestone:** Merge DEC-123's source-only EXP-048 fit-regime consensus protocol, then implement a separate deterministic training/evaluation core that adds only the three disjoint HGB fit-regime models and unanimous directional consensus around the unchanged density/stability pipeline. No EXP-048 model fit, historical result execution, workflow, or run is authorized. Promotion, shadow/demo execution, broker mutation, live orders, real-money trading, and trading authorization remain locked.
