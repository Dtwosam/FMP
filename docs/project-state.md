...
**Next milestone:** Merge DEC-134's source-only EXP-049 artifact/evidence contract, then freeze a manual-main EXP-049 workflow/CLI that binds the exact DEC-132 through DEC-134 source chain, accepted artifact identities, and pinned runtime while remaining non-dispatchable until terminal review and one-run authorization are separately predeclared. No historical result execution, dispatch, promotion, shadow/demo execution, broker mutation, live orders, real-money trading, or trading authorization is opened.
