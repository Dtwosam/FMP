...
6. DEC-050 / `EXP-20260922-021` is the active source-free milestone: implement the deterministic finite-sequence runtime/replay kernel over exact DEC-049 capture records, including live-shadow bar derivation, immutable champion strategy reconstruction, portfolio routing, shared-account virtual simulation, segment evidence, and byte-identical offline replay; no live capture command is authorized;
7. final Phase 8B acceptance remains a separate later decision. Legacy 8-week / 30-date / 40-trade, coverage, timing, spread-parity, profitability, drawdown, replay, and safety gates remain frozen; demo/live orders, broker mutation, real-money trading, Phase 9, active-champion mutation, acceptance, and promotion remain locked.
