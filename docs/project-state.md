...
**Phase status:** ACTIVE — EXP-044 V1 CLOSED; direct market-learning track closed by DEC-262; EXP-015 Stage A now has DEC-264 first-run governance plus DEC-265 clean-main one-way operator source, with the single authoritative slot still unconsumed; Stage B/C, DEC-042 portfolio selection, and DEC-045 Phase 8A acceptance remain blocked
**Next milestone:** Add a repository-hosted read-only proof for the exact DEC-265 EXP-015 Stage A `next` plan on merged main. Only after that proof succeeds may a separately reviewed one-shot executor source be considered. Phase 8B and all demo/live/broker/real-money paths remain locked.
