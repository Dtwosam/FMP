...
4. DEC-052 / `EXP-20260922-023` is the active milestone: implement the first explicit bounded quote-only `capture-segment` command with create-only raw JSONL, per-record fsync latency audit, fail-closed fixed-session validation, clean DEC-050 compile/replay closure, and immutable `fmp-phase8b-prospective-segment-v1` evidence;
