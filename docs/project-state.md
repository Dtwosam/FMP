...
71. DEC-145 predeclares the exact attempt-1 EXP-050 terminal review. A successful run must have the exact 11-job shape, all nine cell artifacts, the aggregate artifact, and DEC-143 evidence revalidation; non-success may preserve partial cell evidence but cannot claim aggregate evidence, and rerun/replacement remains unauthorized.
...
Phase 8 is not PASS. Phase 8B source-free preparation is active under DEC-046 through DEC-049. EXP-044 feature and outcome data-preparation workflows completed successfully. One guarded EXP-044 V1 model-training workflow, run `35891605645`, executed under DEC-093 and failed; DEC-094 reviews that failure, records zero persisted model artifacts and no aggregate model result, and closes V1 execution without rerun or replacement authorization. DEC-095 through DEC-101 freeze and authorize the single guarded EXP-045 historical model run. Run `35911916239` completed successfully at `6d42a5053c5f2f696071715640dab24973a40517`; DEC-102 records no accepted model challenger and DEC-103 freezes the post-result diagnostic. DEC-104 through DEC-110 freeze, authorize, and safely operate the single guarded EXP-046 historical model run. Run `35978474425` completed successfully at `dabafcc290d2b383531532d873c7d6c697198d5a`; DEC-111 reviews its complete 18-cell aggregate evidence and records no stable model challenger. DEC-112 freezes the EXP-045/EXP-046 cross-run reproducibility audit: HGB candidate decisions reproduce materially, while logistic family availability changes in five cells. DEC-113 through DEC-120 freeze, authorize, safely operate, and repair the single guarded EXP-047 HGB density experiment. Run `35993400007` completed successfully at `5c4d81c0ebc9f930b2361d54cb0245a3d8c886d2`; DEC-121 reviews its complete 18-cell aggregate evidence and records no stable model challenger. DEC-122 freezes the temporal-concentration diagnostic. DEC-123 through DEC-129 freeze, authorize, and safely operate the single guarded EXP-048 HGB regime-consensus experiment. Run `36006524422` completed successfully at `60b2796a64f0a4f7f95660d45ef7ab7fac519e9c`; DEC-130 reviews its complete 18-cell aggregate evidence and records no stable model challenger. DEC-131 freezes the post-result diagnostic: all 17 aggregate passes fail at least one financial stability window, 13 also fail candidate-share stability, and four fail financial stability only. The EXP-048 run slot is consumed. DEC-132 freezes source-only EXP-049, replacing direction-class fitting with paired 0.5-pip net-utility HGB regressors across the same three fit regimes while preserving every density/stability gate. DEC-133 implements the deterministic in-memory training/evaluation core. DEC-134 freezes the artifact-backed result-evidence contract and keeps the authoritative bundle non-executable before artifact loading. DEC-135 freezes the manual-main workflow, CLI, pinned runtime, and exact-source execution gate while keeping dispatch/result/fit authorization false. DEC-136 predeclares the exact attempt-1 terminal review, complete success evidence, partial-failure evidence, and no-rerun semantics before any run authorization. DEC-137 verifies zero prior manual-main EXP-049 runs, hardens the workflow with a first-run rejection guard, and opens exactly one outer historical result-producing slot without dispatching it. DEC-138 freezes the clean-main, double-plan one-way operator; only the exact zero-run state could expose the single dispatch, while active/terminal states route to observation or DEC-136 review with no second-run path. Run `36029925264` then completed successfully on attempt 1 at `eeb735bca7d38c3246f22a9606dfafe9c3df8279`. DEC-139 reviews the complete 18-cell / 108-regressor evidence, records eight aggregate-pass regime-utility variants with zero temporal-stability passes, records no stable challenger, and closes the consumed EXP-049 run slot. DEC-140 then freezes the post-result diagnostic: 31 of 54 predeclared budget variants are unavailable for insufficient utility-eligible rows; eight of 23 available variants pass the aggregate gate; all eight fail the 10% share floor in both 2021 halves and also fail at least one financial stability window; all eight passes are 240m and six fail financial signs in 2022 H2. DEC-141 opens source-only EXP-050 with three four-year leave-one-regime-out utility-fit views while preserving the positive-utility threshold, unanimous consensus, minimum-view score, budgets, aggregate/stability gates, chronology, and forward semantics. DEC-142 implements the deterministic source-only training/evaluation core, reusing the exact DEC-133 utility/gate helpers under blob binding while changing only jackknife fit-view assembly and view-aware result identity. DEC-143 freezes the artifact-backed runner/evidence contract, requiring exact 18-cell / 108-regressor completeness, exact jackknife-view identities, complete consensus/budget/financial/stability/status evidence, and canonical cell/aggregate fingerprints while keeping the authoritative bundle locked before artifact loading. DEC-144 freezes the input-free manual-main workflow, CLI, pinned Python 3.12.14 runtime, and exact-source execution gate with no first-run guard and all dispatch/result/fit flags false. DEC-145 predeclares the exact attempt-1 terminal review: success requires the exact 11-job shape, all nine cell artifacts, the aggregate artifact, and DEC-143 evidence revalidation; non-success may preserve partial cell evidence only, and rerun/replacement remains closed. The next gate is an independent zero-run verification followed by a separate one-run authorization/first-run guard decision that must not dispatch the workflow. Promotion/shadow/demo/broker/live/real-money/trading paths remain locked. No prospective shadow campaign has begun.
