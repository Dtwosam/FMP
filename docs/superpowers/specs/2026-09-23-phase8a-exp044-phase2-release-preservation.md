...
DEC-084 adds a manual preservation workflow that republishes the exact already accepted Phase 2 ZIP bytes as GitHub release assets before expiry.
...
DEC-081/082/083 operator behavior remains intact.

DEC-084 adds one dry-run-by-default operator command:
...
The command keeps the same clean-main, exact-origin, GitHub-CLI-auth, source-preflight, and duplicate-run safeguards already used by the EXP-044 operator.
...
3. dispatch outcomes only after exact feature evidence is verified;
4. inspect the complete readiness chain with DEC-083.

DEC-084 itself produces no market-research result.
