...
**Exact-gap execution safety:** a fresh plan is not sufficient by age alone. Before any exact-gap source request, the workflow pages the `phase1-full-acquisition` run history, validates consistent GitHub `total_count`, exact flattened-count reconciliation, and unique positive run IDs, then rejects the plan if another source-capable run was updated after the plan audit. The current exact-gap run and push-triggered `[phase1-no-source]` runs do not invalidate the plan. Manual `workflow_dispatch` runs remain source-capable even if their underlying head commit contains `[phase1-no-source]`. GitHub workflow `updated_at` values must use UTC-zero offsets and are only second-granularity, so a source-capable run reported in the same UTC second as the plan audit is conservatively treated as intervening; generate a fresh audit rather than guessing event order.


**Ingest deployment preflight:** every cloud-mirrored `fetch` / `fetch-plan` run from current `main` authenticates to `fmp-raw-ingest` before source access and requires the exact protocol response `{"status":"ready","protocol":"fmp-raw-ingest-v2"}`. The currently deployed pre-hardening v3 endpoint does not implement this GET handshake, so future main-branch source runs fail before Dukascopy until the tested repository ingest version is deployed.
