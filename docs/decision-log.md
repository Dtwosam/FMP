...
Live workflow state is one-way: `MISSING`, `IN_PROGRESS`, or `TERMINAL`. More than one manual-main EXP-060 workflow run fails closed. Only `MISSING` may expose the exact dispatch command; `IN_PROGRESS` and `TERMINAL` expose no dispatch or replacement action. Terminal state routes through DEC-257.

Operator source `src/fmp/market_learning/model_successor_fit_temporal_residual_regime_balance_utility_repair_operator.py` is blob `504bf66e6774c2fcc46dd6f21c68762ec251ccd0`. Public CLI `scripts/phase8a_exp060_operator.py` is blob `3d0647254e548d719d47981b11e1cd90028bd657`. Focused tests `tests/test_phase8a_exp060_operator.py` are blob `091bd1f2e0498c57885f869dc34cf9d8d10d626a` and include an integration check against the real DEC-258 source gate.
