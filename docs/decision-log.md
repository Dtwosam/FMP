...
- frozen JSON version fields are exact integers, not Python-equality lookalikes: boolean `true` is rejected for exact-gap `plan_version` and manifest `manifest_version`;
