...
- provenance carries a valid acquisition baseline run ID/completion time, binds final acquisition-history guard version 1 and its UTC start timestamp into the same JSON, keeps those times non-future, and confirms acquisition remained unchanged during verification;
