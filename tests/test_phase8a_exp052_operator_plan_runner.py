...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp052-operator-plan.yml"
)


class Exp052OperatorPlanRunnerTests(unittest.TestCase):
    def test_runner_is_read_only_and_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("name: phase8a-exp052-operator-plan", text)
    ...
    def test_runner_executes_only_dec169_next_plan(self) -> None:
        ...
        self.assertNotIn(
            "phase8a_exp052_operator.py advance",
            text,
        )
        ...
        self.assertIn(
            'assert plan["operator_decision"] == "DEC-169"',
            text,
        )
        self.assertIn(
