...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp055-operator-plan.yml"
)


class Exp055OperatorPlanRunnerTests(unittest.TestCase):
    def test_runner_is_read_only_and_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("name: phase8a-exp055-operator-plan", text)
    ...
    def test_runner_executes_only_dec204_next_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
        ...
        self.assertIn(
            'assert plan["operator_decision"] == "DEC-204"',
            text,
        )
        self.assertIn(
