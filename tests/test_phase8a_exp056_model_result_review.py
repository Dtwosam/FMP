...
class Exp056ModelTerminalReviewTests(unittest.TestCase):
    ...
    def test_success_requires_complete_evidence_and_revalidation(self) -> None:
        ...
        with patch(
            "fmp.market_learning."
            "model_successor_fit_temporal_residual_lower_tail_utility_result_review."
        ...
        self.assertEqual(
            result["stage"],
            "FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_MODEL_RESULT_REVIEW_REQUIRED",
        )
    ...
    def test_terminal_failure_preserves_partial_evidence_without_retry(self) -> None:
        ...
        self.assertEqual(
            result["stage"],
            "FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_MODEL_RUN_FAILURE_REVIEW_REQUIRED",
        )
    ...
    def test_unexpected_artifact_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            ValueError,
            "unexpected EXP-056 terminal review artifact",
        ):
            validate_fit_temporal_residual_lower_tail_utility_model_terminal_review(
                run=_run("failure"),
                jobs_payload=_jobs(
