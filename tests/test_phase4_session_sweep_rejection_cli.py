...
SCRIPT = Path("scripts/phase4_session_sweep_rejection.py")


class Phase4SessionSweepRejectionCliTests(unittest.TestCase):
    ...
    def test_final_split_is_rejected_before_data_access(self) -> None:
        completed = subprocess.run(
            [
                sys.executable,
                str(SCRIPT),
                ...
                "/definitely/not/read",
                "--manifest",
                "/definitely/not/read/manifest.json",
                "--symbol",
                "EURUSD",
                "--timeframe",
                "1h",
                ...
                "final",
                "--out",
                "/tmp/phase4-session-sweep-final-must-not-run",
                "--code-commit",
                "test-commit",
            ],
            check=False,
    ...
    def test_script_delegates_to_frozen_runner_and_is_source_free(self) -> None:
        text = SCRIPT.read_text(encoding="utf-8")
        self.assertIn("run_session_sweep_rejection_grid", text)
