...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp055-operator-execute.yml"
)


class Exp055OperatorExecutorTests(unittest.TestCase):
    def test_executor_is_single_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "name: phase8a-exp055-operator-execute",
            text,
        )
    ...
    def test_executor_uses_only_existing_dec204_execute_path(
        self,
    ) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_executor_binds_successful_read_only_plan(
        ...
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("36162871611", text)
        self.assertIn("10875772980", text)
        self.assertIn(
    ...
    def test_executor_keeps_checkout_immutable(self) -> None:
        ...
        self.assertNotIn(
            "requirements/exp055-model-run.txt -e .",
            text,
        )
        self.assertIn(
    ...
    def test_executor_receipt_claims_dispatch_only(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            'assert receipt["operator_decision"] == "DEC-204"',
            text,
        )
        self.assertIn(
