...
from fmp.market_learning.model_successor_temporal_calibrated_utility_result_decision import (
    ...
    REVIEWED_AGGREGATE_PASS_BUDGET,
    REVIEWED_AGGREGATE_PASS_CELL,
...
class Exp051ResultDecisionTests(unittest.TestCase):
    ...
    def test_exact_review_closes_without_stable_candidate(
        ...
        self.assertEqual(
            tuple(report["reviewed_aggregate_pass_cell"]),
            REVIEWED_AGGREGATE_PASS_CELL,
        )
        self.assertEqual(
            report["reviewed_aggregate_pass_budget"],
            REVIEWED_AGGREGATE_PASS_BUDGET,
        )
        self.assertEqual(
            REVIEWED_AGGREGATE_PASS_BUDGET,
    ...
    def test_calibration_reference_count_drift_fails_closed(
        self,
        terminal_review,
    ) -> None:
        drifted = _terminal_review()
        drifted["verified_calibration_reference_count"] = 107
        terminal_review.return_value = drifted

        with self.assertRaisesRegex(
            ValueError,
            "verified_calibration_reference_count mismatch",
        ):
            validate_reviewed_temporal_calibrated_utility_model_result(
                run=_run(),
                jobs_payload={},
    ...
    def test_stable_pass_drift_fails_closed(
        self,
        terminal_review,
    ) -> None:
        drifted = _terminal_review()
