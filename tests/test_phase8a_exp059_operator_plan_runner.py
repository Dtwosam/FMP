...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp059-operator-plan.yml"
)
...
class Exp059OperatorPlanRunnerTests(unittest.TestCase):
    def test_runner_is_read_only_and_main_push_scoped(self) -> None:
        ...
        self.assertIn(
            ...
            "model_successor_fit_temporal_residual_regime_balance_utility_operator.py",
            text,
        )
    ...
    def test_runner_executes_only_dec248_next_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "python scripts/phase8a_exp059_operator.py next",
            text,
        )
        self.assertNotIn(
            "phase8a_exp059_operator.py advance",
            text,
        )
