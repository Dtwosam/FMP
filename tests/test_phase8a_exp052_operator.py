...
from fmp.market_learning.model_successor_fit_temporal_support_utility_operator import (
...
SCRIPT = ROOT / "scripts/phase8a_exp052_operator.py"
SHA = "a" * 40
...
def _run(
    *,
    ...
    return {
        "id": 123,
        "name": FIT_TEMPORAL_SUPPORT_UTILITY_MODEL_WORKFLOW_NAME,
        "path": FIT_TEMPORAL_SUPPORT_UTILITY_MODEL_WORKFLOW_PATH,
...
class Exp052FitTemporalSupportUtilityOperatorTests(unittest.TestCase):
    ...
    def test_checkout_requires_clean_exact_main(self) -> None:
        report = validate_fit_temporal_support_utility_operator_checkout(
            branch="main",
            head_sha=SHA,
            origin_main_sha=SHA,
            porcelain_status="",
    ...
    def test_exact_workflow_endpoints_and_dispatch(self) -> None:
        ...
        endpoint = fit_temporal_support_utility_model_runs_endpoint()
        self.assertIn(
        ...
        self.assertEqual(
            fit_temporal_support_utility_model_run_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123",
        )
        self.assertEqual(
            fit_temporal_support_utility_model_run_jobs_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/jobs?per_page=100",
        )
        self.assertEqual(
            fit_temporal_support_utility_model_run_artifacts_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/artifacts?per_page=100",
        )
        self.assertEqual(
            fit_temporal_support_utility_artifact_download_endpoint(456),
    ...
    def test_run_selection_forbids_second_manual_main_run(self) -> None:
        ...
        selected = select_fit_temporal_support_utility_manual_main_run(
            {"workflow_runs": [_run()]}
        )
    ...
    def test_run_classification_is_one_way(self) -> None:
        self.assertEqual(
            classify_fit_temporal_support_utility_model_run(None)[
                "run_state"
            ],
            "MISSING",
        )
    ...
    def test_missing_run_is_only_dispatchable_state(self) -> None:
        report = build_fit_temporal_support_utility_operator_report(
            checkout=_checkout(),
            run=None,
        )
        self.assertEqual(
        ...
        active = build_fit_temporal_support_utility_operator_report(
            checkout=_checkout(),
            run=_run(status="in_progress", conclusion=None),
        )
        self.assertEqual(
            active["stage"],
            "FIT_TEMPORAL_SUPPORT_UTILITY_MODEL_RUN_IN_PROGRESS",
        )
        self.assertIsNone(
            dispatch_command_for_fit_temporal_support_utility_report(
                active
        ...
        terminal = build_fit_temporal_support_utility_operator_report(
            checkout=_checkout(),
    ...
    def test_gate_metadata_matches_dec168_source_shape(self) -> None:
        gate = {
            ...
            "dec166_merged_commit": "4" * 40,
            "dec167_merged_commit": "5" * 40,
            "dec166_workflow_blob_sha": "6" * 40,
            "dec166_cli_blob_sha": "7" * 40,
            "dec166_gate_blob_sha": "8" * 40,
            "dec167_review_blob_sha": "9" * 40,
            "fit_temporal_support_utility_workflow_blob_sha": "a" * 40,
            "fit_temporal_support_utility_cli_blob_sha": "b" * 40,
        }
        metadata = fit_temporal_support_utility_operator_gate_metadata(
            gate
        )
        self.assertEqual(metadata["dec167_merged_commit"], "5" * 40)
        self.assertEqual(
            metadata["fit_temporal_support_utility_workflow_blob_sha"],
            "a" * 40,
        )
        drifted = dict(gate)
        drifted.pop("dec167_merged_commit")
        with self.assertRaisesRegex(
            ValueError,
    ...
    def test_aggregate_artifact_selection_is_exact(self) -> None:
        ...
        selected = select_fit_temporal_support_utility_aggregate_artifact(
            {
                "artifacts": [
                    {
                        "id": 55,
