...
class Exp060RegimeBalanceRepairArtifactContractTests(unittest.TestCase):
    def test_source_binding_and_authorization_are_exact(self) -> None:
        ...
        self.assertEqual(
            report["dec254_training_core_blob_sha"],
            DEC254_TRAINING_CORE_BLOB_SHA,
        )
        self.assertEqual(
            DEC254_TRAINING_CORE_BLOB_SHA,
    ...
    def test_regime_balance_provenance_drift_fails_closed(self) -> None:
        ...
        cells[0]["dec253_protocol_blob_sha"] = "0" * 40
        supplied = dict(cells[0])
        supplied.pop("result_fingerprint")
        cells[0]["result_fingerprint"] = hashlib.sha256(
            _canonical_json(supplied)
    ...
    def test_authoritative_bundle_rejects_before_execution(self) -> None:
        ...
        with self.assertRaisesRegex(
            PermissionError,
            "DEC-255 source is non-executable",
        ):
            run_authoritative_fit_temporal_residual_regime_balance_utility_repair_model_bundle()
