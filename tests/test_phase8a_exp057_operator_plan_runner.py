...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp057-operator-plan.yml"
)


class Exp057OperatorPlanRunnerTests(unittest.TestCase):
    def test_runner_is_read_only_and_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("name: phase8a-exp057-operator-plan", text)
        ...
        self.assertIn(
            ...
            "model_successor_fit_temporal_residual_lower_tail_utility_repair_operator.py",
            text,
        )
    ...
    def test_runner_executes_only_dec226_next_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "python scripts/phase8a_exp057_operator.py next",
            text,
        )
        self.assertNotIn(
            "phase8a_exp057_operator.py advance",
            text,
        )
