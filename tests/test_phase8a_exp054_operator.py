...
SCRIPT = ROOT / "scripts/phase8a_exp054_operator.py"
SHA = "a" * 40
...
def _run(
    *,
    ...
    return {
        "id": 123,
        "name": FIT_TEMPORAL_RESIDUAL_BOUND_UTILITY_MODEL_WORKFLOW_NAME,
        "path": FIT_TEMPORAL_RESIDUAL_BOUND_UTILITY_MODEL_WORKFLOW_PATH,
...
def _checkout() -> dict[str, object]:
    return {
        ...
        "dec191_merged_commit": DEC179_MERGED_COMMIT,
        "branch": "main",
        "head_sha": SHA,
        "clean_worktree": True,
...
class Exp053FitTemporalSupportUtilityOperatorTests(unittest.TestCase):
    def test_authorization_merge_identity_is_frozen(self) -> None:
        self.assertEqual(
            ...
            "70925fdc418c76ab378a56235a766435fe6aedb6",
        )

    def test_checkout_requires_clean_exact_main(self) -> None:
        report = validate_fit_temporal_residual_bound_utility_operator_checkout(
            branch="main",
            head_sha=SHA,
            origin_main_sha=SHA,
            porcelain_status="",
    ...
    def test_exact_workflow_endpoints_and_dispatch(self) -> None:
        ...
        self.assertEqual(
            fit_temporal_residual_bound_utility_model_run_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123",
        )
        self.assertEqual(
            fit_temporal_residual_bound_utility_model_run_jobs_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/jobs?per_page=100",
        )
        self.assertEqual(
            fit_temporal_residual_bound_utility_model_run_artifacts_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/artifacts?per_page=100",
        )
        self.assertEqual(
            fit_temporal_residual_bound_utility_artifact_download_endpoint(456),
    ...
    def test_run_selection_forbids_second_manual_main_run(self) -> None:
        ...
        selected = select_fit_temporal_residual_bound_utility_manual_main_run(
            {"workflow_runs": [_run()]}
        )
    ...
    def test_run_classification_is_one_way(self) -> None:
        self.assertEqual(
            classify_fit_temporal_residual_bound_utility_model_run(None)[
                "run_state"
            ],
            "MISSING",
        )
    ...
    def test_missing_run_is_only_dispatchable_state(self) -> None:
        ...
        active = build_fit_temporal_residual_bound_utility_operator_report(
            checkout=_checkout(),
            run=_run(status="in_progress", conclusion=None),
        )
        self.assertEqual(
            active["stage"],
            "FIT_TEMPORAL_RESIDUAL_BOUND_UTILITY_MODEL_RUN_IN_PROGRESS",
        )
        self.assertIsNone(
            dispatch_command_for_fit_temporal_residual_bound_utility_report(
                active
        ...
        terminal = build_fit_temporal_residual_bound_utility_operator_report(
            checkout=_checkout(),
    ...
    def test_gate_metadata_matches_dec191_source_shape(self) -> None:
        gate = {
            ...
            "dec189_workflow_blob_sha": "5" * 40,
            "dec189_cli_blob_sha": "6" * 40,
            "dec189_gate_blob_sha": "7" * 40,
            "dec190_review_blob_sha": "8" * 40,
            "fit_temporal_residual_bound_utility_workflow_blob_sha": "a" * 40,
            "fit_temporal_residual_bound_utility_cli_blob_sha": "b" * 40,
        }
        ...
        drifted = dict(gate)
        drifted.pop("dec190_merged_commit")
        with self.assertRaisesRegex(
            ValueError,
    ...
    def test_public_cli_binds_dec188_and_dec190(self) -> None:
