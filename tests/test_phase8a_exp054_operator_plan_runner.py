...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp054-operator-plan.yml"
)


class Exp054OperatorPlanRunnerTests(unittest.TestCase):
    def test_runner_is_read_only_and_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("name: phase8a-exp054-operator-plan", text)
        self.assertIn("push:", text)
        self.assertIn("branches:", text)
        self.assertIn("- main", text)
        self.assertIn(
            "- .github/workflows/phase8a-exp054-operator-plan.yml",
            text,
        )
    ...
    def test_runner_executes_only_dec180_next_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "python scripts/phase8a_exp054_operator.py next",
            text,
        )
        self.assertNotIn(
            "phase8a_exp054_operator.py advance",
            text,
        )
        ...
        self.assertIn(
            'assert plan["operator_decision"] == "DEC-192"',
            text,
        )
        self.assertIn(
    ...
    def test_runner_persists_plan_and_downstream_locks(self) -> None:
        ...
        self.assertIn(
            "exp054-dec180-read-only-operator-plan-",
            text,
        )
        self.assertIn(
