...
class Exp056ResidualLowerTailProtocolTests(unittest.TestCase):
    def test_identity_and_fingerprint_are_frozen(self) -> None:
        ...
        first = fit_temporal_residual_lower_tail_utility_protocol_fingerprint()
        second = fit_temporal_residual_lower_tail_utility_protocol_fingerprint()
        self.assertEqual(first, second)
        self.assertEqual(len(first), 64)
        int(first, 16)

    def test_lower_tail_reuses_exact_twelve_predecessor_bounds(self) -> None:
        payload = fit_temporal_residual_lower_tail_utility_protocol_payload()
        tail = payload["fit_temporal_residual_lower_tail"]
        self.assertEqual(
            FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_BOUND_COUNT_PER_ROW,
            12,
        )
        self.assertEqual(FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_COUNT, 3)
        self.assertEqual(FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_FRACTION, 0.25)
        self.assertEqual(tail["source_lower_bound_count_per_row"], 12)
        self.assertEqual(tail["lower_tail_count"], 3)
        self.assertEqual(tail["lower_tail_fraction"], 0.25)
        self.assertEqual(tail["sort_order"], "ascending_lower_bounds")
        self.assertEqual(tail["aggregation"], "arithmetic_mean")
        self.assertFalse(tail["interpolation_used"])
        self.assertFalse(tail["trimming_used"])
        self.assertFalse(tail["winsorization_used"])
        self.assertTrue(tail["uses_existing_exp055_lower_bounds"])
        self.assertFalse(tail["creates_new_reference_vectors"])
        self.assertFalse(tail["uses_realized_selection_outcomes"])
        self.assertFalse(
            tail["uses_realized_validation_outcomes_for_ranking"]
        )
        self.assertFalse(
            tail["uses_realized_holdout_outcomes_for_ranking"]
        )
        self.assertFalse(tail["selection_window_identity_enters_ranking"])
        self.assertFalse(tail["selection_window_quota_used"])
        self.assertFalse(tail["stability_gate_changed"])

    def test_exp055_breadth_and_residual_blocks_are_retained_exactly(self) -> None:
        ...
        self.assertEqual(
            payload["fit_temporal_residual_breadth"],
            predecessor["fit_temporal_residual_breadth"],
        )
        utility = payload["utility_consensus"]
        self.assertTrue(utility["fit_temporal_residual_bound_retained"])
        self.assertTrue(utility["fit_temporal_residual_breadth_retained"])
        self.assertTrue(utility["fit_temporal_residual_lower_tail_added"])
        self.assertEqual(utility["ranking_score_rule"], RANKING_RULE)

    def test_ranking_and_cutoff_are_lower_tail_first(self) -> None:
        payload = fit_temporal_residual_lower_tail_utility_protocol_payload()
    ...
    def test_forward_stages_reuse_frozen_lower_tail_cutoff(self) -> None:
        payload = fit_temporal_residual_lower_tail_utility_protocol_payload()
        validation = payload["validation"]
        holdout = payload["retrospective_holdout"]
        expected = "frozen_exp055_twelve_fit_half_year_lower_bounds"
        self.assertEqual(
            validation["fit_temporal_residual_lower_tail_source"],
            expected,
        )
        self.assertFalse(
            validation[
                "rebuild_fit_temporal_residual_lower_tail_on_validation"
            ]
        )
        self.assertTrue(
            validation[
                "selection_derived_residual_lower_tail_cutoff_reused"
            ]
        )
        self.assertEqual(
            holdout["fit_temporal_residual_lower_tail_source"],
            expected,
        )
        self.assertFalse(
            holdout[
                "rebuild_fit_temporal_residual_lower_tail_on_holdout"
            ]
        )
        self.assertTrue(
            holdout[
    ...
    def test_only_lower_tail_protocol_change_is_open(self) -> None:
        payload = fit_temporal_residual_lower_tail_utility_protocol_payload()
        auth = payload["authorization"]
        self.assertTrue(FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_AUTHORIZED)
        self.assertTrue(
            auth["fit_temporal_residual_lower_tail_authorized"]
        )
