...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp060-operator-execute.yml"
)


class Exp060OperatorExecutorTests(unittest.TestCase):
    def test_executor_is_single_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "name: phase8a-exp060-operator-execute",
            text,
        )
    ...
    def test_executor_uses_only_existing_dec259_execute_path(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_executor_binds_successful_dec260_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("36258921030", text)
        self.assertIn("10911029039", text)
        self.assertIn(
            "0339c58f59206b6e70fb5019be403ee9ffdd1a34",
            text,
        )
        self.assertIn(
            "sha256:b51e691984368757138913d646c4561150b02e6afd437773a3758e5559f3748f",
            text,
        )
        self.assertIn(
    ...
    def test_executor_verifies_plan_zip_digest_and_content(self) -> None:
        ...
        self.assertIn("dec260-plan.zip", text)
    ...
    def test_executor_evidence_stays_outside_checkout(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            '"$RUNNER_TEMP/exp060-executor-evidence"',
            text,
        )
        self.assertIn(
            "path: ${{ runner.temp }}/exp060-executor-evidence",
            text,
        )
        self.assertNotIn(
            "path: exp060-executor-evidence",
            text,
        )
