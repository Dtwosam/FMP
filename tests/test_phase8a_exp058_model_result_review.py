...
class Exp058ModelTerminalReviewTests(unittest.TestCase):
    ...
    def test_success_requires_complete_evidence_and_revalidation(self) -> None:
        ...
        with patch(
            "fmp.market_learning."
            "model_successor_fit_temporal_residual_regime_floor_utility_result_review."
    ...
    def test_unexpected_artifact_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            ValueError,
            "unexpected EXP-058 terminal review artifact",
        ):
            validate_fit_temporal_residual_regime_floor_utility_model_terminal_review(
                run=_run("failure"),
                jobs_payload=_jobs(
