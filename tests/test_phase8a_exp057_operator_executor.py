...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp057-operator-execute.yml"
)


class Exp057OperatorExecutorTests(unittest.TestCase):
    def test_executor_is_single_main_push_scoped(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "name: phase8a-exp057-operator-execute",
            text,
        )
    ...
    def test_executor_binds_successful_dec227_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("36192020278", text)
        self.assertIn("10888591924", text)
        self.assertIn(
            "7fc1396bbe983070dcbed410f43ce779f286986d",
            text,
        )
        self.assertIn(
            "sha256:92904a1e937494fcb065c414ac70fef9e4d4907bc86510d4bfaa98943982b9cf",
            text,
        )
        self.assertIn(
            "exp057-dec226-read-only-operator-plan-",
            text,
        )
        self.assertIn(
        ...
        self.assertIn(
            'assert plan["operator_decision"] == "DEC-226"',
            text,
        )
        self.assertIn(
    ...
    def test_executor_keeps_checkout_immutable(self) -> None:
        ...
        self.assertNotIn(
            "requirements/exp057-model-run.txt -e .",
            text,
        )
        self.assertIn(
    ...
    def test_executor_evidence_stays_outside_checkout(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            '"$RUNNER_TEMP/exp057-executor-evidence"',
            text,
        )
        self.assertIn(
            "path: ${{ runner.temp }}/exp057-executor-evidence",
            text,
        )
        self.assertNotIn(
            "path: exp057-executor-evidence",
            text,
        )
