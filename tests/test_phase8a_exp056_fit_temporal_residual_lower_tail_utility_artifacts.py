...
from fmp.market_learning.model_successor_fit_temporal_residual_lower_tail_utility_artifacts import (
...
def _cell(symbol: str, timeframe: str, horizon: int) -> dict[str, object]:
    row: dict[str, object] = {
        ...
        "selection": {
            ...
            "fit_temporal_residual_lower_tail_utility_consensus_digest": SHA,
            "variants": [
                {
...
class Exp056ArtifactContractTests(unittest.TestCase):
    ...
    def test_septuple_cutoff_is_exact_and_finite(self) -> None:
        raw = {
        ...
        self.assertEqual(
            validate_residual_lower_tail_cutoff(raw),
            (-0.4, 0.75, -0.2, 0.8, 0.7, 0.6, 0.1),
        )
    ...
    def test_authoritative_bundle_rejects_before_execution(self) -> None:
        ...
        with self.assertRaisesRegex(
            PermissionError,
            "DEC-211 source is non-executable",
        ):
            run_authoritative_fit_temporal_residual_lower_tail_utility_model_bundle()
