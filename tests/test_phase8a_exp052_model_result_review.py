...
class Exp052ModelTerminalReviewTests(unittest.TestCase):
    ...
    def test_success_requires_complete_aggregate_and_revalidates_dec165(
        self,
    ) -> None:
        ...
        with patch(
            "fmp.market_learning."
            "model_successor_fit_temporal_support_utility_result_review."
    ...
    def test_unexpected_artifact_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            ValueError,
            "unexpected EXP-052 terminal review artifact",
        ):
            validate_fit_temporal_support_utility_model_terminal_review(
                run=_run("failure"),
                jobs_payload=_jobs(
