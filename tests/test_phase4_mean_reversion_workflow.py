...
WORKFLOW = Path(".github/workflows/phase4-mean-reversion.yml")


class Phase4MeanReversionWorkflowTests(unittest.TestCase):
    def test_workflow_is_source_free_fixed_matrix_and_fail_closed(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        lowered = text.lower()
        self.assertIn("name: phase4-mean-reversion", text)
        ...
        self.assertIn("python scripts/phase4_mean_reversion.py", text)
        self.assertIn('--code-commit "${GITHUB_SHA}"', text)
        self.assertIn("fmp-phase4-mean-reversion-grid-v1", text)
        self.assertIn("len(benchmark['configuration_rows']) == 18", text)
    ...
    def test_push_paths_are_scoped_to_mean_reversion_surface(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("paths:", text)
        for path in (
            "src/fmp/research/mean_reversion.py",
            "src/fmp/strategies/mean_reversion.py",
            "scripts/phase4_mean_reversion.py",
            ".github/workflows/phase4-mean-reversion.yml",
        ):
