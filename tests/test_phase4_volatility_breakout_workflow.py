...
WORKFLOW = Path(".github/workflows/phase4-volatility-breakout.yml")


class Phase4VolatilityBreakoutWorkflowTests(unittest.TestCase):
    def test_workflow_is_source_free_fixed_matrix_and_fail_closed(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        lowered = text.lower()
        self.assertIn("name: phase4-volatility-breakout", text)
        ...
        self.assertIn("python scripts/phase4_volatility_breakout.py", text)
        self.assertIn('--code-commit "${GITHUB_SHA}"', text)
        self.assertIn("fmp-phase4-volatility-breakout-grid-v1", text)
        self.assertIn("len(benchmark['configuration_rows']) == 9", text)
        self.assertIn("for range_multiplier in (1.0, 1.5, 2.0):", text)
    ...
    def test_push_paths_are_scoped_to_volatility_breakout_surface(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("paths:", text)
        for path in (
            "src/fmp/research/volatility_breakout.py",
            "src/fmp/strategies/volatility_breakout.py",
            "scripts/phase4_volatility_breakout.py",
            ".github/workflows/phase4-volatility-breakout.yml",
        ):
