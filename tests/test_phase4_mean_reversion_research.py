...
from fmp.research.mean_reversion import (
    MEAN_REVERSION_GRID,
    SLIPPAGE_SCENARIOS,
    run_mean_reversion_grid,
)
...
def no_trade(config) -> tuple[SignalCandidate, ...]:
    return (
        SignalCandidate(
            candidate_id=f"NT-{config.timeframe}-{config.lookback_hours}-{config.threshold_sigma}",
            symbol="EURUSD",
...
class Phase4MeanReversionResearchTests(unittest.TestCase):
    def test_grid_and_slippage_are_exactly_frozen(self) -> None:
        self.assertEqual(
            MEAN_REVERSION_GRID,
            ((4, 1.5), (4, 2.0), (8, 1.5), (8, 2.0), (16, 1.5), (16, 2.0)),
        )
    ...
    def test_runner_reuses_candidates_and_preserves_all_18_rows(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            generated: list[tuple[int, float, str]] = []

            def fake_generate(_bars, *, config):
                generated.append((config.lookback_hours, config.threshold_sigma, config.timeframe))
                return no_trade(config)

            with patch("fmp.research.mean_reversion.load_processed_bars", return_value=loaded) as load_mock, patch(
                "fmp.research.mean_reversion.generate_mean_reversion_candidates",
                side_effect=fake_generate,
            ) as generate_mock:
                result = run_mean_reversion_grid(
                    dataset_root=root,
                    manifest_path=manifest,
                    symbol="EURUSD",
                    timeframe="1h",
                    split_name="development",
                    code_commit="mean-reversion-test-commit",
                )

        self.assertEqual(load_mock.call_count, 1)
        self.assertEqual(generate_mock.call_count, 6)
        self.assertEqual(generated, [(lookback, threshold, "1h") for lookback, threshold in MEAN_REVERSION_GRID])
        rows = result["configuration_rows"]
        self.assertEqual(len(rows), 18)
        self.assertEqual(
            [(row["lookback_hours"], row["threshold_sigma"], row["slippage_pips"]) for row in rows],
            [(lookback, threshold, slip) for lookback, threshold in MEAN_REVERSION_GRID for slip in SLIPPAGE_SCENARIOS],
        )
        self.assertEqual(result["protocol"], "fmp-phase4-mean-reversion-grid-v1")
        self.assertEqual(result["excluded_incomplete_bar_count"], 4)
        self.assertEqual(result["code_commit"], "mean-reversion-test-commit")
        digests: dict[tuple[int, float], set[str]] = {}
        for row in rows:
            key = (row["lookback_hours"], row["threshold_sigma"])
    ...
    def test_final_split_fails_before_loader_is_called(self) -> None:
        with tempfile.TemporaryDirectory() as tmp, patch(
            "fmp.research.mean_reversion.load_processed_bars"
        ) as load_mock:
            root = Path(tmp)
            manifest = root / "manifest.json"
