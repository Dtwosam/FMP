...
WORKFLOW = (
    ROOT
    / ".github/workflows/"
    "phase8a-exp058-operator-plan.yml"
)
...
class Exp058OperatorPlanRunnerTests(unittest.TestCase):
    def test_runner_is_read_only_and_main_push_scoped(self) -> None:
        ...
        self.assertIn("name: phase8a-exp058-operator-plan", text)
        ...
        self.assertIn(
            ...
            "model_successor_fit_temporal_residual_regime_floor_utility_operator.py",
            text,
        )
    ...
    def test_runner_executes_only_dec237_next_plan(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "python scripts/phase8a_exp058_operator.py next",
            text,
        )
        self.assertNotIn(
            "phase8a_exp058_operator.py advance",
            text,
        )
