...
SCRIPT = Path("scripts/phase4_mean_reversion.py")


class Phase4MeanReversionCliTests(unittest.TestCase):
    ...
    def test_final_split_is_rejected_by_parser_before_data_access(self) -> None:
        completed = subprocess.run(
            [
                ...
                "--timeframe", "1h",
                "--split", "final",
                "--out", "/tmp/phase4-mean-reversion-final-must-not-run",
    ...
    def test_script_delegates_to_frozen_runner_and_writer_and_is_source_free(self) -> None:
        ...
        self.assertIn("run_mean_reversion_grid", text)
