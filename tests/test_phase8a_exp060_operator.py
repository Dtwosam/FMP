...
SCRIPT = ROOT / "scripts/phase8a_exp060_operator.py"
SHA = "a" * 40
...
def _run(
    *,
    ...
    return {
        "id": 123,
        "name": FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_WORKFLOW_NAME,
        "path": FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_WORKFLOW_PATH,
...
class Exp060ResidualRegimeBalanceRepairOperatorTests(unittest.TestCase):
    ...
    def test_checkout_requires_clean_exact_main(self) -> None:
        report = validate_fit_temporal_residual_regime_balance_utility_repair_operator_checkout(
            branch="main",
            head_sha=SHA,
            origin_main_sha=SHA,
            porcelain_status="",
    ...
    def test_exact_workflow_endpoints_and_dispatch(self) -> None:
        ...
        endpoint = fit_temporal_residual_regime_balance_utility_repair_model_runs_endpoint()
        self.assertIn(
        ...
        self.assertEqual(
            fit_temporal_residual_regime_balance_utility_repair_model_run_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123",
        )
        self.assertEqual(
            fit_temporal_residual_regime_balance_utility_repair_model_run_jobs_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/jobs?per_page=100",
        )
        self.assertEqual(
            fit_temporal_residual_regime_balance_utility_repair_model_run_artifacts_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/artifacts?per_page=100",
        )
        self.assertEqual(
            fit_temporal_residual_regime_balance_utility_repair_artifact_download_endpoint(456),
    ...
    def test_run_selection_forbids_second_manual_main_run(self) -> None:
        ...
        selected = select_fit_temporal_residual_regime_balance_utility_repair_manual_main_run(
            {"workflow_runs": [_run()]}
        )
    ...
    def test_run_classification_is_one_way(self) -> None:
        self.assertEqual(
            classify_fit_temporal_residual_regime_balance_utility_repair_model_run(None)[
                "run_state"
            ],
            "MISSING",
        )
    ...
    def test_missing_run_is_only_dispatchable_state(self) -> None:
        ...
        active = build_fit_temporal_residual_regime_balance_utility_repair_operator_report(
            checkout=_checkout(),
            run=_run(status="in_progress", conclusion=None),
        )
        self.assertEqual(
            active["stage"],
            "FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_RUN_IN_PROGRESS",
        )
        self.assertIsNone(
            dispatch_command_for_fit_temporal_residual_regime_balance_utility_repair_report(
                active
        ...
        terminal = build_fit_temporal_residual_regime_balance_utility_repair_operator_report(
            checkout=_checkout(),
    ...
    def test_gate_metadata_matches_dec258_source_shape(self) -> None:
        gate = {
            ...
            "dec253_merged_commit": "1" * 40,
            "dec254_merged_commit": "2" * 40,
            "dec255_merged_commit": "3" * 40,
            "dec256_merged_commit": "4" * 40,
            "dec257_merged_commit": "5" * 40,
            "dec256_workflow_blob_sha": "6" * 40,
        ...
        drifted = dict(gate)
        drifted.pop("dec257_merged_commit")
        with self.assertRaisesRegex(
            ValueError,
    ...
    def test_public_cli_binds_dec258_and_dec257(self) -> None:
        ...
        self.assertIn('OPERATOR_DECISION = "DEC-259"', text)
