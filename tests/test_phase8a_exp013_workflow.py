...
class Phase8AExp013WorkflowTests(unittest.TestCase):
    def test_workflow_is_manual_frozen_and_covers_exact_nine_cells(self) -> None:
        ...
        self.assertIn("scripts/phase8a_exp013.py stage-a-cell", text)
        self.assertIn("--code-commit \"${GITHUB_SHA}\"", text)
        self.assertIn("scripts/phase8a_exp013.py stage-a-authorize", text)
    ...
    def test_workflow_verifies_nonpromotion_and_frozen_stage_a_ranges(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn('"2015-01-01"', text)
        self.assertIn('"2021-01-01"', text)
