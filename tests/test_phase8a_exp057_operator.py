...
SCRIPT = ROOT / "scripts/phase8a_exp057_operator.py"
SHA = "a" * 40
...
def _run(
    *,
    ...
    return {
        "id": 123,
        "name": FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_WORKFLOW_NAME,
        "path": FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_WORKFLOW_PATH,
...
class Exp057ResidualLowerTailOperatorTests(unittest.TestCase):
    ...
    def test_checkout_requires_clean_exact_main(self) -> None:
        report = validate_fit_temporal_residual_lower_tail_utility_repair_operator_checkout(
            branch="main",
            head_sha=SHA,
            origin_main_sha=SHA,
            porcelain_status="",
    ...
    def test_exact_workflow_endpoints_and_dispatch(self) -> None:
        ...
        endpoint = fit_temporal_residual_lower_tail_utility_repair_model_runs_endpoint()
        self.assertIn(
        ...
        self.assertEqual(
            fit_temporal_residual_lower_tail_utility_repair_model_run_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123",
        )
        self.assertEqual(
            fit_temporal_residual_lower_tail_utility_repair_model_run_jobs_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/jobs?per_page=100",
        )
        self.assertEqual(
            fit_temporal_residual_lower_tail_utility_repair_model_run_artifacts_endpoint(123),
            "repos/Dtwosam/FMP/actions/runs/123/artifacts?per_page=100",
        )
        self.assertEqual(
            fit_temporal_residual_lower_tail_utility_repair_artifact_download_endpoint(456),
    ...
    def test_run_selection_forbids_second_manual_main_run(self) -> None:
        ...
        selected = select_fit_temporal_residual_lower_tail_utility_repair_manual_main_run(
            {"workflow_runs": [_run()]}
        )
    ...
    def test_run_classification_is_one_way(self) -> None:
        self.assertEqual(
            classify_fit_temporal_residual_lower_tail_utility_repair_model_run(None)[
                "run_state"
            ],
            "MISSING",
        )
    ...
    def test_missing_run_is_only_dispatchable_state(self) -> None:
        ...
        active = build_fit_temporal_residual_lower_tail_utility_repair_operator_report(
            checkout=_checkout(),
            run=_run(status="in_progress", conclusion=None),
        )
        self.assertEqual(
            active["stage"],
            "FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_RUN_IN_PROGRESS",
        )
        self.assertIsNone(
            dispatch_command_for_fit_temporal_residual_lower_tail_utility_repair_report(
                active
        ...
        terminal = build_fit_temporal_residual_lower_tail_utility_repair_operator_report(
            checkout=_checkout(),
    ...
    def test_public_cli_binds_dec225_and_dec224(self) -> None:
        ...
        self.assertIn('OPERATOR_DECISION = "DEC-226"', text)
        self.assertIn("DEC-225 source gate", text)
