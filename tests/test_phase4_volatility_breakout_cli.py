...
SCRIPT = Path("scripts/phase4_volatility_breakout.py")


class Phase4VolatilityBreakoutCliTests(unittest.TestCase):
    ...
    def test_final_split_is_rejected_before_data_access(self) -> None:
        completed = subprocess.run(
            [
                ...
                "--timeframe", "1h",
                "--split", "final",
                "--out", "/tmp/phase4-volatility-final-must-not-run",
    ...
    def test_script_delegates_to_frozen_runner_and_is_source_free(self) -> None:
        ...
        self.assertIn("run_volatility_breakout_grid", text)
