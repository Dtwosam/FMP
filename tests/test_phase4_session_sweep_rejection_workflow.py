...
WORKFLOW = Path(".github/workflows/phase4-session-sweep-rejection.yml")


class Phase4SessionSweepRejectionWorkflowTests(unittest.TestCase):
    def test_workflow_is_source_free_fixed_matrix_and_fail_closed(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        lowered = text.lower()
        self.assertIn("name: phase4-session-sweep-rejection", text)
        ...
        self.assertIn("python scripts/phase4_session_sweep_rejection.py", text)
        self.assertIn('--code-commit "${GITHUB_SHA}"', text)
        self.assertIn("fmp-phase4-session-sweep-rejection-grid-v1", text)
        self.assertIn("len(benchmark['configuration_rows']) == 9", text)
        self.assertIn("for buffer_pips in (0, 2, 5):", text)
        self.assertIn("assert len(digests) == 1", text)
    ...
    def test_push_paths_are_scoped_to_session_sweep_surface(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("paths:", text)
        expected_paths = (
            "src/fmp/research/session_sweep_rejection.py",
            "src/fmp/strategies/session_sweep_rejection.py",
            "scripts/phase4_session_sweep_rejection.py",
            ".github/workflows/phase4-session-sweep-rejection.yml",
        )
        ...
        path_lines = [
            ...
            if line.strip().startswith("- '") and (
                "src/fmp/" in line or "scripts/" in line or ".github/workflows/" in line
            )
        ]
        self.assertEqual(tuple(path_lines), expected_paths)
