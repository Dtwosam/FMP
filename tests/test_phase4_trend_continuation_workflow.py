...
WORKFLOW = Path(".github/workflows/phase4-trend-continuation.yml")


class Phase4TrendContinuationWorkflowTests(unittest.TestCase):
    def test_workflow_is_source_free_fixed_matrix_and_fail_closed(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        lowered = text.lower()
        self.assertIn("name: phase4-trend-continuation", text)
        ...
        self.assertIn("python scripts/phase4_trend_continuation.py", text)
        self.assertIn('--code-commit "${GITHUB_SHA}"', text)
        self.assertIn("fmp-phase4-trend-continuation-grid-v1", text)
    ...
    def test_push_paths_are_scoped_to_trend_continuation_surface(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("paths:", text)
        for path in (
            "src/fmp/research/trend_continuation.py",
            "src/fmp/strategies/trend_continuation.py",
            "scripts/phase4_trend_continuation.py",
            ".github/workflows/phase4-trend-continuation.yml",
        ):
