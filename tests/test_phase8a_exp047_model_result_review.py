...
def _run(conclusion: str = "success") -> dict[str, object]:
    return {
        ...
        "path": ".github/workflows/phase8a-exp047-density-model-training.yml",
...
def _cell_artifact_names() -> list[str]:
    return [
        f"exp047-density-model-cell-results-{symbol}-{timeframe}-{SHA}"
...
class Exp047ModelTerminalReviewTests(unittest.TestCase):
    def test_success_requires_complete_aggregate_and_stays_non_promotional(
        ...
        with patch(
            ...
            result = validate_density_model_terminal_review(
                run=_run(),
                jobs_payload=_jobs(),
        ...
        self.assertEqual(
            result["density_model_result_review_decision"],
            DENSITY_MODEL_RESULT_REVIEW_DECISION,
        )
        self.assertEqual(
            result["stage"],
    ...
    def test_unexpected_artifact_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            ValueError,
            "unexpected EXP-047 terminal review artifact",
        ):
            validate_density_model_terminal_review(
                run=_run("failure"),
                jobs_payload=_jobs(
