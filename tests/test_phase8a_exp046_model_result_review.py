...
def _run(conclusion: str = "success") -> dict[str, object]:
    return {
        ...
        "path": ".github/workflows/phase8a-exp046-stability-model-training.yml",
...
def _cell_artifact_names() -> list[str]:
    return [
        f"exp046-stability-model-cell-results-{symbol}-{timeframe}-{SHA}"
...
class Exp046ModelTerminalReviewTests(unittest.TestCase):
    def test_success_requires_complete_aggregate_and_stays_non_promotional(
        ...
        self.assertEqual(
            result["stability_model_result_review_decision"],
            STABILITY_MODEL_RESULT_REVIEW_DECISION,
        )
        self.assertEqual(
            result["stage"],
    ...
    def test_unexpected_artifact_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            ValueError,
            "unexpected EXP-046 terminal review artifact",
        ):
            validate_stability_model_terminal_review(
                run=_run("failure"),
                jobs_payload=_jobs(
