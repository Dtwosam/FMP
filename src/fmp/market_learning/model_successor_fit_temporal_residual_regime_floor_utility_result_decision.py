...
from .model_successor_fit_temporal_residual_regime_floor_utility_result_review import (
...
REVIEWED_MODEL_RUN_ID = 36207673978
REVIEWED_MODEL_HEAD_SHA = "2339762cda013322c8704cee12218ec4f4fb8c36"
REVIEWED_MODEL_RUN_ATTEMPT = 1
REVIEWED_MODEL_RUN_CONCLUSION = "success"

REVIEWED_AGGREGATE_ARTIFACT_ID = 10895236824
...
REVIEWED_AGGREGATE_ARTIFACT_DIGEST = (
    "sha256:"
    "0e4efa5602410a58fa754dfcd44745d69f4e2905d6ce0dd2062030e791808c3a"
)
...
REVIEWED_AGGREGATE_SELECTION_PASS_VARIANT_COUNT = 3
...
REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS = {
    ...
    ("USDJPY", "5m", 60, 500): (0, 0, 7, 493),
    ("USDJPY", "5m", 60, 1000): (0, 3, 72, 925),
}
...
def _validate_aggregate_artifact_identity(
    ...
    if not isinstance(artifacts, list):
        raise ValueError("DEC-240 artifact listing is malformed")

    matches: list[Mapping[str, object]] = []
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError("DEC-240 artifact row is malformed")
        if raw.get("name") == REVIEWED_AGGREGATE_ARTIFACT_NAME:
            matches.append(raw)

    if len(matches) != 1:
        raise ValueError(
            "DEC-240 requires exactly one reviewed aggregate artifact"
        )

    artifact = matches[0]
    if artifact.get("id") != REVIEWED_AGGREGATE_ARTIFACT_ID:
        raise ValueError("DEC-240 aggregate artifact id mismatch")
    if artifact.get("expired") is not False:
        raise ValueError(
            "DEC-240 aggregate artifact must remain non-expired"
        )
    if artifact.get("digest") != REVIEWED_AGGREGATE_ARTIFACT_DIGEST:
        raise ValueError("DEC-240 aggregate artifact digest mismatch")
...
def _aggregate_pass_variants(
    ...
    if not isinstance(cells, list):
        raise ValueError("DEC-240 aggregate evidence cells malformed")

    passed: list[tuple[str, str, int, int]] = []
    for raw_cell in cells:
        if not isinstance(raw_cell, Mapping):
            raise ValueError("DEC-240 aggregate cell malformed")
        ...
        if not isinstance(identity, Mapping) or not isinstance(selection, Mapping):
            raise ValueError("DEC-240 aggregate cell identity malformed")
        variants = selection.get("variants")
        if not isinstance(variants, list):
            raise ValueError("DEC-240 aggregate variants malformed")
        for variant in variants:
            if not isinstance(variant, Mapping):
                raise ValueError("DEC-240 aggregate variant malformed")
    ...
    for raw_cell in cells:
        ...
        for variant in variants:
            ...
            if not isinstance(stability, Mapping):
                raise ValueError("DEC-240 stability block malformed")
            windows = stability.get("windows")
            if not isinstance(windows, list) or len(windows) != 4:
                raise ValueError("DEC-240 stability window inventory mismatch")
            counts: list[int] = []
            for window in windows:
                if not isinstance(window, Mapping):
                    raise ValueError("DEC-240 stability window malformed")
                metrics = window.get("metrics")
                if not isinstance(metrics, Mapping):
                    raise ValueError("DEC-240 stability metrics malformed")
                count = metrics.get("directional_candidate_count")
                if not isinstance(count, int) or isinstance(count, bool) or count < 0:
                    raise ValueError(
                        "DEC-240 stability candidate count malformed"
                    )
                counts.append(count)
            key = (
                str(identity.get("symbol")),
...
def _summary(
    ...
    if aggregate_evidence.get("code_commit") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-240 aggregate evidence code commit mismatch")
    if aggregate_evidence.get("evidence_fingerprint") != REVIEWED_EVIDENCE_FINGERPRINT:
        raise ValueError("DEC-240 aggregate evidence fingerprint mismatch")
    ...
    for raw_cell in cells:
        ...
        if not isinstance(fingerprint, str) or _canonical_hash(supplied) != fingerprint:
            raise ValueError("DEC-240 cell result fingerprint mismatch")
        ...
        if (
            ...
            raise ValueError("DEC-240 aggregate cell blocks malformed")
        ...
        for value, label, target in (
            ...
            if not isinstance(value, int) or isinstance(value, bool):
                raise ValueError(f"DEC-240 {label} malformed")
            target.add(value)
        ...
        for variant in variants:
            ...
            if (
                ...
                raise ValueError(
                    "DEC-240 eligible selection row count malformed"
                )
            ...
            if status == "AVAILABLE":
                ...
                raise ValueError("DEC-240 unexpected variant status")
        ...
        if len(cell_eligible_counts) != 1:
            raise ValueError(
                "DEC-240 eligible selection row count drift within cell"
            )
    ...
    return {
        ...
        "no_fit_temporal_residual_lower_tail_utility_stable_model_challenger_count": (
            no_stable_cells
        ),
...
def validate_reviewed_fit_temporal_residual_regime_floor_utility_model_result(
    *,
    ...
    if run.get("id") != REVIEWED_MODEL_RUN_ID:
        raise ValueError("DEC-240 reviewed model run id mismatch")
    if run.get("head_sha") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-240 reviewed model head SHA mismatch")
    if run.get("run_attempt") != REVIEWED_MODEL_RUN_ATTEMPT:
        raise ValueError("DEC-240 reviewed model run attempt mismatch")
    if run.get("conclusion") != REVIEWED_MODEL_RUN_CONCLUSION:
        raise ValueError("DEC-240 reviewed model run conclusion mismatch")
    ...
    exact_review = {
        ...
        "fit_temporal_residual_regime_floor_utility_model_result_evidence_verified": (
            True
        ),
    ...
    for field, expected in exact_review.items():
        if review.get(field) != expected:
            raise ValueError(
                f"DEC-240 terminal review {field} mismatch"
            )
    ...
    for field, expected in exact_summary.items():
        if summary.get(field) != expected:
            raise ValueError(f"DEC-240 reviewed {field} mismatch")

    passes = _aggregate_pass_variants(aggregate_evidence)
    if passes != tuple(sorted(REVIEWED_AGGREGATE_PASS_VARIANTS)):
        raise ValueError("DEC-240 aggregate pass inventory mismatch")
    ...
    if window_counts != REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS:
        raise ValueError(
            "DEC-240 aggregate-pass stability-window inventory mismatch"
        )
    ...
    return {
        ...
        **summary,
        ...
        "stage": (
            "FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_UTILITY_MODEL_RESULT_"
...
__all__ = [
    ...
    "SHADOW_AUTHORIZED",
    "TRADING_AUTHORIZED",
    "validate_reviewed_fit_temporal_residual_regime_floor_utility_model_result",
]
