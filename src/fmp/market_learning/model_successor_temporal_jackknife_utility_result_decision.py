...
from .model_successor_temporal_jackknife_utility_result_review import (
    validate_temporal_jackknife_utility_model_terminal_review,
)


TEMPORAL_JACKKNIFE_UTILITY_MODEL_RESULT_DECISION = "DEC-148"

REVIEWED_MODEL_RUN_ID = 36049824739
...
REVIEWED_AGGREGATE_ARTIFACT_ID = 10829959147
...
REVIEWED_AGGREGATE_ARTIFACT_DIGEST = (
    "sha256:"
    "4a2b223425c2e8df60e13ec0ad22c46da618957999a7a7d23908ff9fd3b20275"
)
...
def _validate_aggregate_artifact_identity(
    ...
    if not isinstance(artifacts, list):
        raise ValueError(
            "DEC-148 artifact listing is malformed"
        )
    ...
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "DEC-148 artifact row is malformed"
            )
    ...
    if len(matches) != 1:
        raise ValueError(
            "DEC-148 requires exactly one reviewed aggregate artifact"
        )

    artifact = matches[0]
    if artifact.get("id") != REVIEWED_AGGREGATE_ARTIFACT_ID:
        raise ValueError(
            "DEC-148 aggregate artifact id mismatch"
        )
    if artifact.get("expired") is not False:
        raise ValueError(
            "DEC-148 aggregate artifact must remain non-expired"
        )
    ...
    if (
        ...
        raise ValueError(
            "DEC-148 aggregate artifact digest mismatch"
        )


def validate_reviewed_temporal_jackknife_utility_model_result(
    *,
    ...
    if run.get("id") != REVIEWED_MODEL_RUN_ID:
        raise ValueError(
            "DEC-148 reviewed model run id mismatch"
        )
    if run.get("head_sha") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError(
            "DEC-148 reviewed model head SHA mismatch"
        )
    if run.get("run_attempt") != REVIEWED_MODEL_RUN_ATTEMPT:
        raise ValueError(
            "DEC-148 reviewed model run attempt mismatch"
        )
    if run.get("conclusion") != REVIEWED_MODEL_RUN_CONCLUSION:
        raise ValueError(
            "DEC-148 reviewed model run conclusion mismatch"
        )
    ...
    exact = {
        ...
        "no_temporal_jackknife_utility_stable_model_challenger_count": (
    ...
    for field, expected in exact.items():
        if review.get(field) != expected:
            raise ValueError(
                f"DEC-148 terminal review {field} mismatch"
            )

    return {
        **review,
        "temporal_jackknife_utility_model_result_decision": (
            TEMPORAL_JACKKNIFE_UTILITY_MODEL_RESULT_DECISION
        ),
        "stage": (
            "TEMPORAL_JACKKNIFE_UTILITY_MODEL_RESULT_"
...
__all__ = [
    ...
    "TEMPORAL_JACKKNIFE_UTILITY_MODEL_RESULT_DECISION",
    "TRADING_AUTHORIZED",
    "validate_reviewed_temporal_jackknife_utility_model_result",
]
