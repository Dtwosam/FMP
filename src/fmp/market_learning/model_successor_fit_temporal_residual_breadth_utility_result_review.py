...
from .model_successor_fit_temporal_residual_breadth_utility_artifacts import (
    compile_fit_temporal_residual_breadth_utility_model_result_evidence,
)


FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_RESULT_REVIEW_DECISION = "DEC-202"
...
DEC201_EXECUTION_GATE_BLOB_SHA = (
...
def _validate_head_sha(value: object) -> str:
    if not isinstance(value, str) or len(value) != 40:
        raise ValueError(
            "EXP-055 terminal review head SHA must be a 40-character commit"
        )
    try:
        int(value, 16)
    except ValueError as exc:
        raise ValueError(
            "EXP-055 terminal review head SHA must be hexadecimal"
        ) from exc
    return value.lower()
...
def _index_jobs(
    ...
    if not isinstance(jobs, list):
        raise ValueError("EXP-055 terminal review jobs payload is malformed")
    if len(jobs) != 11:
        raise ValueError(
            "EXP-055 terminal review requires exactly 11 workflow jobs"
        )
    ...
    for raw in jobs:
        if not isinstance(raw, Mapping):
            raise ValueError("EXP-055 terminal review job row is malformed")
        job_id = raw.get("id")
        if (
            ...
            raise ValueError(
                "EXP-055 terminal review job id is malformed or duplicated"
            )
        seen_ids.add(job_id)
        if raw.get("status") != "completed":
            raise ValueError("EXP-055 terminal review requires completed jobs")
        name = raw.get("name")
        if not isinstance(name, str):
            raise ValueError("EXP-055 terminal review job name is malformed")
        if name == "authorization-preflight":
            ...
            raise ValueError(
                f"unexpected EXP-055 terminal review job name: {name!r}"
            )

    if len(preflight) != 1:
        raise ValueError(
            "EXP-055 terminal review requires one authorization-preflight job"
        )
    if len(matrix) != 9:
        raise ValueError(
            "EXP-055 terminal review requires nine matrix jobs"
        )
    if len(aggregate) != 1:
        raise ValueError(
            "EXP-055 terminal review requires one aggregate job"
        )
...
def _validate_artifacts(
    ...
    *,
    ...
    if not isinstance(artifacts, list):
        raise ValueError(
            "EXP-055 terminal review artifact listing is malformed"
        )
    expected_cells, expected_aggregate = _expected_artifact_names(head_sha)
    seen: set[str] = set()
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError("EXP-055 terminal review artifact row is malformed")
        name = raw.get("name")
        if not isinstance(name, str) or name in seen:
            raise ValueError(
                "EXP-055 terminal review artifact name is malformed or duplicated"
            )
        if raw.get("expired") is not False:
            raise ValueError(
                "EXP-055 terminal review requires non-expired artifacts"
            )
        if name not in expected_cells and name != expected_aggregate:
            raise ValueError(
                f"unexpected EXP-055 terminal review artifact: {name!r}"
            )
        seen.add(name)
    total_count = artifacts_payload.get("total_count")
    if total_count is not None and total_count != len(artifacts):
        raise ValueError("EXP-055 terminal review artifact count mismatch")
...
def _revalidate_aggregate_evidence(
    ...
    *,
    ...
    if not isinstance(cells, list):
        raise ValueError(
            "successful EXP-055 run requires aggregate evidence cells"
        )
    recompiled = (
        compile_fit_temporal_residual_breadth_utility_model_result_evidence(
            cells,
    ...
    if dict(evidence) != recompiled:
        raise ValueError(
            "EXP-055 aggregate evidence does not match deterministic recompilation"
        )
    return {
        "fit_temporal_residual_breadth_utility_model_result_evidence_verified": True,
...
def validate_fit_temporal_residual_breadth_utility_model_terminal_review(
    *,
    ...
    if not isinstance(run_id, int) or isinstance(run_id, bool):
        raise ValueError("EXP-055 terminal review run id is malformed")
    if run.get("name") != FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_WORKFLOW_NAME:
        raise ValueError("EXP-055 terminal review workflow name mismatch")
    if run.get("path") != FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_WORKFLOW_PATH:
        raise ValueError("EXP-055 terminal review workflow path mismatch")
    if run.get("event") != "workflow_dispatch":
        raise ValueError("EXP-055 terminal review event mismatch")
    if run.get("head_branch") != "main":
        raise ValueError("EXP-055 terminal review branch mismatch")
    if run.get("run_attempt") != 1:
        raise ValueError("EXP-055 terminal review forbids rerun attempts")
    if run.get("status") != "completed":
        raise ValueError("EXP-055 terminal review run is not completed")

    head_sha = _validate_head_sha(run.get("head_sha"))
    conclusion = run.get("conclusion")
    if conclusion not in {"success", *TERMINAL_NON_SUCCESS_CONCLUSIONS}:
        raise ValueError(
            f"unexpected EXP-055 terminal run conclusion: {conclusion!r}"
        )
    ...
    review: dict[str, object] = {
        ...
        "dec189_merged_commit": DEC201_MERGED_COMMIT,
        "dec189_workflow_blob_sha": DEC201_WORKFLOW_BLOB_SHA,
        "dec189_cli_blob_sha": DEC201_CLI_BLOB_SHA,
        "dec189_execution_gate_blob_sha": DEC201_EXECUTION_GATE_BLOB_SHA,
    ...
    if conclusion == "success":
        if preflight_conclusion != "success":
            raise ValueError(
                "successful EXP-055 run requires successful preflight"
            )
        if any(value != "success" for value in matrix_conclusions):
            raise ValueError(
                "successful EXP-055 run requires all matrix jobs to succeed"
            )
        if aggregate_conclusion != "success":
            raise ValueError(
                "successful EXP-055 run requires aggregate job success"
            )
        if len(persisted_cells) != 9 or not aggregate_present:
            raise ValueError(
                "successful EXP-055 run requires all ten expected artifacts"
            )
        if aggregate_evidence is None:
            raise ValueError(
                "successful EXP-055 run requires aggregate evidence"
            )
    ...
    if aggregate_evidence is not None:
        raise ValueError(
            "non-success EXP-055 run cannot claim aggregate result evidence"
        )
    if aggregate_present:
        raise ValueError(
            "non-success EXP-055 run cannot claim aggregate artifact"
        )
    if aggregate_conclusion not in {"skipped", "failure", "cancelled"}:
        raise ValueError(
            "non-success EXP-055 aggregate job conclusion mismatch"
        )
    return {
        **review,
        "stage": (
