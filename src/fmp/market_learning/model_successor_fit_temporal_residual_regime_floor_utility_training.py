...
from . import model_successor_fit_temporal_residual_lower_tail_utility_repair_training as _predecessor
...
PREDECESSOR_TRAINING_CORE_BLOB_SHA = "ef0ffc46b130d5cfe5b1a19f86bea6a2d41d0cbd"
...
def validate_fit_temporal_residual_regime_floor_utility_training_sources(
    *,
    ...
    ] != _predecessor.FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_TRAINING_CORE_DECISION:
    ...
    if FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_AUTHORIZED is not True:
        raise ValueError("EXP-058 residual regime-floor change is not authorized")
    ...
    return {
        ...
        "residual_regime_floor_authorized": True,
...
def _fit_temporal_residual_regime_floor_utility(
    *,
    ...
    if len(view_predictions) != FIT_TEMPORAL_RESIDUAL_REGIME_COUNT:
        raise ValueError("EXP-058 residual regime view count drift")
    ...
    for index in np.flatnonzero(eligible):
        ...
        for view_name in sorted(view_predictions):
            ...
            if len(residuals) != FIT_TEMPORAL_RESIDUAL_WINDOWS_PER_REGIME:
                raise ValueError(
                    "EXP-058 residual windows-per-regime count drift"
                )
            bounds = np.asarray(
                [
        ...
        if (
            len(values) != FIT_TEMPORAL_RESIDUAL_REGIME_COUNT
            or bound_count
            != FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_BOUND_COUNT_PER_ROW
        ...
        result[index] = float(np.min(values))
    return result
...
def _score_fit_temporal_residual_regime_floor_consensus(
    *,
    ...
    residual_regime_floor = _fit_temporal_residual_regime_floor_utility(
        directions=directions,
