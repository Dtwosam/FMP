...
from . import model_successor_fit_temporal_residual_lower_tail_utility_repair_training as _lower_tail_predecessor
...
FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_UTILITY_TRAINING_CORE_DECISION = "DEC-232"
...
FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_UTILITY_RESULT_EXECUTION_AUTHORIZED = False
...
def validate_fit_temporal_residual_regime_floor_utility_training_sources(
    *,
    ...
    expected = {
        ...
        "exp057_predecessor_training_core": (
            root
            / "src/fmp/market_learning/"
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(f"missing EXP-058 training dependency: {path}")
    ...
    ] != _lower_tail_predecessor.FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_TRAINING_CORE_DECISION:
        raise ValueError("EXP-058 predecessor training decision drift")
    if _lower_tail_predecessor.MODEL_FIT_AUTHORIZED is not False:
        raise ValueError("EXP-058 predecessor model fit must remain closed")
    if (
        _lower_tail_predecessor
        ...
        raise ValueError(
            "EXP-058 predecessor result execution must remain closed"
        )
    ...
    if len(fingerprint) != 64:
        raise ValueError("EXP-058 protocol fingerprint is invalid")
    if FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_AUTHORIZED is not True:
        raise ValueError("EXP-058 fit-regime-floor score is not authorized")
    ...
    return {
        ...
        "dec231_protocol_blob_sha": actual["dec231_protocol"],
        "exp057_predecessor_training_core_blob_sha": actual[
            "exp057_predecessor_training_core"
        ],
    ...
    *,
    ...
    for index in np.flatnonzero(eligible):
        ...
        if (
            ...
            raise ValueError(
                "EXP-058 residual lower-tail bound inventory invalid"
            )
...
def _fit_temporal_residual_regime_floor_utility(
    *,
    directions: np.ndarray,
    view_predictions: Mapping[str, Mapping[str, np.ndarray]],
    residual_references: Mapping[str, Mapping[str, Mapping[str, float]]],
    ...
    directions = np.asarray(directions, dtype=object)
    result = np.full(len(directions), -np.inf, dtype=np.float64)
    if set(view_predictions) != set(residual_references):
        raise ValueError("EXP-058 regime-floor view inventory mismatch")
    if len(view_predictions) != FIT_TEMPORAL_RESIDUAL_REGIME_COUNT:
        raise ValueError("EXP-058 regime-floor view count drift")
    ...
    for index in np.flatnonzero(eligible):
        ...
        for view_name in sorted(view_predictions):
            prediction = float(view_predictions[view_name][target][index])
            references = residual_references[view_name][target]
            if len(references) != FIT_TEMPORAL_RESIDUAL_WINDOWS_PER_REGIME:
                raise ValueError(
                    "EXP-058 residual windows per regime drift"
                )
            bounds = np.asarray(
                [
                    prediction + float(residual)
                    for residual in references.values()
                ],
                dtype=np.float64,
            )
            if not np.isfinite(bounds).all():
                raise ValueError(
                    "EXP-058 residual regime bounds must be finite"
                )
        ...
        if (
            total_bounds
            != FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_BOUND_COUNT_PER_ROW
            or len(regime_means) != FIT_TEMPORAL_RESIDUAL_REGIME_COUNT
            or not np.isfinite(np.asarray(regime_means)).all()
        ):
            raise ValueError(
                "EXP-058 residual regime-floor inventory invalid"
            )
        result[index] = min(regime_means)
    return result

def _candidate_directions_residual_regime_floor(
    *,
    directions: np.ndarray,
    ...
    if any(len(value) != len(directions) for value in values):
        raise ValueError("EXP-058 cutoff input length mismatch")
...
def _derive_residual_regime_floor_cutoff(
    *,
    directions: np.ndarray,
    residual_regime_floor: np.ndarray,
    residual_lower_tail: np.ndarray,
    ...
    if budget not in _base.CANDIDATE_BUDGET_ANCHORS:
        raise ValueError("unsupported EXP-058 candidate budget")
    directions = np.asarray(directions, dtype=object)
    arrays = [
        np.asarray(residual_regime_floor, dtype=np.float64),
        np.asarray(residual_lower_tail, dtype=np.float64),
    ...
    if (
        ...
        raise ValueError("EXP-058 cutoff row mismatch")
    ...
    for index in indices.tolist():
        if not np.isfinite(arrays[0][index]):
            raise ValueError(
                "EXP-058 residual regime-floor utility must be finite"
            )
        if not np.isfinite(arrays[1][index]):
            raise ValueError(
                "EXP-058 residual lower-tail mean must be finite"
            )
        breadth = arrays[2][index]
        if not np.isfinite(breadth) or breadth < 0.0 or breadth > 1.0:
            raise ValueError(
                "EXP-058 residual breadth must be within [0, 1]"
            )
        if not np.isfinite(arrays[3][index]):
            raise ValueError(
                "EXP-058 residual-bound utility must be finite"
            )
        for label, value in (
            ("feature support", arrays[4][index]),
            ("fit-temporal utility support", arrays[5][index]),
            ("pooled calibrated utility", arrays[6][index]),
        ):
            if not np.isfinite(value) or value < 0.0 or value > 1.0:
                raise ValueError(
                    f"EXP-058 {label} must be within [0, 1]"
                )
        if not np.isfinite(arrays[7][index]) or arrays[7][index] <= 0.0:
            raise ValueError(
                "EXP-058 directional raw utility must be positive"
            )
    ...
    passing = _candidate_directions_residual_regime_floor(
        directions=directions,
        residual_regime_floor=arrays[0],
        residual_lower_tail=arrays[1],
        residual_breadth=arrays[2],
        residual_bound_utility=arrays[3],
        feature_support=arrays[4],
        fit_temporal_support=arrays[5],
        pooled_calibrated_utility=arrays[6],
        raw_utility=arrays[7],
        residual_regime_floor_cutoff=cutoff[0],
        residual_lower_tail_cutoff=cutoff[1],
        residual_breadth_cutoff=cutoff[2],
        residual_bound_cutoff=cutoff[3],
        feature_cutoff=cutoff[4],
        utility_cutoff=cutoff[5],
        pooled_cutoff=cutoff[6],
        raw_cutoff=cutoff[7],
    )
    return {
        "status": "AVAILABLE",
        "candidate_budget_anchor": budget,
        "eligible_selection_row_count": int(len(indices)),
        "selection_derived_residual_regime_floor_cutoff": cutoff[0],
        "selection_derived_residual_lower_tail_cutoff": cutoff[1],
        "selection_derived_residual_breadth_cutoff": cutoff[2],
        "selection_derived_residual_bound_cutoff": cutoff[3],
        "selection_derived_feature_support_cutoff": cutoff[4],
        "selection_derived_support_cutoff": cutoff[5],
        "selection_derived_pooled_calibrated_cutoff": cutoff[6],
        "selection_derived_raw_cutoff": cutoff[7],
...
def _residual_regime_floor_consensus_digest(
    *,
    ...
    if not (
        len(row_ids)
        == len(directions)
        == len(residual_regime_floor)
    ):
        raise ValueError("EXP-058 consensus digest row mismatch")
    digest = hashlib.sha256()
    digest.update(str(predecessor_digest).encode("utf-8"))
    for row_id, direction, value in zip(
        row_ids,
        directions.tolist(),
        residual_regime_floor.tolist(),
        strict=True,
    ):
...
def _score_fit_temporal_residual_regime_floor_consensus(
    *,
    ...
    (
        ...
        row_ids,
        predecessor_digest,
    ) = _lower_tail_predecessor._score_fit_temporal_residual_lower_tail_consensus(
    ...
    if tuple(row_ids) != tuple(scored_row_ids):
        raise ValueError(
            "EXP-058 predecessor/scored row identity drift"
        )
    ...
    values = residual_regime_floor[eligible]
    if bool(eligible.any()) and not np.isfinite(values).all():
        raise ValueError(
            "EXP-058 eligible residual regime-floor utility is invalid"
        )
    diagnostics = {
        **dict(predecessor_diagnostics),
        ...
        "residual_regime_floor_view_prediction_digests": (
            view_prediction_digests
        ),
    }
    digest = _residual_regime_floor_consensus_digest(
...
def _evaluate_residual_regime_floor_cutoff(
    frame: pl.DataFrame,
    *,
    directions: np.ndarray,
    residual_regime_floor: np.ndarray,
...
def _evaluate_temporal_stability_residual_regime_floor(
    *,
    ...
    if any(len(value) != selection_frame.height for value in arrays):
        raise ValueError("EXP-058 stability row mismatch")
    ...
    for raw_window in _base.TEMPORAL_STABILITY_WINDOWS:
        ...
        evaluated = _evaluate_residual_regime_floor_cutoff(
            frame,
...
def _evaluate_forward_split_residual_regime_floor(
    *,
    ...
    (
        ...
        row_ids,
        digest,
    ) = _score_fit_temporal_residual_regime_floor_consensus(
        fitted_models=fitted_models,
        pooled_calibration_references=pooled_calibration_references,
        support_references=support_references,
        feature_references=feature_references,
    ...
    return {
        ...
        "row_count": frame.height,
        "fit_temporal_residual_regime_floor_utility_consensus": diagnostics,
        "fit_temporal_residual_regime_floor_utility_consensus_digest": digest,
        **evaluated,
    }


def run_fit_temporal_residual_regime_floor_utility_model_cell_core(
    *,
    ...
    if any(frame.is_empty() for frame in split_frames.values()):
        ...
        raise ValueError(
            f"EXP-058 model cell has empty required split(s): {empty}"
        )
    ...
    if set(regime_frames) != set(_base.PREDECESSOR_REGIME_NAMES):
        raise ValueError(
            "EXP-058 predecessor fit-regime identity drift"
        )
    if any(frame.is_empty() for frame in regime_frames.values()):
        ...
        raise ValueError(
            f"EXP-058 model cell has empty fit regime(s): {empty}"
        )
    ...
    for view_name, fit_frame in view_frames.items():
        ...
        if len(excluded) != 1:
            raise ValueError(
                "EXP-058 jackknife excluded-regime count drift"
            )
    ...
    for budget in _base.CANDIDATE_BUDGET_ANCHORS:
        cutoff_record = _derive_residual_regime_floor_cutoff(
        ...
        if aggregate_passed:
            stability = (
                _evaluate_temporal_stability_residual_regime_floor(
    ...
    result: dict[str, object] = {
        ...
        "selection": {
            ...
            "fit_temporal_residual_regime_floor_utility_consensus_digest": (
                selection_consensus_digest
            ),
    ...
    validation = _evaluate_forward_split_residual_regime_floor(
        fitted_models=fitted_models,
        pooled_calibration_references=pooled_calibration_references,
        support_references=support_references,
        feature_references=feature_references,
    ...
    result["retrospective_holdout"] = (
        _evaluate_forward_split_residual_regime_floor(
            fitted_models=fitted_models,
            pooled_calibration_references=(
                pooled_calibration_references
            ),
...
def build_fit_temporal_residual_regime_floor_training_core_gate(
) -> dict[str, object]:
    return {
        "experiment_id": (
            FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_UTILITY_EXPERIMENT_ID
        ...
        "residual_regime_floor_added": True,
        "residual_regime_floor_authorized": (
            FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_AUTHORIZED
        ),
