...
def validate_fit_temporal_residual_bound_utility_training_sources(
    *,
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(
                f"missing EXP-054 training dependency: {path}"
            )
    ...
    if predecessor[
        ...
        raise ValueError(
            "EXP-054 predecessor result execution must remain closed"
        )
    if PREDECESSOR_MODEL_FIT_AUTHORIZED is not False:
        raise ValueError(
            "EXP-054 predecessor source fit authorization drift"
        )
    if (
        FIT_TEMPORAL_FEATURE_SUPPORT_UTILITY_RESULT_EXECUTION_AUTHORIZED
        is not False
        ...
        raise ValueError(
            "EXP-054 predecessor source execution authorization drift"
        )
...
def _residual_windows_by_parent(
    ...
    if set(result) != set(PREDECESSOR_REGIME_NAMES):
        raise ValueError(
            "EXP-054 residual parent-regime inventory mismatch"
        )
    if any(len(windows) != 4 for windows in result.values()):
        raise ValueError(
            "EXP-054 residual window count mismatch"
        )
    return result


def _downside_residual(sorted_residuals: np.ndarray) -> tuple[int, float]:
    ...
    if (
        ...
        raise ValueError("EXP-054 residual reference is invalid")
    if bool((residuals[1:] < residuals[:-1]).any()):
        raise ValueError("EXP-054 residual reference must be sorted")
    index = int(
        np.floor(
            RESIDUAL_DOWNSIDE_QUANTILE
            * float(len(residuals) - 1)
        )
    )
    if index < 0 or index >= len(residuals):
        raise ValueError("EXP-054 residual quantile index is invalid")
...
def _build_fit_temporal_residual_references(
    *,
    ...
    if set(fitted_models) != set(view_regimes):
        raise ValueError(
            "EXP-054 fitted view inventory mismatch"
        )
    if set(regime_frames) != set(PREDECESSOR_REGIME_NAMES):
        raise ValueError(
            "EXP-054 fit-regime inventory mismatch"
        )
    ...
    for view_name in sorted(fitted_models):
        ...
        if len(excluded) != 1:
            raise ValueError(
                "EXP-054 excluded fit-regime count drift"
            )
        ...
        for target in FINANCIAL_TARGET_COLUMNS:
            fitted = fitted_models[view_name][target]
            ...
            for raw_window in windows_by_parent[excluded_regime]:
                ...
                if window_frame.is_empty():
                    raise ValueError(
                        "EXP-054 residual reference window is empty"
                    )
                predictions, prediction_digest, _ = _score_regressor(
                    fitted,
                    window_frame,
                    cell=cell,
                )
                realized = np.asarray(
                    window_frame[target].to_list(),
                    dtype=np.float64,
                )
                if (
                    realized.shape != predictions.shape
                    or not np.isfinite(realized).all()
                ):
                    raise ValueError(
                        "EXP-054 realized residual targets are invalid"
                    )
                residuals = realized - predictions
                if not np.isfinite(residuals).all():
                    raise ValueError(
                        "EXP-054 residual values must be finite"
                    )
                sorted_residuals = np.sort(
                    residuals.astype(np.float64, copy=True)
                )
                quantile_index, downside = _downside_residual(
                    sorted_residuals
                )
                window_name = str(window["name"])
                window_refs[window_name] = downside
                window_evidence[window_name] = {
                    ...
                    "mean_residual": float(sorted_residuals.mean()),
                    "sorted_residual_digest": _float_array_digest(
                        sorted_residuals
                    ),
                    "downside_quantile": RESIDUAL_DOWNSIDE_QUANTILE,
                    "downside_quantile_index": quantile_index,
                    "downside_residual": downside,
                }
            if len(window_refs) != 4:
                raise ValueError(
                    "EXP-054 target residual reference count drift"
                )
            target_refs[target] = window_refs
        ...
        evidence[view_name] = {
            ...
            "residual_reference_count": sum(
                len(value) for value in target_refs.values()
            ),
    ...
    if count != FIT_TEMPORAL_RESIDUAL_REFERENCE_COUNT_PER_CELL:
        raise ValueError(
            "EXP-054 total residual reference count drift"
        )
    return references, evidence


def _score_residual_bound(
    *,
    fitted_models: Mapping[
        str,
        Mapping[str, _base.FittedMarketModel],
    ...
    if frame.is_empty():
        raise ValueError(
            "EXP-054 residual-bound scored frame must be non-empty"
        )
    if len(directions) != frame.height:
        raise ValueError(
            "EXP-054 residual-bound direction row mismatch"
        )
    if set(residual_references) != set(fitted_models):
        raise ValueError(
            "EXP-054 residual reference view inventory mismatch"
        )
    ...
    for view_name in sorted(fitted_models):
        ...
        for target in FINANCIAL_TARGET_COLUMNS:
            values, _, row_ids = _score_regressor(
            ...
            if tuple(row_ids) != tuple(expected_row_ids):
                raise ValueError(
                    "EXP-054 residual-bound row identity mismatch"
                )
            target_predictions[target] = values
    ...
    for index, direction in enumerate(directions.tolist()):
        ...
        target = (
            "long_net_pips_0p5"
            if direction == "LONG"
            else "short_net_pips_0p5"
        )
        bounds: list[float] = []
        for view_name in sorted(fitted_models):
            predicted = float(predictions[view_name][target][index])
            refs = residual_references[view_name][target]
            if len(refs) != 4:
                raise ValueError(
                    "EXP-054 residual-bound reference count drift"
                )
            for window_name in sorted(refs):
                downside = float(refs[window_name])
                bound = predicted + downside
                if not np.isfinite(bound):
                    raise ValueError(
                        "EXP-054 residual-bound utility must be finite"
                    )
                bounds.append(bound)
        if len(bounds) != 12:
            raise ValueError(
                "EXP-054 residual-bound comparison count drift"
            )
        out[index] = min(bounds)

    eligible = np.isin(
        directions,
        np.asarray(["LONG", "SHORT"], dtype=object),
    )
    if bool(eligible.any()) and not np.isfinite(out[eligible]).all():
        raise ValueError(
            "EXP-054 eligible residual-bound utility is invalid"
        )
    return out
...
def _residual_bound_consensus_digest(
    *,
    ...
    if not (
        ...
        raise ValueError(
            "EXP-054 consensus digest row mismatch"
        )
...
def _score_fit_temporal_residual_bound_consensus(
    *,
    ...
    residual_bound = _score_residual_bound(
        fitted_models=fitted_models,
...
def _passes_residual_bound_cutoff(
    *,
    ...
    return bool(
        residual_bound > residual_cutoff
        or (
            residual_bound == residual_cutoff
            and (
...
def _derive_residual_bound_cutoff(
    *,
    ...
    if budget not in CANDIDATE_BUDGET_ANCHORS:
        raise ValueError(
            "unsupported EXP-054 candidate budget"
        )
    if not (
        len(row_ids)
        == len(directions)
    ...
    for index in eligible:
        if not np.isfinite(float(residual_bound[index])):
            raise ValueError(
                "EXP-054 residual-bound utility must be finite"
            )
        ...
        if not np.isfinite(raw_value) or raw_value <= 0.0:
            raise ValueError(
                "EXP-054 directional raw utility must be positive"
            )
        for label, value in (
            ("feature support", float(feature_support[index])),
            (
            ...
            if (
                ...
                raise ValueError(
                    f"EXP-054 {label} must be within [0, 1]"
                )
    ...
    residual_cutoff = float(residual_bound[cutoff_index])
    ...
    selected_count = sum(
        1
        for index in eligible
        if _passes_residual_bound_cutoff(
            residual_bound=float(residual_bound[index]),
    ...
    return {
        ...
        "candidate_budget_anchor": budget,
        "eligible_utility_row_count": len(eligible),
        "selection_derived_residual_bound_cutoff": residual_cutoff,
...
def _candidate_directions_residual_bound(
    *,
    ...
    if not np.isfinite(residual_cutoff):
        raise ValueError("EXP-054 residual cutoff is invalid")
    for label, value in (
        ...
        if (
            ...
            raise ValueError(f"EXP-054 {label} is invalid")
    if not np.isfinite(raw_cutoff) or raw_cutoff <= 0.0:
        raise ValueError("EXP-054 raw cutoff is invalid")
    if not (
        len(directions)
        == len(raw_utility)
        == len(pooled_calibrated_utility)
        ...
        raise ValueError("EXP-054 candidate row mismatch")

    out = np.full(len(directions), "NO_TRADE", dtype=object)
    for index, direction in enumerate(directions.tolist()):
        if direction not in {"LONG", "SHORT"}:
            continue
        residual_value = float(residual_bound[index])
        raw_value = float(raw_utility[index])
        feature_value = float(feature_support[index])
        utility_value = float(fit_temporal_support[index])
        pooled_value = float(pooled_calibrated_utility[index])
        if (
            not np.isfinite(residual_value)
            or not np.isfinite(raw_value)
            or raw_value <= 0.0
        ):
            raise ValueError(
                "EXP-054 directional ranking values are invalid"
            )
        for label, value in (
            ...
            if (
                ...
                raise ValueError(
                    f"EXP-054 directional {label} is invalid"
                )
        if _passes_residual_bound_cutoff(
    ...
    *,
    ...
    for raw_window in TEMPORAL_STABILITY_WINDOWS:
        ...
        evaluated = _evaluate_residual_bound_cutoff(
            ...
            residual_bound=residual_bound[index_array],
            residual_cutoff=residual_cutoff,
            feature_cutoff=feature_cutoff,
            utility_cutoff=utility_cutoff,
            pooled_cutoff=pooled_cutoff,
...
def run_fit_temporal_residual_bound_utility_model_cell_core(
    *,
    ...
    if any(frame.is_empty() for frame in split_frames.values()):
        ...
        raise ValueError(
            "EXP-054 model cell has empty required "
            f"split(s): {empty}"
        )
    ...
    if set(regime_frames) != set(PREDECESSOR_REGIME_NAMES):
        raise ValueError(
            "EXP-054 predecessor fit-regime identity drift"
        )
    if any(frame.is_empty() for frame in regime_frames.values()):
        ...
        raise ValueError(
            "EXP-054 model cell has empty fit regime(s): "
            f"{empty}"
        )
    ...
    for view_name, fit_frame in view_frames.items():
        ...
        if len(excluded) != 1:
            raise ValueError(
                "EXP-054 jackknife excluded-regime count drift"
            )
    ...
    for budget in CANDIDATE_BUDGET_ANCHORS:
        cutoff_record = _derive_residual_bound_cutoff(
            ...
            residual_bound=selection_residual_bound,
    ...
    result: dict[str, object] = {
        ...
        "fit": {
            ...
            "hist_gradient_boosting_classifier": {
                "status": "EXCLUDED_BY_DEC150_DEC163_DEC174_DEC185",
                "fit_attempt_count": 0,
            },
            "logistic_regression": {
                "status": "EXCLUDED_BY_DEC112_DEC150_DEC163_DEC174_DEC185",
                "fit_attempt_count": 0,
            },
        },
        "selection": {
            ...
            "fit_temporal_residual_bound_consensus": (
                selection_diagnostics
            ),
