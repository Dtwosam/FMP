...
from .model_successor_fit_temporal_residual_breadth_utility_training import (
...
FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_ARTIFACT_RUNNER_VERSION = (
...
FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_RESULT_EVIDENCE_VERSION = 1
...
def validate_fit_temporal_residual_breadth_utility_artifact_contract_sources(
    *,
    repository_root: Path,
) -> dict[str, object]:
    root = Path(repository_root)
    expected = {
        "dec199_training_core": (
            root
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(f"missing EXP-055 artifact dependency: {path}")
    ...
    if training[
        ...
        raise ValueError("EXP-055 training-core decision drift")

    fingerprint = fit_temporal_residual_breadth_utility_protocol_fingerprint()
    if len(fingerprint) != 64:
        raise ValueError("EXP-055 protocol fingerprint invalid")
    ...
    return {
        ...
        "dec199_training_core_blob_sha": actual["dec199_training_core"],
...
def validate_residual_breadth_cutoff(
    ...
    if not 0.0 <= values[0] <= 1.0:
        raise ValueError("EXP-055 residual-breadth cutoff out of range")
    for value in values[2:5]:
        if not 0.0 <= value <= 1.0:
            raise ValueError(
                "EXP-055 support/calibration cutoff out of range"
            )
    if values[5] <= 0.0:
        raise ValueError("EXP-055 raw cutoff must be positive")
    return values
...
def _validate_cell_fingerprint(row: Mapping[str, object]) -> None:
    ...
    if _validate_sha256(
        fingerprint,
        field="EXP-055 cell result fingerprint",
    ) != expected:
        raise ValueError("EXP-055 cell result fingerprint mismatch")
...
def _validate_fit_block(fit: Mapping[str, object]) -> dict[str, int]:
    ...
    for field, expected in exact.items():
        if fit.get(field) != expected:
            raise ValueError(f"EXP-055 {field} mismatch")

    residual_count = validate_fit_temporal_residual_references(fit)
    if residual_count != 24:
        raise ValueError("EXP-055 residual reference count mismatch")
...
def _validate_selection_block(selection: Mapping[str, object]) -> None:
    ...
    if not isinstance(diagnostics, Mapping):
        raise ValueError("EXP-055 consensus diagnostics missing")
    if diagnostics.get(
        "fit_temporal_residual_breadth_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_BREADTH_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-055 breadth bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_reference_count"
    ) != 24:
        raise ValueError("EXP-055 consensus residual reference count mismatch")
    ...
    for field in (
        ...
        if value is not None:
            ...
            if not 0.0 <= numeric <= 1.0:
                raise ValueError(
                    "EXP-055 consensus breadth diagnostic out of range"
                )
    ...
    _validate_sha256(
        ...
        field="EXP-055 consensus digest",
    )
    ...
    if not isinstance(variants, list) or len(variants) != len(
        ...
        raise ValueError("EXP-055 selection variant inventory mismatch")
    if {
        row.get("candidate_budget_anchor")
        for row in variants
        if isinstance(row, Mapping)
    } != set(CANDIDATE_BUDGET_ANCHORS):
        raise ValueError("EXP-055 selection budget identity mismatch")
    ...
    for row in variants:
        if not isinstance(row, Mapping):
            raise ValueError("EXP-055 selection variant malformed")
        status = row.get("status")
        evaluation = row.get("evaluation_status")
        if status == "BUDGET_UNAVAILABLE":
            if evaluation != "BUDGET_UNAVAILABLE":
                raise ValueError("EXP-055 unavailable variant status drift")
            if any(row.get(field) is not None for field in cutoff_fields):
                raise ValueError(
                    "EXP-055 unavailable variant exposes cutoff"
                )
            if row.get("selection_gate_passed") is not False:
                raise ValueError(
                    "EXP-055 unavailable variant cannot pass selection"
                )
            continue

        if status != "AVAILABLE" or evaluation != "EVALUATED":
            raise ValueError("EXP-055 evaluated variant status drift")
        validate_residual_breadth_cutoff(row)
        if not isinstance(row.get("scenarios"), Mapping):
            raise ValueError("EXP-055 evaluated variant scenarios missing")
        if not isinstance(row.get("temporal_stability"), Mapping):
            raise ValueError("EXP-055 temporal stability evidence missing")
    ...
    if selected is None:
        if status != (
            ...
            raise ValueError("EXP-055 no-selection status mismatch")
    else:
        if not isinstance(selected, Mapping):
            raise ValueError("EXP-055 selected variant malformed")
        if status != "SELECTED":
            raise ValueError("EXP-055 selected status mismatch")
        if selected.get("candidate_budget_anchor") not in (
            CANDIDATE_BUDGET_ANCHORS
        ):
            raise ValueError("EXP-055 selected budget mismatch")
        validate_residual_breadth_cutoff(selected)
...
def _validate_cell(
    ...
    if row.get("experiment_id") != (
        ...
        raise ValueError("EXP-055 cell experiment identity mismatch")
    if row.get("protocol_decision") != (
        FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_PROTOCOL_DECISION
    ):
        raise ValueError("EXP-055 cell protocol decision mismatch")
    if row.get("training_core_decision") != (
        FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_TRAINING_CORE_DECISION
    ):
        raise ValueError("EXP-055 cell training decision mismatch")
    ...
    if (
        ...
        raise ValueError("EXP-055 cell evidence malformed")
    ...
    if selection.get("selected_variant") is None:
        if validation.get("status") != "LOCKED_NO_SELECTION":
            raise ValueError("EXP-055 validation lock mismatch")
        if holdout.get("status") != "LOCKED_NO_SELECTION":
            raise ValueError("EXP-055 holdout lock mismatch")
    ...
    for field in (
        ...
        if row.get(field) is not False:
            raise ValueError(f"EXP-055 cell {field} must remain false")

    return counts


def compile_fit_temporal_residual_breadth_utility_model_result_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("EXP-055 model-result cell evidence incomplete")
    ...
    if totals != expected_totals:
        raise ValueError("EXP-055 aggregate evidence counts mismatch")

    payload = {
        "evidence_version": (
            FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_RESULT_EVIDENCE_VERSION
        ),
        "experiment_id": (
            FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_EXPERIMENT_ID
        ),
...
def write_fit_temporal_residual_breadth_utility_model_result_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "conflicting existing EXP-055 model-result evidence"
        )
    destination.write_text(payload, encoding="utf-8")


def run_authoritative_fit_temporal_residual_breadth_utility_model_bundle(
    **_: object,
) -> dict[str, object]:
    if (
        ...
        raise PermissionError(
            "DEC-200 source is non-executable for authoritative "
            "EXP-055 model fitting"
        )
    raise PermissionError("EXP-055 authoritative runner is not armed")
...
__all__ = [
    ...
    "write_fit_temporal_residual_breadth_utility_model_result_evidence",
]
