...
from . import model_successor_fit_temporal_residual_regime_floor_utility_training as _predecessor
...
FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_RESULT_EXECUTION_AUTHORIZED = False
...
def validate_fit_temporal_residual_regime_balance_utility_repair_training_sources(
    *,
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(f"missing EXP-060 training dependency: {path}")
    ...
    ] != _predecessor.FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_UTILITY_TRAINING_CORE_DECISION:
        raise ValueError("EXP-060 predecessor training decision drift")
    if _predecessor.MODEL_FIT_AUTHORIZED is not False:
        raise ValueError("EXP-060 predecessor model fit must remain closed")
    if (
        _predecessor
        ...
        raise ValueError(
            "EXP-060 predecessor result execution must remain closed"
        )
    ...
    if len(fingerprint) != 64:
        raise ValueError("EXP-060 protocol fingerprint is invalid")
    if IMPLEMENTATION_DEPENDENCY_REPAIR_AUTHORIZED is not True:
        raise ValueError("EXP-060 implementation repair is not authorized")
    if PROTOCOL_SEMANTICS_CHANGE_AUTHORIZED is not False:
        raise ValueError("EXP-060 protocol semantics change must remain closed")
    ...
    return {
        ...
        "dec253_protocol_blob_sha": actual["dec253_protocol"],
        "failed_exp059_training_core_blob_sha": actual[
            "failed_exp059_training_core"
        ],
...
def _fit_temporal_residual_regime_balance_utility(
    *,
    ...
    balance = np.full(len(directions), -np.inf, dtype=np.float64)
    floor = np.full(len(directions), -np.inf, dtype=np.float64)
    eligible = np.isin(
        directions,
        np.asarray(["LONG", "SHORT"], dtype=object),
    )
    if len(view_predictions) != FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_REGIME_COUNT:
        raise ValueError("EXP-060 residual regime view count drift")
    ...
    for index in np.flatnonzero(eligible):
        ...
        for view_name in sorted(view_predictions):
            ...
            residuals = residual_references[view_name][target]
            if len(residuals) != FIT_TEMPORAL_RESIDUAL_WINDOWS_PER_REGIME:
                raise ValueError(
                    "EXP-060 residual windows-per-regime count drift"
                )
            bounds = np.asarray(
                [
                    prediction + float(residual)
            ...
            if not np.isfinite(bounds).all():
                raise ValueError(
                    "EXP-060 residual regime bounds must be finite"
                )
            regime_means.append(float(np.mean(bounds)))
            bound_count += len(bounds)

        values = np.asarray(regime_means, dtype=np.float64)
        if (
            len(values) != FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_REGIME_COUNT
            or bound_count
            != FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_SOURCE_BOUND_COUNT_PER_ROW
            or not np.isfinite(values).all()
        ):
            raise ValueError(
                "EXP-060 residual regime-balance inventory invalid"
            )
        center = float(np.mean(values))
        spread = float(np.max(values) - np.min(values))
        floor[index] = float(np.min(values))
        balance[index] = (
            center
            - FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_PENALTY_MULTIPLIER
            * spread
...
def _candidate_directions_residual_regime_balance(
    *,
    directions: np.ndarray,
    residual_regime_balance: np.ndarray,
    residual_regime_floor: np.ndarray,
    ...
    residual_regime_balance_cutoff: float,
    residual_regime_floor_cutoff: float,
    ...
    directions = np.asarray(directions, dtype=object)
    values = [
        np.asarray(residual_regime_balance, dtype=np.float64),
        np.asarray(residual_regime_floor, dtype=np.float64),
    ...
    cutoffs = [
        residual_regime_balance_cutoff,
    ...
    if any(len(value) != len(directions) for value in values):
        raise ValueError("EXP-060 cutoff input length mismatch")
...
def _derive_residual_regime_balance_cutoff(
    *,
    directions: np.ndarray,
    residual_regime_balance: np.ndarray,
    residual_regime_floor: np.ndarray,
    ...
    if budget not in _base.CANDIDATE_BUDGET_ANCHORS:
        raise ValueError("unsupported EXP-060 candidate budget")
    directions = np.asarray(directions, dtype=object)
    arrays = [
        np.asarray(residual_regime_balance, dtype=np.float64),
        np.asarray(residual_regime_floor, dtype=np.float64),
    ...
    if (
        ...
        raise ValueError("EXP-060 cutoff row mismatch")
    ...
    for index in indices.tolist():
        if not np.isfinite(arrays[0][index]):
            raise ValueError(
                "EXP-060 residual regime-balance utility must be finite"
            )
        if not np.isfinite(arrays[1][index]):
            raise ValueError(
                "EXP-060 residual regime-floor utility must be finite"
            )
        if not np.isfinite(arrays[2][index]):
            raise ValueError(
                "EXP-060 residual lower-tail mean must be finite"
            )
        breadth = arrays[3][index]
        if not np.isfinite(breadth) or breadth < 0.0 or breadth > 1.0:
            raise ValueError(
                "EXP-060 residual breadth must be within [0, 1]"
            )
        if not np.isfinite(arrays[4][index]):
            raise ValueError(
                "EXP-060 residual-bound utility must be finite"
            )
        for label, value in (
            ("feature support", arrays[5][index]),
            ("fit-temporal utility support", arrays[6][index]),
            ("pooled calibrated utility", arrays[7][index]),
        ):
            if not np.isfinite(value) or value < 0.0 or value > 1.0:
                raise ValueError(
                    f"EXP-060 {label} must be within [0, 1]"
                )
        if not np.isfinite(arrays[8][index]) or arrays[8][index] <= 0.0:
            raise ValueError(
                "EXP-060 directional raw utility must be positive"
            )
    ...
    passing = _candidate_directions_residual_regime_balance(
        directions=directions,
        residual_regime_balance=arrays[0],
        residual_regime_floor=arrays[1],
        residual_lower_tail=arrays[2],
        residual_breadth=arrays[3],
        residual_bound_utility=arrays[4],
        feature_support=arrays[5],
        fit_temporal_support=arrays[6],
        pooled_calibrated_utility=arrays[7],
        raw_utility=arrays[8],
        residual_regime_balance_cutoff=cutoff[0],
        residual_regime_floor_cutoff=cutoff[1],
        residual_lower_tail_cutoff=cutoff[2],
        residual_breadth_cutoff=cutoff[3],
        residual_bound_cutoff=cutoff[4],
        feature_cutoff=cutoff[5],
        utility_cutoff=cutoff[6],
        pooled_cutoff=cutoff[7],
        raw_cutoff=cutoff[8],
    )
    return {
        "status": "AVAILABLE",
        "candidate_budget_anchor": budget,
        "eligible_selection_row_count": int(len(indices)),
        **{field: cutoff[index] for index, field in enumerate(fields)},
...
def _residual_regime_balance_consensus_digest(
    *,
    ...
    if not (
        len(row_ids)
        == len(directions)
        == len(residual_regime_balance)
    ):
        raise ValueError("EXP-060 consensus digest row mismatch")
    digest = hashlib.sha256()
    digest.update(str(predecessor_digest).encode("utf-8"))
    for row_id, direction, value in zip(
        row_ids,
        directions.tolist(),
        residual_regime_balance.tolist(),
        strict=True,
    ):
...
def _score_fit_temporal_residual_regime_balance_consensus(
    *,
    ...
    (
        ...
        residual_breadth,
        residual_lower_tail,
        residual_regime_floor,
        predecessor_diagnostics,
        row_ids,
        predecessor_digest,
    ) = _predecessor._score_fit_temporal_residual_regime_floor_consensus(
    ...
    if tuple(row_ids) != tuple(scored_row_ids):
        raise ValueError(
            "EXP-060 predecessor/scored row identity drift"
        )
    residual_regime_balance, recomputed_floor = (
    ...
    if not np.array_equal(
        recomputed_floor[eligible],
        np.asarray(residual_regime_floor, dtype=np.float64)[eligible],
    ):
        raise ValueError(
            "EXP-060 retained regime-floor value drift"
        )
    values = residual_regime_balance[eligible]
    if bool(eligible.any()) and not np.isfinite(values).all():
        raise ValueError(
            "EXP-060 eligible residual regime-balance utility is invalid"
        )
    diagnostics = {
        **dict(predecessor_diagnostics),
        ...
        "residual_regime_balance_view_prediction_digests": (
            view_prediction_digests
        ),
    }
    digest = _residual_regime_balance_consensus_digest(
        predecessor_digest=predecessor_digest,
        row_ids=row_ids,
        directions=directions,
        residual_regime_balance=residual_regime_balance,
    )
    return (
        directions,
        raw_utility,
...
def _evaluate_residual_regime_balance_cutoff(
    frame: pl.DataFrame,
    *,
    directions: np.ndarray,
    residual_regime_balance: np.ndarray,
    ...
    candidates = _candidate_directions_residual_regime_balance(
        directions=directions,
        residual_regime_balance=residual_regime_balance,
        residual_regime_floor=residual_regime_floor,
        ...
        residual_regime_balance_cutoff=residual_regime_balance_cutoff,
...
def _evaluate_temporal_stability_residual_regime_balance(
    *,
    ...
    if any(len(value) != selection_frame.height for value in arrays):
        raise ValueError("EXP-060 stability row mismatch")
    ...
    for raw_window in _base.TEMPORAL_STABILITY_WINDOWS:
        ...
        indices = _base._window_indices(
            ...
            end_exclusive=_base._parse_utc_date(str(window["end_exclusive"])),
        )
        frame = selection_frame[indices]
        index_array = np.asarray(indices, dtype=np.int64)
        row_ids = _base._base._row_identity(frame, cell=cell)
        evaluated = _evaluate_residual_regime_balance_cutoff(
            frame,
            directions=directions[index_array],
            residual_regime_balance=residual_regime_balance[index_array],
            residual_regime_floor=residual_regime_floor[index_array],
...
def _evaluate_forward_split_residual_regime_balance(
    *,
    ...
    (
        ...
        row_ids,
        digest,
    ) = _score_fit_temporal_residual_regime_balance_consensus(
        fitted_models=fitted_models,
        pooled_calibration_references=pooled_calibration_references,
        support_references=support_references,
        feature_references=feature_references,
    ...
    return {
        ...
        "row_count": frame.height,
        "fit_temporal_residual_regime_balance_utility_consensus": diagnostics,
        "fit_temporal_residual_regime_balance_utility_consensus_digest": digest,
        **evaluated,
    }


def run_fit_temporal_residual_regime_balance_utility_model_cell_core(
    *,
    ...
    if any(frame.is_empty() for frame in split_frames.values()):
        ...
        raise ValueError(
            f"EXP-060 model cell has empty required split(s): {empty}"
        )
    ...
    if set(regime_frames) != set(_base.PREDECESSOR_REGIME_NAMES):
        raise ValueError(
            "EXP-060 predecessor fit-regime identity drift"
        )
    if any(frame.is_empty() for frame in regime_frames.values()):
        ...
        raise ValueError(
            f"EXP-060 model cell has empty fit regime(s): {empty}"
        )
    ...
    for view_name, fit_frame in view_frames.items():
        ...
        if len(excluded) != 1:
            raise ValueError(
                "EXP-060 jackknife excluded-regime count drift"
            )
    ...
    (
        ...
        selection_residual_regime_floor,
        selection_residual_regime_balance,
    ...
    for budget in _base.CANDIDATE_BUDGET_ANCHORS:
        cutoff_record = _derive_residual_regime_balance_cutoff(
            directions=selection_directions,
            residual_regime_balance=selection_residual_regime_balance,
            residual_regime_floor=selection_residual_regime_floor,
        ...
        if aggregate_passed:
            stability = (
                _evaluate_temporal_stability_residual_regime_balance(
    ...
    result: dict[str, object] = {
        ...
        "dec253_merged_commit": DEC253_MERGED_COMMIT,
        "dec253_protocol_blob_sha": DEC253_PROTOCOL_BLOB_SHA,
        "failed_exp059_training_core_blob_sha": (
            FAILED_EXP059_TRAINING_CORE_BLOB_SHA
        ),
        "predecessor_training_core_blob_sha": (
        ...
        "fit": {
            ...
            "fit_temporal_support_reference_count": (
                _base.FIT_TEMPORAL_SUPPORT_REFERENCE_COUNT_PER_CELL
            ),
            ...
            "robust_fit_temporal_support_score_rule": (
                _base.ROBUST_FIT_TEMPORAL_SUPPORT_SCORE_RULE
            ),
            "fit_temporal_feature_support_references": (
                feature_evidence
            ),
            "fit_temporal_feature_support_reference_count": (
                _base.FIT_TEMPORAL_FEATURE_SUPPORT_REFERENCE_COUNT_PER_CELL
            ),
            ...
            "fit_temporal_feature_support_percentile_rule": (
                _base.FIT_TEMPORAL_FEATURE_SUPPORT_PERCENTILE_RULE
            ),
            "robust_fit_temporal_feature_support_score_rule": (
                _base.ROBUST_FIT_TEMPORAL_FEATURE_SUPPORT_SCORE_RULE
            ),
            ...
            "fit_temporal_residual_breadth_rule": (
                _predecessor._predecessor._predecessor.FIT_TEMPORAL_RESIDUAL_BREADTH_RULE
            ),
            "residual_breadth_positivity_rule": (
                _predecessor._predecessor._predecessor.RESIDUAL_BREADTH_POSITIVITY_RULE
            ),
            "residual_breadth_eligibility_rule": (
                _predecessor._predecessor._predecessor.RESIDUAL_BREADTH_ELIGIBILITY_RULE
            ),
            "fit_temporal_residual_breadth_bound_count_per_row": (
                _predecessor._predecessor._predecessor.FIT_TEMPORAL_RESIDUAL_BREADTH_BOUND_COUNT_PER_ROW
            ),
            "fit_temporal_residual_lower_tail_rule": (
                FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_RULE
            ),
            ...
            "full_fit_single_model": {
                "status": (
                    "FORBIDDEN_BY_DEC150_DEC163_DEC174_DEC185_DEC198_DEC231_DEC253"
                ),
                "fit_attempt_count": 0,
            },
            "view_weight_search": {
            ...
            "hist_gradient_boosting_classifier": {
                "status": (
                    "EXCLUDED_BY_DEC150_DEC163_DEC174_DEC185_DEC198_DEC231_DEC253"
                ),
                "fit_attempt_count": 0,
            },
            "logistic_regression": {
                "status": (
                    "EXCLUDED_BY_DEC112_DEC150_DEC163_DEC174_DEC185_DEC198_DEC231_DEC253"
                ),
                "fit_attempt_count": 0,
            },
        },
            ...
            "selected_variant": (
                {
                    ...
                    "selection_derived_residual_regime_balance_cutoff": float(
                        selected[
                            "selection_derived_residual_regime_balance_cutoff"
                        ]
                    ),
                    "selection_derived_residual_regime_floor_cutoff": float(
                        selected[
                            "selection_derived_residual_regime_floor_cutoff"
                        ]
                    ),
    ...
    residual_regime_balance_cutoff = float(
        selected["selection_derived_residual_regime_balance_cutoff"]
    )
    residual_regime_floor_cutoff = float(
        selected["selection_derived_residual_regime_floor_cutoff"]
    )
    residual_lower_tail_cutoff = float(
    ...
    validation = _evaluate_forward_split_residual_regime_balance(
        fitted_models=fitted_models,
        pooled_calibration_references=pooled_calibration_references,
        support_references=support_references,
        feature_references=feature_references,
    ...
    result["retrospective_holdout"] = (
        _evaluate_forward_split_residual_regime_balance(
            fitted_models=fitted_models,
            pooled_calibration_references=(
                pooled_calibration_references
            ),
...
def build_fit_temporal_residual_regime_balance_repair_training_core_gate(
) -> dict[str, object]:
    return {
        "experiment_id": (
            FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_EXPERIMENT_ID
        ...
        "residual_regime_count": FIT_TEMPORAL_RESIDUAL_REGIME_COUNT,
        "residual_windows_per_regime": FIT_TEMPORAL_RESIDUAL_WINDOWS_PER_REGIME,
        "residual_regime_floor_bound_count_per_row": (
            FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_BOUND_COUNT_PER_ROW
        ),
        "residual_regime_balance_regime_count": (
            FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_REGIME_COUNT
        ),
        "residual_regime_balance_source_bound_count_per_row": (
            FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_SOURCE_BOUND_COUNT_PER_ROW
        ),
        "residual_regime_balance_penalty_multiplier": (
            FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_PENALTY_MULTIPLIER
        ),
        "implementation_dependency_repair_authorized": (
            IMPLEMENTATION_DEPENDENCY_REPAIR_AUTHORIZED
        ...
        "residual_regime_balance_retained": True,
        "result_execution_authorized": False,
        "model_fit_authorized": False,
        "promotion_authorized": False,
        "shadow_authorized": False,
