...
STABILITY_MODEL_WORKFLOW_FILE = "phase8a-exp046-stability-model-training.yml"
...
def validate_stability_operator_checkout(
    *,
    branch: str,
    head_sha: str,
    origin_main_sha: str,
    ...
    if branch != "main":
        raise ValueError(
            "EXP-046 dispatch requires the local main branch"
        )
    ...
    if porcelain_status.strip():
        raise ValueError(
            "EXP-046 dispatch requires a clean working tree"
        )
...
def stability_model_runs_endpoint() -> str:
    return (
        f"repos/{REPOSITORY}/actions/workflows/"
        f"{STABILITY_MODEL_WORKFLOW_FILE}/runs"
...
def _positive_run_id(value: object) -> int:
    if (
        ...
        raise ValueError(
            "EXP-046 model run id must be a positive integer"
        )
    return value


def stability_model_run_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return f"repos/{REPOSITORY}/actions/runs/{value}"


def stability_model_run_jobs_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return (
...
def stability_model_run_artifacts_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return (
...
def stability_artifact_download_endpoint(
    artifact_id: int,
) -> str:
    if (
        ...
        raise ValueError(
            "EXP-046 artifact id must be a positive integer"
        )
    return (
...
def select_stability_manual_main_run(
    payload: Mapping[str, object],
) -> Mapping[str, object] | None:
    runs = payload.get("workflow_runs")
    if not isinstance(runs, list):
        raise ValueError(
            "EXP-046 workflow-run listing is malformed"
        )
    ...
    for raw in runs:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "EXP-046 workflow-run listing contains a malformed row"
            )
    ...
    if len(relevant) > 1:
        ...
        raise ValueError(
            "EXP-046 workflow has multiple manual main runs: "
            f"{ids}; DEC-109 authorizes only one"
        )
    if not relevant:
        return None
    ...
    if run.get("name") != STABILITY_MODEL_WORKFLOW_NAME:
        raise ValueError(
            "EXP-046 manual main run name mismatch"
        )
    if run.get("path") != STABILITY_MODEL_WORKFLOW_PATH:
        raise ValueError(
            "EXP-046 manual main run path mismatch"
        )
    status = run.get("status")
    conclusion = run.get("conclusion")
    if not isinstance(status, str) or not status:
        raise ValueError(
            "EXP-046 manual main run status is invalid"
        )
    if (
        ...
        raise ValueError(
            "EXP-046 manual main run conclusion is invalid"
        )
    return run


def classify_stability_model_run(
...
def stability_model_dispatch_command() -> tuple[str, ...]:
    return (
        "gh",
        "workflow",
        "run",
...
def build_stability_operator_report(
    *,
    checkout: Mapping[str, object],
    run: Mapping[str, object] | None,
) -> dict[str, object]:
    state = classify_stability_model_run(run)
    ...
    if state["run_state"] == "MISSING":
        command = stability_model_dispatch_command()
        return {
            **common,
            "stage": "STABILITY_MODEL_RUN_DISPATCH_REQUIRED",
            "next_action": (
                "Dispatch exactly one guarded historical EXP-046 "
    ...
    if state["run_state"] == "IN_PROGRESS":
        return {
            **common,
            "stage": "STABILITY_MODEL_RUN_IN_PROGRESS",
            "next_action": (
                "Inspect the existing EXP-046 model run; "
...
def dispatch_command_for_stability_report(
    report: Mapping[str, object],
) -> tuple[str, ...] | None:
    if report.get("read_only") is not True:
        raise ValueError(
            "EXP-046 next report must be explicitly read-only"
        )
    ...
    for field in (
        ...
        if report.get(field) is not False:
            raise ValueError(
                f"EXP-046 next report {field} must remain false"
            )

    stage = report.get("stage")
    if stage != "STABILITY_MODEL_RUN_DISPATCH_REQUIRED":
        for field in (
            ...
            if report.get(field) is not False:
                raise ValueError(
                    f"EXP-046 non-dispatch report {field} "
                    "must be false"
                )
        if "dispatch_command" in report:
            raise ValueError(
                "EXP-046 non-dispatch report must not contain "
                "a dispatch command"
            )
        return None
    ...
    for field in (
        ...
        if report.get(field) is not True:
            raise ValueError(
                f"EXP-046 dispatch report {field} must be true"
            )

    command = stability_model_dispatch_command()
    if report.get("dispatch_command") != shell_join(command):
        raise ValueError(
            "EXP-046 dispatch command does not match frozen workflow"
        )
    return command


def select_stability_aggregate_artifact(
    payload: Mapping[str, object],
    *,
    head_sha: str,
) -> dict[str, object]:
    sha = _validate_sha(
        head_sha,
        field="EXP-046 model-result head SHA",
    )
    artifacts = payload.get("artifacts")
    if not isinstance(artifacts, list):
        raise ValueError(
            "EXP-046 artifact listing is malformed"
        )
    expected = (
    ...
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "EXP-046 artifact listing contains a malformed row"
            )
    ...
    if len(matches) != 1:
        raise ValueError(
            "expected exactly one non-expired EXP-046 aggregate "
            f"artifact: {expected}"
        )
    ...
    if (
        ...
        raise ValueError(
            "EXP-046 aggregate artifact id is invalid"
        )
    return {
