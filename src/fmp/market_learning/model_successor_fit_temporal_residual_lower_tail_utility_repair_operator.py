...
DEC225_MERGED_COMMIT = "6ab3bff41b072b2a322ed1d89a3df852c8bef812"
...
FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_WORKFLOW_PATH = (
    ".github/workflows/"
    "phase8a-exp057-fit-temporal-residual-lower-tail-utility-model-training.yml"
)
...
def validate_fit_temporal_residual_lower_tail_utility_repair_operator_checkout(
    *,
    branch: str,
    head_sha: str,
    origin_main_sha: str,
    ...
    if branch != "main":
        raise ValueError("EXP-057 dispatch requires the local main branch")
    ...
    if porcelain_status.strip():
        raise ValueError("EXP-057 dispatch requires a clean working tree")
...
def fit_temporal_residual_lower_tail_utility_repair_model_runs_endpoint() -> str:
    return (
        f"repos/{REPOSITORY}/actions/workflows/"
        f"{FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_WORKFLOW_FILE}/runs"
...
def _positive_run_id(value: object) -> int:
    if (
        ...
        raise ValueError("EXP-057 model run id must be a positive integer")
    return value


def fit_temporal_residual_lower_tail_utility_repair_model_run_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return f"repos/{REPOSITORY}/actions/runs/{value}"


def fit_temporal_residual_lower_tail_utility_repair_model_run_jobs_endpoint(
    run_id: int,
) -> str:
    value = _positive_run_id(run_id)
...
def fit_temporal_residual_lower_tail_utility_repair_model_run_artifacts_endpoint(
    run_id: int,
) -> str:
    value = _positive_run_id(run_id)
...
def fit_temporal_residual_lower_tail_utility_repair_artifact_download_endpoint(
    artifact_id: int,
) -> str:
    if (
        ...
        raise ValueError("EXP-057 artifact id must be a positive integer")
    return f"repos/{REPOSITORY}/actions/artifacts/{artifact_id}/zip"


def select_fit_temporal_residual_lower_tail_utility_repair_manual_main_run(
    payload: Mapping[str, object],
) -> Mapping[str, object] | None:
    runs = payload.get("workflow_runs")
    if not isinstance(runs, list):
        raise ValueError("EXP-057 workflow-run listing is malformed")

    relevant: list[Mapping[str, object]] = []
    for raw in runs:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "EXP-057 workflow-run listing contains a malformed row"
            )
    ...
    if len(relevant) > 1:
        ...
        raise ValueError(
            "EXP-057 workflow has multiple manual main runs: "
            f"{ids}; DEC-225 authorizes only one"
        )
    if not relevant:
        return None
    ...
    if run.get("name") != FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_WORKFLOW_NAME:
        raise ValueError("EXP-057 manual main run name mismatch")
    if run.get("path") != FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_WORKFLOW_PATH:
        raise ValueError("EXP-057 manual main run path mismatch")
    status = run.get("status")
    conclusion = run.get("conclusion")
    if not isinstance(status, str) or not status:
        raise ValueError("EXP-057 manual main run status is invalid")
    if conclusion is not None and not isinstance(conclusion, str):
        raise ValueError("EXP-057 manual main run conclusion is invalid")
    return run


def classify_fit_temporal_residual_lower_tail_utility_repair_model_run(
...
def fit_temporal_residual_lower_tail_utility_repair_model_dispatch_command() -> tuple[str, ...]:
    return (
        "gh",
        "workflow",
        "run",
...
def build_fit_temporal_residual_lower_tail_utility_repair_operator_report(
    *,
    checkout: Mapping[str, object],
    run: Mapping[str, object] | None,
) -> dict[str, object]:
    state = classify_fit_temporal_residual_lower_tail_utility_repair_model_run(run)
    ...
    if state["run_state"] == "MISSING":
        command = fit_temporal_residual_lower_tail_utility_repair_model_dispatch_command()
        return {
            **common,
            "stage": (
            ...
            "next_action": (
                "Dispatch exactly one guarded historical EXP-057 "
    ...
    if state["run_state"] == "IN_PROGRESS":
        return {
            **common,
            "stage": "FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_RUN_IN_PROGRESS",
            "next_action": (
                "Inspect the existing EXP-057 model run; "
    ...
    return {
        **common,
        ...
        "next_action": (
            "Validate the terminal run through the frozen DEC-224 "
...
def fit_temporal_residual_lower_tail_utility_repair_operator_gate_metadata(
    gate: Mapping[str, object],
) -> dict[str, object]:
    decisions = {
        "fit_temporal_residual_lower_tail_utility_repair_model_execution_gate_decision": "DEC-223",
        "fit_temporal_residual_lower_tail_utility_repair_model_execution_authorization_decision": "DEC-225",
    }
    ...
    for field in sha_fields:
        metadata[field] = _validate_sha(
            gate.get(field),
            field=f"EXP-057 operator gate {field}",
        )
    return metadata


def dispatch_command_for_fit_temporal_residual_lower_tail_utility_repair_report(
    report: Mapping[str, object],
) -> tuple[str, ...] | None:
    if report.get("read_only") is not True:
        raise ValueError("EXP-057 next report must be explicitly read-only")
    ...
    for field in (
        ...
        if report.get(field) is not False:
            raise ValueError(
                f"EXP-057 next report {field} must remain false"
            )

    stage = report.get("stage")
    if stage != "FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_REPAIR_MODEL_RUN_DISPATCH_REQUIRED":
        for field in (
            ...
            if report.get(field) is not False:
                raise ValueError(
                    f"EXP-057 non-dispatch report {field} must be false"
                )
        if "dispatch_command" in report:
            raise ValueError(
                "EXP-057 non-dispatch report must not contain a dispatch command"
            )
        return None
    ...
    for field in (
        ...
        if report.get(field) is not True:
            raise ValueError(
                f"EXP-057 dispatch report {field} must be true"
            )

    command = fit_temporal_residual_lower_tail_utility_repair_model_dispatch_command()
    if report.get("dispatch_command") != shell_join(command):
        raise ValueError(
            "EXP-057 dispatch command does not match frozen workflow"
        )
    return command


def select_fit_temporal_residual_lower_tail_utility_repair_aggregate_artifact(
    payload: Mapping[str, object],
    *,
    head_sha: str,
) -> dict[str, object]:
    sha = _validate_sha(head_sha, field="EXP-057 model-result head SHA")
    artifacts = payload.get("artifacts")
    if not isinstance(artifacts, list):
        raise ValueError("EXP-057 artifact listing is malformed")
    expected = (
        f"exp057-fit-temporal-residual-lower-tail-utility-model-result-evidence-{sha}-"
    ...
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "EXP-057 artifact listing contains a malformed row"
            )
    ...
    if len(matches) != 1:
        raise ValueError(
            "expected exactly one non-expired EXP-057 aggregate "
            f"artifact: {expected}"
        )
    ...
    if (
        ...
        raise ValueError("EXP-057 aggregate artifact id is invalid")
    return {
...
__all__ = [
    ...
    "classify_fit_temporal_residual_lower_tail_utility_repair_model_run",
    ...
    "fit_temporal_residual_lower_tail_utility_repair_model_dispatch_command",
