...
from .model_successor_fit_temporal_residual_breadth_utility_result_review import (
    validate_fit_temporal_residual_breadth_utility_model_terminal_review,
)


FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_RESULT_DECISION = "DEC-207"

REVIEWED_MODEL_RUN_ID = 36163466744
REVIEWED_MODEL_HEAD_SHA = "fa3f90709fa71f3b49f43985c505707da3c524af"
REVIEWED_MODEL_RUN_ATTEMPT = 1
REVIEWED_MODEL_RUN_CONCLUSION = "success"

REVIEWED_AGGREGATE_ARTIFACT_ID = 10878066267
REVIEWED_AGGREGATE_ARTIFACT_NAME = (
    "exp055-fit-temporal-residual-breadth-utility-model-result-evidence-"
    "fa3f90709fa71f3b49f43985c505707da3c524af-"
    "from-feature-35867307338-outcome-35876715434"
)
REVIEWED_AGGREGATE_ARTIFACT_DIGEST = (
    "sha256:"
    "146e07d58b7dbd1086bd2aec63abdfc50dcd4427f382ae576a27c4ac732c27ba"
)
...
REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS = {
    ("USDJPY", "5m", 60, 250): (0, 0, 0, 251),
    ("USDJPY", "5m", 60, 1000): (0, 1, 83, 916),
}
...
def _validate_aggregate_artifact_identity(
    ...
    if not isinstance(artifacts, list):
        raise ValueError("DEC-207 artifact listing is malformed")

    matches: list[Mapping[str, object]] = []
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError("DEC-207 artifact row is malformed")
        if raw.get("name") == REVIEWED_AGGREGATE_ARTIFACT_NAME:
            matches.append(raw)

    if len(matches) != 1:
        raise ValueError(
            "DEC-207 requires exactly one reviewed aggregate artifact"
        )

    artifact = matches[0]
    if artifact.get("id") != REVIEWED_AGGREGATE_ARTIFACT_ID:
        raise ValueError("DEC-207 aggregate artifact id mismatch")
    if artifact.get("expired") is not False:
        raise ValueError(
            "DEC-207 aggregate artifact must remain non-expired"
        )
    if artifact.get("digest") != REVIEWED_AGGREGATE_ARTIFACT_DIGEST:
        raise ValueError("DEC-207 aggregate artifact digest mismatch")
...
def _aggregate_pass_variants(
    ...
    if not isinstance(cells, list):
        raise ValueError("DEC-207 aggregate evidence cells malformed")

    passed: list[tuple[str, str, int, int]] = []
    for raw_cell in cells:
        if not isinstance(raw_cell, Mapping):
            raise ValueError("DEC-207 aggregate cell malformed")
        ...
        if not isinstance(identity, Mapping) or not isinstance(selection, Mapping):
            raise ValueError("DEC-207 aggregate cell identity malformed")
        variants = selection.get("variants")
        if not isinstance(variants, list):
            raise ValueError("DEC-207 aggregate variants malformed")
        for variant in variants:
            if not isinstance(variant, Mapping):
                raise ValueError("DEC-207 aggregate variant malformed")
    ...
    for raw_cell in cells:
        ...
        for variant in variants:
            ...
            if not isinstance(stability, Mapping):
                raise ValueError("DEC-207 stability block malformed")
            windows = stability.get("windows")
            if not isinstance(windows, list) or len(windows) != 4:
                raise ValueError("DEC-207 stability window inventory mismatch")
            counts: list[int] = []
            for window in windows:
                if not isinstance(window, Mapping):
                    raise ValueError("DEC-207 stability window malformed")
                metrics = window.get("metrics")
                if not isinstance(metrics, Mapping):
                    raise ValueError("DEC-207 stability metrics malformed")
                count = metrics.get("directional_candidate_count")
                if not isinstance(count, int) or isinstance(count, bool) or count < 0:
                    raise ValueError(
                        "DEC-207 stability candidate count malformed"
                    )
                counts.append(count)
            key = (
                str(identity.get("symbol")),
...
def _summary(
    ...
    for raw_cell in cells:
        ...
        if not isinstance(fingerprint, str) or _canonical_hash(supplied) != fingerprint:
            raise ValueError("DEC-207 cell result fingerprint mismatch")
        ...
        if (
            ...
            raise ValueError("DEC-207 aggregate cell blocks malformed")
        ...
        if not isinstance(breadth_count, int) or isinstance(breadth_count, bool):
            raise ValueError("DEC-207 breadth-bound count malformed")
        breadth_bound_counts.add(breadth_count)
        ...
        for variant in variants:
            ...
            if (
                ...
                raise ValueError(
                    "DEC-207 eligible selection row count malformed"
                )
        ...
        if len(cell_eligible_counts) > 1:
            raise ValueError(
                "DEC-207 eligible selection row count drift within cell"
            )
        ...
        if selection.get("status") == (
            "NO_FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_STABLE_MODEL_CHALLENGER"
        ):
            no_stable_cells += 1
    ...
    if breadth_bound_counts != {
        ...
        raise ValueError("DEC-207 breadth-bound count drift")
    ...
    return {
        ...
        "no_fit_temporal_residual_breadth_utility_stable_model_challenger_count": (
            no_stable_cells
        ),
...
def validate_reviewed_fit_temporal_residual_breadth_utility_model_result(
    *,
    ...
    if run.get("id") != REVIEWED_MODEL_RUN_ID:
        raise ValueError("DEC-207 reviewed model run id mismatch")
    if run.get("head_sha") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-207 reviewed model head SHA mismatch")
    if run.get("run_attempt") != REVIEWED_MODEL_RUN_ATTEMPT:
        raise ValueError("DEC-207 reviewed model run attempt mismatch")
    if run.get("conclusion") != REVIEWED_MODEL_RUN_CONCLUSION:
        raise ValueError("DEC-207 reviewed model run conclusion mismatch")
    ...
    exact_review = {
        ...
        "fit_temporal_residual_breadth_utility_model_terminal_reviewed": True,
        "fit_temporal_residual_breadth_utility_model_result_review_decision": (
            "DEC-202"
        ),
        ...
        "fit_temporal_residual_breadth_utility_model_result_evidence_verified": (
            True
        ),
    ...
    for field, expected in exact_review.items():
        if review.get(field) != expected:
            raise ValueError(
                f"DEC-207 terminal review {field} mismatch"
            )
    ...
    for field, expected in exact_summary.items():
        if summary.get(field) != expected:
            raise ValueError(f"DEC-207 reviewed {field} mismatch")

    passes = _aggregate_pass_variants(aggregate_evidence)
    if passes != tuple(sorted(REVIEWED_AGGREGATE_PASS_VARIANTS)):
        raise ValueError("DEC-207 aggregate pass inventory mismatch")
    ...
    if window_counts != REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS:
        raise ValueError(
            "DEC-207 aggregate-pass stability-window inventory mismatch"
        )

    return {
        **review,
        **summary,
        "fit_temporal_residual_breadth_utility_model_result_decision": (
            FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_RESULT_DECISION
        ),
        "stage": (
            "FIT_TEMPORAL_RESIDUAL_BREADTH_UTILITY_MODEL_RESULT_"
...
__all__ = [
    ...
    "SHADOW_AUTHORIZED",
    "TRADING_AUTHORIZED",
    "validate_reviewed_fit_temporal_residual_breadth_utility_model_result",
]
