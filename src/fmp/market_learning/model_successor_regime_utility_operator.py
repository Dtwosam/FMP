...
def validate_regime_utility_operator_checkout(
    *,
    branch: str,
    head_sha: str,
    origin_main_sha: str,
    ...
    if branch != "main":
        raise ValueError(
            "EXP-049 dispatch requires the local main branch"
        )
    ...
    if porcelain_status.strip():
        raise ValueError(
            "EXP-049 dispatch requires a clean working tree"
        )
    ...
    return {
        ...
        "dec137_merged_commit": DEC137_MERGED_COMMIT,
        "branch": "main",
        "head_sha": head,
        "clean_worktree": True,
...
def regime_utility_model_runs_endpoint() -> str:
    return (
        f"repos/{REPOSITORY}/actions/workflows/"
        f"{REGIME_UTILITY_MODEL_WORKFLOW_FILE}/runs"
...
def _positive_run_id(value: object) -> int:
    if (
        ...
        raise ValueError(
            "EXP-049 model run id must be a positive integer"
        )
    return value


def regime_utility_model_run_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return f"repos/{REPOSITORY}/actions/runs/{value}"


def regime_utility_model_run_jobs_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return (
...
def regime_utility_model_run_artifacts_endpoint(run_id: int) -> str:
    value = _positive_run_id(run_id)
    return (
...
def regime_utility_artifact_download_endpoint(
    artifact_id: int,
) -> str:
    if (
        ...
        raise ValueError(
            "EXP-049 artifact id must be a positive integer"
        )
    return (
...
def select_regime_utility_manual_main_run(
    payload: Mapping[str, object],
) -> Mapping[str, object] | None:
    runs = payload.get("workflow_runs")
    if not isinstance(runs, list):
        raise ValueError(
            "EXP-049 workflow-run listing is malformed"
        )
    ...
    for raw in runs:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "EXP-049 workflow-run listing contains a malformed row"
            )
    ...
    if len(relevant) > 1:
        ...
        raise ValueError(
            "EXP-049 workflow has multiple manual main runs: "
            f"{ids}; DEC-137 authorizes only one"
        )
    if not relevant:
        return None
    ...
    if run.get("name") != REGIME_UTILITY_MODEL_WORKFLOW_NAME:
        raise ValueError(
            "EXP-049 manual main run name mismatch"
        )
    if run.get("path") != REGIME_UTILITY_MODEL_WORKFLOW_PATH:
        raise ValueError(
            "EXP-049 manual main run path mismatch"
        )
    status = run.get("status")
    conclusion = run.get("conclusion")
    if not isinstance(status, str) or not status:
        raise ValueError(
            "EXP-049 manual main run status is invalid"
        )
    if (
        ...
        raise ValueError(
            "EXP-049 manual main run conclusion is invalid"
        )
    return run


def classify_regime_utility_model_run(
...
def regime_utility_model_dispatch_command() -> tuple[str, ...]:
    return (
        "gh",
        "workflow",
        "run",
...
def build_regime_utility_operator_report(
    *,
    checkout: Mapping[str, object],
    run: Mapping[str, object] | None,
) -> dict[str, object]:
    state = classify_regime_utility_model_run(run)
    ...
    if state["run_state"] == "MISSING":
        command = regime_utility_model_dispatch_command()
        return {
            **common,
            "stage": "REGIME_UTILITY_MODEL_RUN_DISPATCH_REQUIRED",
            "next_action": (
                "Dispatch exactly one guarded historical EXP-049 "
    ...
    if state["run_state"] == "IN_PROGRESS":
        return {
            **common,
            "stage": "REGIME_UTILITY_MODEL_RUN_IN_PROGRESS",
            "next_action": (
                "Inspect the existing EXP-049 model run; "
    ...
    return {
        **common,
        "stage": "REGIME_UTILITY_MODEL_TERMINAL_REVIEW_REQUIRED",
        "next_action": (
            "Validate the terminal run through the frozen DEC-136 "
...
def regime_utility_operator_gate_metadata(
    ...
    for field, expected in decisions.items():
        if gate.get(field) != expected:
            raise ValueError(
                f"EXP-049 operator gate {field} mismatch"
            )
    ...
    for field in sha_fields:
        metadata[field] = _validate_sha(
            gate.get(field),
            field=f"EXP-049 operator gate {field}",
        )
    return metadata


def dispatch_command_for_regime_utility_report(
    report: Mapping[str, object],
) -> tuple[str, ...] | None:
    if report.get("read_only") is not True:
        raise ValueError(
            "EXP-049 next report must be explicitly read-only"
        )
    ...
    for field in (
        ...
        if report.get(field) is not False:
            raise ValueError(
                f"EXP-049 next report {field} must remain false"
            )

    stage = report.get("stage")
    if stage != "REGIME_UTILITY_MODEL_RUN_DISPATCH_REQUIRED":
        for field in (
            ...
            if report.get(field) is not False:
                raise ValueError(
                    f"EXP-049 non-dispatch report {field} "
                    "must be false"
                )
        if "dispatch_command" in report:
            raise ValueError(
                "EXP-049 non-dispatch report must not contain "
                "a dispatch command"
            )
        return None
    ...
    for field in (
        ...
        if report.get(field) is not True:
            raise ValueError(
                f"EXP-049 dispatch report {field} must be true"
            )

    command = regime_utility_model_dispatch_command()
    if report.get("dispatch_command") != shell_join(command):
        raise ValueError(
            "EXP-049 dispatch command does not match frozen workflow"
        )
    return command


def select_regime_utility_aggregate_artifact(
    payload: Mapping[str, object],
    *,
    head_sha: str,
) -> dict[str, object]:
    sha = _validate_sha(
        head_sha,
        field="EXP-049 model-result head SHA",
    )
    ...
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "EXP-049 artifact listing contains a malformed row"
            )
    ...
    if len(matches) != 1:
        raise ValueError(
            "expected exactly one non-expired EXP-049 aggregate "
            f"artifact: {expected}"
        )
    ...
    if (
        ...
        raise ValueError(
            "EXP-049 aggregate artifact id is invalid"
        )
    return {
