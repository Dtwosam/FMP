...
def validate_fit_temporal_residual_lower_tail_utility_repair_training_sources(
    *,
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(f"missing EXP-057 training dependency: {path}")
    ...
    if predecessor[
        ...
        raise ValueError("EXP-057 predecessor training decision drift")
    if _predecessor.MODEL_FIT_AUTHORIZED is not False:
        raise ValueError("EXP-057 predecessor model fit must remain closed")
    if (
        _predecessor
        ...
        raise ValueError(
            "EXP-057 predecessor result execution must remain closed"
        )
    ...
    if len(fingerprint) != 64:
        raise ValueError("EXP-057 repair protocol fingerprint is invalid")
    if IMPLEMENTATION_DEPENDENCY_REPAIR_AUTHORIZED is not True:
        raise ValueError("EXP-057 implementation repair is not authorized")
    ...
    return {
        ...
        "dec220_protocol_blob_sha": actual["dec220_protocol"],
        "failed_exp056_training_core_blob_sha": actual[
            "failed_exp056_training_core"
        ],
        "exp055_predecessor_training_core_blob_sha": actual[
            "exp055_predecessor_training_core"
        ],
    ...
    *,
    ...
    for index in np.flatnonzero(eligible):
        ...
        if (
            ...
            raise ValueError(
                "EXP-057 residual lower-tail bound inventory invalid"
            )
...
def _candidate_directions_residual_lower_tail(
    *,
    ...
    if any(len(value) != len(directions) for value in values):
        raise ValueError("EXP-057 cutoff input length mismatch")
...
def _derive_residual_lower_tail_cutoff(
    *,
    ...
    if budget not in _base.CANDIDATE_BUDGET_ANCHORS:
        raise ValueError("unsupported EXP-057 candidate budget")
    directions = np.asarray(directions, dtype=object)
    arrays = [
        np.asarray(residual_lower_tail, dtype=np.float64),
        np.asarray(residual_breadth, dtype=np.float64),
    ...
    if (
        ...
        raise ValueError("EXP-057 cutoff row mismatch")
    ...
    for index in indices.tolist():
        if not np.isfinite(arrays[0][index]):
            raise ValueError(
                "EXP-057 residual lower-tail mean must be finite"
            )
        breadth = arrays[1][index]
        if not np.isfinite(breadth) or breadth < 0.0 or breadth > 1.0:
            raise ValueError(
                "EXP-057 residual breadth must be within [0, 1]"
            )
        if not np.isfinite(arrays[2][index]):
            raise ValueError(
                "EXP-057 residual-bound utility must be finite"
            )
        for label, value in (
            ...
            if not np.isfinite(value) or value < 0.0 or value > 1.0:
                raise ValueError(
                    f"EXP-057 {label} must be within [0, 1]"
                )
        if not np.isfinite(arrays[6][index]) or arrays[6][index] <= 0.0:
            raise ValueError(
                "EXP-057 directional raw utility must be positive"
            )
...
def _residual_lower_tail_consensus_digest(
    *,
    ...
    if not (
        ...
        raise ValueError("EXP-057 consensus digest row mismatch")
...
def _score_fit_temporal_residual_lower_tail_consensus(
    *,
    ...
    if tuple(row_ids) != tuple(scored_row_ids):
        raise ValueError(
            "EXP-057 predecessor/scored row identity drift"
        )
    ...
    if bool(eligible.any()) and not np.isfinite(values).all():
        raise ValueError(
            "EXP-057 eligible residual lower-tail mean is invalid"
        )
    diagnostics = {
...
def _evaluate_temporal_stability_residual_lower_tail(
    *,
    ...
    if any(len(value) != selection_frame.height for value in arrays):
        raise ValueError("EXP-057 stability row mismatch")
...
def run_fit_temporal_residual_lower_tail_utility_model_cell_core(
    *,
    ...
    if any(frame.is_empty() for frame in split_frames.values()):
        ...
        raise ValueError(
            f"EXP-057 model cell has empty required split(s): {empty}"
        )
    ...
    if set(regime_frames) != set(_base.PREDECESSOR_REGIME_NAMES):
        raise ValueError(
            "EXP-057 predecessor fit-regime identity drift"
        )
    if any(frame.is_empty() for frame in regime_frames.values()):
        ...
        raise ValueError(
            f"EXP-057 model cell has empty fit regime(s): {empty}"
        )
    ...
    for view_name, fit_frame in view_frames.items():
        ...
        if len(excluded) != 1:
            raise ValueError(
                "EXP-057 jackknife excluded-regime count drift"
            )
    ...
    result: dict[str, object] = {
        ...
        "dec220_merged_commit": DEC220_MERGED_COMMIT,
        "dec220_protocol_blob_sha": DEC220_PROTOCOL_BLOB_SHA,
        "failed_exp056_training_core_blob_sha": (
            FAILED_EXP056_TRAINING_CORE_BLOB_SHA
        ),
        "exp055_predecessor_training_core_blob_sha": (
