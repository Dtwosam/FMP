...
from .model_successor_fit_temporal_residual_lower_tail_utility_repair_result_review import (
...
REVIEWED_MODEL_RUN_ID = 36192572271
REVIEWED_MODEL_HEAD_SHA = "491a2e2715b4da7013737ecaebc65a58ac3417f9"
REVIEWED_MODEL_RUN_ATTEMPT = 1
REVIEWED_MODEL_RUN_CONCLUSION = "success"

REVIEWED_AGGREGATE_ARTIFACT_ID = 10890155292
REVIEWED_AGGREGATE_ARTIFACT_NAME = (
    "exp057-fit-temporal-residual-lower-tail-utility-model-result-evidence-"
    "491a2e2715b4da7013737ecaebc65a58ac3417f9-"
...
def _validate_aggregate_artifact_identity(
    ...
    if not isinstance(artifacts, list):
        raise ValueError("DEC-229 artifact listing is malformed")

    matches: list[Mapping[str, object]] = []
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError("DEC-229 artifact row is malformed")
        if raw.get("name") == REVIEWED_AGGREGATE_ARTIFACT_NAME:
            matches.append(raw)

    if len(matches) != 1:
        raise ValueError(
            "DEC-229 requires exactly one reviewed aggregate artifact"
        )

    artifact = matches[0]
    if artifact.get("id") != REVIEWED_AGGREGATE_ARTIFACT_ID:
        raise ValueError("DEC-229 aggregate artifact id mismatch")
    if artifact.get("expired") is not False:
        raise ValueError(
            "DEC-229 aggregate artifact must remain non-expired"
        )
    if artifact.get("digest") != REVIEWED_AGGREGATE_ARTIFACT_DIGEST:
        raise ValueError("DEC-229 aggregate artifact digest mismatch")
...
def _aggregate_pass_variants(
    ...
    if not isinstance(cells, list):
        raise ValueError("DEC-229 aggregate evidence cells malformed")

    passed: list[tuple[str, str, int, int]] = []
    for raw_cell in cells:
        if not isinstance(raw_cell, Mapping):
            raise ValueError("DEC-229 aggregate cell malformed")
        ...
        if not isinstance(identity, Mapping) or not isinstance(selection, Mapping):
            raise ValueError("DEC-229 aggregate cell identity malformed")
        variants = selection.get("variants")
        if not isinstance(variants, list):
            raise ValueError("DEC-229 aggregate variants malformed")
        for variant in variants:
            if not isinstance(variant, Mapping):
                raise ValueError("DEC-229 aggregate variant malformed")
    ...
    for raw_cell in cells:
        ...
        for variant in variants:
            ...
            if not isinstance(stability, Mapping):
                raise ValueError("DEC-229 stability block malformed")
            windows = stability.get("windows")
            if not isinstance(windows, list) or len(windows) != 4:
                raise ValueError("DEC-229 stability window inventory mismatch")
            counts: list[int] = []
            for window in windows:
                if not isinstance(window, Mapping):
                    raise ValueError("DEC-229 stability window malformed")
                metrics = window.get("metrics")
                if not isinstance(metrics, Mapping):
                    raise ValueError("DEC-229 stability metrics malformed")
                count = metrics.get("directional_candidate_count")
                if not isinstance(count, int) or isinstance(count, bool) or count < 0:
                    raise ValueError(
                        "DEC-229 stability candidate count malformed"
                    )
                counts.append(count)
            key = (
                str(identity.get("symbol")),
...
def _summary(
    ...
    if aggregate_evidence.get("code_commit") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-229 aggregate evidence code commit mismatch")
    if aggregate_evidence.get("evidence_fingerprint") != REVIEWED_EVIDENCE_FINGERPRINT:
        raise ValueError("DEC-229 aggregate evidence fingerprint mismatch")
    ...
    for raw_cell in cells:
        ...
        if not isinstance(fingerprint, str) or _canonical_hash(supplied) != fingerprint:
            raise ValueError("DEC-229 cell result fingerprint mismatch")
        ...
        if (
            ...
            raise ValueError("DEC-229 aggregate cell blocks malformed")
        ...
        for value, label, target in (
            ...
            if not isinstance(value, int) or isinstance(value, bool):
                raise ValueError(f"DEC-229 {label} malformed")
            target.add(value)
        ...
        for variant in variants:
            ...
            if (
                ...
                raise ValueError(
                    "DEC-229 eligible selection row count malformed"
                )
            ...
            if status == "AVAILABLE":
                ...
                raise ValueError("DEC-229 unexpected variant status")
        ...
        if len(cell_eligible_counts) != 1:
            raise ValueError(
                "DEC-229 eligible selection row count drift within cell"
            )
    ...
    if breadth_bound_counts != {
        ...
        raise ValueError("DEC-229 breadth-bound count drift")
    if lower_tail_bound_counts != {
        REVIEWED_VERIFIED_RESIDUAL_LOWER_TAIL_BOUND_COUNT_PER_ROW
    }:
        raise ValueError("DEC-229 lower-tail bound count drift")
    if lower_tail_counts != {
        REVIEWED_VERIFIED_RESIDUAL_LOWER_TAIL_COUNT
    }:
        raise ValueError("DEC-229 lower-tail count drift")
...
def validate_reviewed_fit_temporal_residual_lower_tail_utility_repair_model_result(
    *,
    ...
    if run.get("id") != REVIEWED_MODEL_RUN_ID:
        raise ValueError("DEC-229 reviewed model run id mismatch")
    if run.get("head_sha") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-229 reviewed model head SHA mismatch")
    if run.get("run_attempt") != REVIEWED_MODEL_RUN_ATTEMPT:
        raise ValueError("DEC-229 reviewed model run attempt mismatch")
    if run.get("conclusion") != REVIEWED_MODEL_RUN_CONCLUSION:
        raise ValueError("DEC-229 reviewed model run conclusion mismatch")
    ...
    for field, expected in exact_review.items():
        if review.get(field) != expected:
            raise ValueError(
                f"DEC-229 terminal review {field} mismatch"
            )
    ...
    for field, expected in exact_summary.items():
        if summary.get(field) != expected:
            raise ValueError(f"DEC-229 reviewed {field} mismatch")

    passes = _aggregate_pass_variants(aggregate_evidence)
    if passes != tuple(sorted(REVIEWED_AGGREGATE_PASS_VARIANTS)):
        raise ValueError("DEC-229 aggregate pass inventory mismatch")
    ...
    if window_counts != REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS:
        raise ValueError(
            "DEC-229 aggregate-pass stability-window inventory mismatch"
        )
