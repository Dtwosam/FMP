...
from .model_successor_regime_utility_result_review import (
    validate_regime_utility_model_terminal_review,
)


REGIME_UTILITY_MODEL_RESULT_DECISION = "DEC-139"

REVIEWED_MODEL_RUN_ID = 36029925264
...
REVIEWED_AGGREGATE_ARTIFACT_ID = 10822530555
...
REVIEWED_AGGREGATE_ARTIFACT_DIGEST = (
    "sha256:"
    "2cafb18dc1130f4fe0bf229b7df1deda24bfb7b1295f9a6ad688c1f5c1fd407c"
)
...
REVIEWED_AGGREGATE_SELECTION_PASS_VARIANT_COUNT = 8
REVIEWED_STABLE_SELECTION_PASS_VARIANT_COUNT = 0
REVIEWED_UNAVAILABLE_BUDGET_VARIANT_COUNT = 31
REVIEWED_UTILITY_ELIGIBLE_SELECTION_ROW_COUNT = 14158
...
def _validate_aggregate_artifact_identity(
    ...
    if not isinstance(artifacts, list):
        raise ValueError(
            "DEC-139 artifact listing is malformed"
        )
    ...
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError(
                "DEC-139 artifact row is malformed"
            )
    ...
    if len(matches) != 1:
        raise ValueError(
            "DEC-139 requires exactly one reviewed aggregate artifact"
        )

    artifact = matches[0]
    if artifact.get("id") != REVIEWED_AGGREGATE_ARTIFACT_ID:
        raise ValueError(
            "DEC-139 aggregate artifact id mismatch"
        )
    if artifact.get("expired") is not False:
        raise ValueError(
            "DEC-139 aggregate artifact must remain non-expired"
        )
    ...
    if (
        ...
        raise ValueError(
            "DEC-139 aggregate artifact digest mismatch"
        )


def validate_reviewed_regime_utility_model_result(
    *,
    ...
    if run.get("id") != REVIEWED_MODEL_RUN_ID:
        raise ValueError(
            "DEC-139 reviewed model run id mismatch"
        )
    if run.get("head_sha") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError(
            "DEC-139 reviewed model head SHA mismatch"
        )
    if run.get("run_attempt") != REVIEWED_MODEL_RUN_ATTEMPT:
        raise ValueError(
            "DEC-139 reviewed model run attempt mismatch"
        )
    if run.get("conclusion") != REVIEWED_MODEL_RUN_CONCLUSION:
        raise ValueError(
            "DEC-139 reviewed model run conclusion mismatch"
        )
    ...
    for field, expected in exact.items():
        if review.get(field) != expected:
            raise ValueError(
                f"DEC-139 terminal review {field} mismatch"
            )
    ...
    return {
        **review,
        ...
        "stage": (
            "REGIME_UTILITY_MODEL_RESULT_REVIEWED_"
