...
from .model_successor_fit_temporal_residual_regime_balance_utility_repair_result_review import (
    validate_fit_temporal_residual_regime_balance_utility_repair_model_terminal_review,
)


FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_RESULT_DECISION = (
    "DEC-262"
)

REVIEWED_MODEL_RUN_ID = 36260155597
REVIEWED_MODEL_HEAD_SHA = "0062546fda38bfc768122cf03b9a4d69d1b8e0b7"
REVIEWED_MODEL_RUN_ATTEMPT = 1
REVIEWED_MODEL_RUN_CONCLUSION = "success"

REVIEWED_AGGREGATE_ARTIFACT_ID = 10912798284
...
REVIEWED_VERIFIED_RESIDUAL_BREADTH_BOUND_COUNT_PER_ROW = 12
REVIEWED_VERIFIED_RESIDUAL_LOWER_TAIL_BOUND_COUNT_PER_ROW = 12
REVIEWED_VERIFIED_RESIDUAL_LOWER_TAIL_COUNT = 3
REVIEWED_VERIFIED_RESIDUAL_REGIME_COUNT = 3
REVIEWED_VERIFIED_RESIDUAL_WINDOWS_PER_REGIME = 4
REVIEWED_VERIFIED_RESIDUAL_REGIME_FLOOR_BOUND_COUNT_PER_ROW = 12
REVIEWED_VERIFIED_RESIDUAL_REGIME_BALANCE_REGIME_COUNT = 3
REVIEWED_VERIFIED_RESIDUAL_REGIME_BALANCE_SOURCE_BOUND_COUNT_PER_ROW = 12
REVIEWED_VERIFIED_RESIDUAL_REGIME_BALANCE_PENALTY_MULTIPLIER = 1.0
...
REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS = {
    ("USDJPY", "5m", 60, 500): (0, 0, 27, 473),
    ("USDJPY", "5m", 60, 1000): (0, 3, 105, 892),
}
...
def _validate_aggregate_artifact_identity(
    ...
    if not isinstance(artifacts, list):
        raise ValueError("DEC-262 artifact listing is malformed")

    matches: list[Mapping[str, object]] = []
    for raw in artifacts:
        if not isinstance(raw, Mapping):
            raise ValueError("DEC-262 artifact row is malformed")
        if raw.get("name") == REVIEWED_AGGREGATE_ARTIFACT_NAME:
            matches.append(raw)

    if len(matches) != 1:
        raise ValueError(
            "DEC-262 requires exactly one reviewed aggregate artifact"
        )

    artifact = matches[0]
    if artifact.get("id") != REVIEWED_AGGREGATE_ARTIFACT_ID:
        raise ValueError("DEC-262 aggregate artifact id mismatch")
    if artifact.get("expired") is not False:
        raise ValueError(
            "DEC-262 aggregate artifact must remain non-expired"
        )
    if artifact.get("digest") != REVIEWED_AGGREGATE_ARTIFACT_DIGEST:
        raise ValueError("DEC-262 aggregate artifact digest mismatch")
...
def _aggregate_pass_variants(
    ...
    if not isinstance(cells, list):
        raise ValueError("DEC-262 aggregate evidence cells malformed")

    passed: list[tuple[str, str, int, int]] = []
    for raw_cell in cells:
        if not isinstance(raw_cell, Mapping):
            raise ValueError("DEC-262 aggregate cell malformed")
        ...
        if not isinstance(identity, Mapping) or not isinstance(selection, Mapping):
            raise ValueError("DEC-262 aggregate cell identity malformed")
        variants = selection.get("variants")
        if not isinstance(variants, list):
            raise ValueError("DEC-262 aggregate variants malformed")
        for variant in variants:
            if not isinstance(variant, Mapping):
                raise ValueError("DEC-262 aggregate variant malformed")
...
def _aggregate_pass_window_candidate_counts(
    aggregate_evidence: Mapping[str, object],
) -> dict[tuple[str, str, int, int], tuple[int, int, int, int]]:
    cells = aggregate_evidence.get("cells")
    if not isinstance(cells, list):
        raise ValueError("DEC-262 aggregate evidence cells malformed")
    ...
    for raw_cell in cells:
        ...
        for variant in variants:
            ...
            if variant.get("aggregate_selection_gate_passed") is not True:
                continue
            stability = variant.get("temporal_stability")
            if not isinstance(stability, Mapping):
                raise ValueError("DEC-262 stability block malformed")
            windows = stability.get("windows")
            if not isinstance(windows, list) or len(windows) != 4:
                raise ValueError("DEC-262 stability window inventory mismatch")
            counts: list[int] = []
            for window in windows:
                if not isinstance(window, Mapping):
                    raise ValueError("DEC-262 stability window malformed")
                metrics = window.get("metrics")
                if not isinstance(metrics, Mapping):
                    raise ValueError("DEC-262 stability metrics malformed")
                count = metrics.get("directional_candidate_count")
                if not isinstance(count, int) or isinstance(count, bool) or count < 0:
                    raise ValueError(
                        "DEC-262 stability candidate count malformed"
                    )
                counts.append(count)
            key = (
                str(identity.get("symbol")),
            ...
            result[key] = tuple(counts)  # type: ignore[assignment]
    return result
...
def _summary(
    ...
    if aggregate_evidence.get("code_commit") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-262 aggregate evidence code commit mismatch")
    if aggregate_evidence.get("evidence_fingerprint") != REVIEWED_EVIDENCE_FINGERPRINT:
        raise ValueError("DEC-262 aggregate evidence fingerprint mismatch")
    ...
    feature_count = 0
    residual_count = 0
    breadth_bound_counts: set[int] = set()
    lower_tail_bound_counts: set[int] = set()
    lower_tail_counts: set[int] = set()
    regime_counts: set[int] = set()
    windows_per_regime_counts: set[int] = set()
    regime_floor_bound_counts: set[int] = set()
    regime_balance_regime_counts: set[int] = set()
    regime_balance_source_bound_counts: set[int] = set()
    regime_balance_penalty_multipliers: set[float] = set()
    total_variants = 0
    ...
    for raw_cell in cells:
        ...
        if not isinstance(fingerprint, str) or _canonical_hash(supplied) != fingerprint:
            raise ValueError("DEC-262 cell result fingerprint mismatch")
        ...
        if (
            ...
            raise ValueError("DEC-262 aggregate cell blocks malformed")
        ...
        for value, label, target in (
            ...
            (
                regime_count,
                "residual regime count",
                regime_counts,
            ),
            (
                windows_per_regime,
                "residual windows-per-regime count",
                windows_per_regime_counts,
            ),
            (
                regime_floor_bound_count,
                "regime-floor bound count",
                regime_floor_bound_counts,
            ),
            (
                regime_balance_regime_count,
                "regime-balance regime count",
                regime_balance_regime_counts,
            ),
            (
                regime_balance_source_bound_count,
                "regime-balance source-bound count",
                regime_balance_source_bound_counts,
            ),
        ):
            if not isinstance(value, int) or isinstance(value, bool):
                raise ValueError(f"DEC-262 {label} malformed")
            target.add(value)

        if (
            not isinstance(regime_balance_penalty_multiplier, (int, float))
            or isinstance(regime_balance_penalty_multiplier, bool)
        ):
            raise ValueError("DEC-262 regime-balance penalty multiplier malformed")
        regime_balance_penalty_multipliers.add(
            float(regime_balance_penalty_multiplier)
        )
        ...
        for variant in variants:
            ...
            if (
                ...
                raise ValueError(
                    "DEC-262 eligible selection row count malformed"
                )
            ...
            elif status == "BUDGET_UNAVAILABLE":
                unavailable_variants += 1
            else:
                raise ValueError("DEC-262 unexpected variant status")
        ...
        if len(cell_eligible_counts) != 1:
            raise ValueError(
                "DEC-262 eligible selection row count drift within cell"
            )
    ...
    if breadth_bound_counts != {
        REVIEWED_VERIFIED_RESIDUAL_BREADTH_BOUND_COUNT_PER_ROW
    }:
        raise ValueError("DEC-262 breadth-bound count drift")
    if lower_tail_bound_counts != {
        REVIEWED_VERIFIED_RESIDUAL_LOWER_TAIL_BOUND_COUNT_PER_ROW
    }:
        raise ValueError("DEC-262 lower-tail bound count drift")
    if lower_tail_counts != {
        REVIEWED_VERIFIED_RESIDUAL_LOWER_TAIL_COUNT
    }:
        raise ValueError("DEC-262 lower-tail count drift")
    if regime_counts != {
        REVIEWED_VERIFIED_RESIDUAL_REGIME_COUNT
    }:
        raise ValueError("DEC-262 residual regime count drift")
    if windows_per_regime_counts != {
        REVIEWED_VERIFIED_RESIDUAL_WINDOWS_PER_REGIME
    }:
        raise ValueError("DEC-262 residual windows-per-regime count drift")
    if regime_floor_bound_counts != {
        REVIEWED_VERIFIED_RESIDUAL_REGIME_FLOOR_BOUND_COUNT_PER_ROW
    }:
        raise ValueError("DEC-262 regime-floor bound count drift")
    if regime_balance_regime_counts != {
        REVIEWED_VERIFIED_RESIDUAL_REGIME_BALANCE_REGIME_COUNT
    }:
        raise ValueError("DEC-262 regime-balance regime count drift")
    if regime_balance_source_bound_counts != {
        REVIEWED_VERIFIED_RESIDUAL_REGIME_BALANCE_SOURCE_BOUND_COUNT_PER_ROW
    }:
        raise ValueError("DEC-262 regime-balance source-bound count drift")
    if regime_balance_penalty_multipliers != {
        REVIEWED_VERIFIED_RESIDUAL_REGIME_BALANCE_PENALTY_MULTIPLIER
    }:
        raise ValueError("DEC-262 regime-balance penalty multiplier drift")
    ...
    return {
        ...
        "no_fit_temporal_residual_regime_balance_utility_stable_model_challenger_count": (
            no_stable_cells
        ),
...
def validate_reviewed_fit_temporal_residual_regime_balance_utility_repair_model_result(
    *,
    ...
    if run.get("id") != REVIEWED_MODEL_RUN_ID:
        raise ValueError("DEC-262 reviewed model run id mismatch")
    if run.get("head_sha") != REVIEWED_MODEL_HEAD_SHA:
        raise ValueError("DEC-262 reviewed model head SHA mismatch")
    if run.get("run_attempt") != REVIEWED_MODEL_RUN_ATTEMPT:
        raise ValueError("DEC-262 reviewed model run attempt mismatch")
    if run.get("conclusion") != REVIEWED_MODEL_RUN_CONCLUSION:
        raise ValueError("DEC-262 reviewed model run conclusion mismatch")
    ...
    exact_review = {
        ...
        "fit_temporal_residual_regime_balance_utility_repair_model_result_evidence_verified": (
            True
        ),
    ...
    for field, expected in exact_review.items():
        if review.get(field) != expected:
            raise ValueError(
                f"DEC-262 terminal review {field} mismatch"
            )
    ...
    for field, expected in exact_summary.items():
        if summary.get(field) != expected:
            raise ValueError(f"DEC-262 reviewed {field} mismatch")

    passes = _aggregate_pass_variants(aggregate_evidence)
    if passes != tuple(sorted(REVIEWED_AGGREGATE_PASS_VARIANTS)):
        raise ValueError("DEC-262 aggregate pass inventory mismatch")

    window_counts = _aggregate_pass_window_candidate_counts(
        aggregate_evidence
    )
    if window_counts != REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS:
        raise ValueError(
            "DEC-262 aggregate-pass stability-window inventory mismatch"
        )
    ...
    return {
        ...
        **summary,
        ...
        "stage": (
            "FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_RESULT_"
        ...
        "reviewed_aggregate_pass_window_candidate_counts": {
            ...
            for key, value in (
                REVIEWED_AGGREGATE_PASS_WINDOW_CANDIDATE_COUNTS.items()
            )
        },
...
__all__ = [
    ...
    "SHADOW_AUTHORIZED",
    "TRADING_AUTHORIZED",
    "validate_reviewed_fit_temporal_residual_regime_balance_utility_repair_model_result",
]
