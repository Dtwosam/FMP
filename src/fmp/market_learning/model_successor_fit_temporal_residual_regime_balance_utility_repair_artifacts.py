...
FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_RESULT_EVIDENCE_VERSION = 1

DEC254_MERGED_COMMIT = "c8cac108bc098dbceda4b8903f5a56ac7f62bf47"
DEC254_TRAINING_CORE_BLOB_SHA = (
...
def validate_fit_temporal_residual_regime_balance_utility_repair_artifact_contract_sources(
    *,
    repository_root: Path,
) -> dict[str, object]:
    root = Path(repository_root)
    expected = {
        "dec254_training_core": (
            root
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(f"missing EXP-060 artifact dependency: {path}")
    ...
    fingerprint = fit_temporal_residual_regime_balance_utility_repair_protocol_fingerprint()
    if len(fingerprint) != 64:
        raise ValueError("EXP-060 protocol fingerprint invalid")
    ...
    return {
        ...
        "dec254_training_core_blob_sha": actual["dec254_training_core"],
...
def validate_residual_regime_balance_cutoff(
    ...
    if not 0.0 <= values[3] <= 1.0:
        raise ValueError("EXP-060 residual-breadth cutoff out of range")
    for value in values[5:8]:
        if not 0.0 <= value <= 1.0:
            raise ValueError(
                "EXP-060 support/calibration cutoff out of range"
            )
    if values[8] <= 0.0:
        raise ValueError("EXP-060 raw cutoff must be positive")
    return values
...
def _validate_cell_fingerprint(row: Mapping[str, object]) -> None:
    ...
    if _validate_sha256(
        fingerprint,
        field="EXP-060 cell result fingerprint",
    ) != expected:
        raise ValueError("EXP-060 cell result fingerprint mismatch")
...
def _validate_fit_block(fit: Mapping[str, object]) -> dict[str, int]:
    ...
    for field, expected in exact.items():
        if fit.get(field) != expected:
            raise ValueError(f"EXP-060 {field} mismatch")

    residual_count = validate_fit_temporal_residual_references(fit)
    if residual_count != 24:
        raise ValueError("EXP-060 residual reference count mismatch")
...
def _validate_selection_block(selection: Mapping[str, object]) -> None:
    ...
    if not isinstance(diagnostics, Mapping):
        raise ValueError("EXP-060 consensus diagnostics missing")
    if diagnostics.get(
        "fit_temporal_residual_breadth_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_BREADTH_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-060 breadth bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_lower_tail_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-060 lower-tail bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_lower_tail_count"
    ) != FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_COUNT:
        raise ValueError("EXP-060 lower-tail count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_regime_count"
    ) != FIT_TEMPORAL_RESIDUAL_REGIME_COUNT:
        raise ValueError("EXP-060 residual regime count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_windows_per_regime"
    ) != FIT_TEMPORAL_RESIDUAL_WINDOWS_PER_REGIME:
        raise ValueError("EXP-060 residual windows-per-regime mismatch")
    if diagnostics.get(
        "fit_temporal_residual_regime_floor_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_REGIME_FLOOR_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-060 regime-floor bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_regime_balance_regime_count"
    ) != FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_REGIME_COUNT:
        raise ValueError("EXP-060 regime-balance regime count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_regime_balance_source_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_SOURCE_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-060 regime-balance source-bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_regime_balance_penalty_multiplier"
    ) != FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_PENALTY_MULTIPLIER:
        raise ValueError("EXP-060 regime-balance penalty multiplier mismatch")
    if diagnostics.get(
        "fit_temporal_residual_reference_count"
    ) != 24:
        raise ValueError("EXP-060 consensus residual reference count mismatch")
    ...
    for field in (
        ...
        if value is not None:
            ...
            if not 0.0 <= numeric <= 1.0:
                raise ValueError(
                    "EXP-060 consensus breadth diagnostic out of range"
                )
    ...
    _validate_sha256(
        ...
        field="EXP-060 consensus digest",
    )
    ...
    if not isinstance(variants, list) or len(variants) != len(
        ...
        raise ValueError("EXP-060 selection variant inventory mismatch")
    if {
        row.get("candidate_budget_anchor")
        for row in variants
        if isinstance(row, Mapping)
    } != set(CANDIDATE_BUDGET_ANCHORS):
        raise ValueError("EXP-060 selection budget identity mismatch")
    ...
    if selected is None:
        if status != (
            ...
            raise ValueError("EXP-060 no-selection status mismatch")
    else:
        if not isinstance(selected, Mapping):
            raise ValueError("EXP-060 selected variant malformed")
        if status != "SELECTED":
            raise ValueError("EXP-060 selected status mismatch")
        if selected.get("candidate_budget_anchor") not in (
            CANDIDATE_BUDGET_ANCHORS
        ):
            raise ValueError("EXP-060 selected budget mismatch")
...
def _validate_cell(
    ...
    if row.get("experiment_id") != (
        ...
        raise ValueError("EXP-060 cell experiment identity mismatch")
    if row.get("protocol_decision") != (
        FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_PROTOCOL_DECISION
    ):
        raise ValueError("EXP-060 cell protocol decision mismatch")
    if row.get("training_core_decision") != (
        FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_TRAINING_CORE_DECISION
    ):
        raise ValueError("EXP-060 cell training decision mismatch")
    if row.get("dec253_merged_commit") != (
        "7e5b399cb7960d385d956b33e5d96cea85bb2c28"
    ):
        raise ValueError("EXP-060 cell DEC-253 merge identity mismatch")
    if row.get("dec253_protocol_blob_sha") != (
        "82d336250e2cdd9894afa5554c6b422e0de6b1fe"
    ):
        raise ValueError("EXP-060 cell repair protocol blob mismatch")
    if row.get("failed_exp059_training_core_blob_sha") != (
        "4f99c1d0cb18551b67cc89357ad4a3940c190cd2"
    ):
        raise ValueError("EXP-060 failed EXP-059 training-core blob mismatch")
    if row.get("predecessor_training_core_blob_sha") != (
        "77f2010574b3d8ecc958930d5bfadf7ddb4f2231"
    ):
        raise ValueError("EXP-060 semantic predecessor training-core blob mismatch")
    if row.get("protocol_version") != (
        FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_PROTOCOL_VERSION
    ):
        raise ValueError("EXP-060 cell protocol version mismatch")
    if row.get("protocol_fingerprint") != (
        fit_temporal_residual_regime_balance_utility_repair_protocol_fingerprint()
    ):
        raise ValueError("EXP-060 cell protocol fingerprint mismatch")
    ...
    if (
        ...
        raise ValueError("EXP-060 cell evidence malformed")
    ...
    if selection.get("selected_variant") is None:
        if validation.get("status") != "LOCKED_NO_SELECTION":
            raise ValueError("EXP-060 validation lock mismatch")
        if holdout.get("status") != "LOCKED_NO_SELECTION":
            raise ValueError("EXP-060 holdout lock mismatch")
    ...
    for field in (
        ...
        if row.get(field) is not False:
            raise ValueError(f"EXP-060 cell {field} must remain false")

    return counts


def compile_fit_temporal_residual_regime_balance_utility_repair_model_result_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("EXP-060 model-result cell evidence incomplete")
    ...
    if totals != expected_totals:
        raise ValueError("EXP-060 aggregate evidence counts mismatch")

    payload = {
        "evidence_version": (
            FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_MODEL_RESULT_EVIDENCE_VERSION
        ),
        "experiment_id": (
            FIT_TEMPORAL_RESIDUAL_REGIME_BALANCE_UTILITY_REPAIR_EXPERIMENT_ID
        ),
...
def write_fit_temporal_residual_regime_balance_utility_repair_model_result_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "conflicting existing EXP-060 model-result evidence"
        )
    destination.write_text(payload, encoding="utf-8")


def run_authoritative_fit_temporal_residual_regime_balance_utility_repair_model_bundle(
    **_: object,
) -> dict[str, object]:
    if (
        ...
        raise PermissionError(
            "DEC-255 source is non-executable for authoritative "
            "EXP-060 model fitting"
        )
    raise PermissionError("EXP-060 repaired authoritative runner is not armed")
...
__all__ = [
    ...
    "run_authoritative_fit_temporal_residual_regime_balance_utility_repair_model_bundle",
