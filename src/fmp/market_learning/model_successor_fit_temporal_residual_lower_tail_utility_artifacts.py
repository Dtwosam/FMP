...
from .model_successor_fit_temporal_residual_lower_tail_utility_training import (
...
FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_MODEL_ARTIFACT_RUNNER_VERSION = (
    "fmp-exp056-fit-temporal-residual-lower-tail-utility-artifact-contract-v1"
)
FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_MODEL_ARTIFACT_RUNNER_DECISION = (
    "DEC-211"
)
FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_MODEL_RESULT_EVIDENCE_VERSION = 1

DEC210_MERGED_COMMIT = "029159999fae7eaa67811f8b3d8bf2bf8834e491"
DEC210_TRAINING_CORE_BLOB_SHA = (
...
def validate_fit_temporal_residual_lower_tail_utility_artifact_contract_sources(
    *,
    repository_root: Path,
) -> dict[str, object]:
    root = Path(repository_root)
    expected = {
        "dec210_training_core": (
            root
    ...
    for label, (path, expected_sha) in expected.items():
        if not path.is_file():
            raise ValueError(f"missing EXP-056 artifact dependency: {path}")
    ...
    if training[
        ...
        raise ValueError("EXP-056 training-core decision drift")

    fingerprint = fit_temporal_residual_lower_tail_utility_protocol_fingerprint()
    if len(fingerprint) != 64:
        raise ValueError("EXP-056 protocol fingerprint invalid")
    ...
    return {
        ...
        "dec210_training_core_blob_sha": actual["dec210_training_core"],
...
def validate_residual_lower_tail_cutoff(
    ...
    if not 0.0 <= values[1] <= 1.0:
        raise ValueError("EXP-056 residual-breadth cutoff out of range")
    for value in values[3:6]:
        if not 0.0 <= value <= 1.0:
            raise ValueError(
                "EXP-056 support/calibration cutoff out of range"
            )
    if values[6] <= 0.0:
        raise ValueError("EXP-056 raw cutoff must be positive")
    return values
...
def _validate_cell_fingerprint(row: Mapping[str, object]) -> None:
    ...
    if _validate_sha256(
        fingerprint,
        field="EXP-056 cell result fingerprint",
    ) != expected:
        raise ValueError("EXP-056 cell result fingerprint mismatch")
...
def _validate_fit_block(fit: Mapping[str, object]) -> dict[str, int]:
    ...
    for field, expected in exact.items():
        if fit.get(field) != expected:
            raise ValueError(f"EXP-056 {field} mismatch")

    residual_count = validate_fit_temporal_residual_references(fit)
    if residual_count != 24:
        raise ValueError("EXP-056 residual reference count mismatch")
...
def _validate_selection_block(selection: Mapping[str, object]) -> None:
    ...
    if not isinstance(diagnostics, Mapping):
        raise ValueError("EXP-056 consensus diagnostics missing")
    if diagnostics.get(
        "fit_temporal_residual_breadth_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_BREADTH_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-056 breadth bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_lower_tail_bound_count_per_row"
    ) != FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_BOUND_COUNT_PER_ROW:
        raise ValueError("EXP-056 lower-tail bound count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_lower_tail_count"
    ) != FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_COUNT:
        raise ValueError("EXP-056 lower-tail count mismatch")
    if diagnostics.get(
        "fit_temporal_residual_reference_count"
    ) != 24:
        raise ValueError("EXP-056 consensus residual reference count mismatch")
    ...
    for field in (
        ...
        if value is not None:
            ...
            if not 0.0 <= numeric <= 1.0:
                raise ValueError(
                    "EXP-056 consensus breadth diagnostic out of range"
                )
    ...
    _validate_sha256(
        ...
        field="EXP-056 consensus digest",
    )
    ...
    if not isinstance(variants, list) or len(variants) != len(
        ...
        raise ValueError("EXP-056 selection variant inventory mismatch")
    if {
        row.get("candidate_budget_anchor")
        for row in variants
        if isinstance(row, Mapping)
    } != set(CANDIDATE_BUDGET_ANCHORS):
        raise ValueError("EXP-056 selection budget identity mismatch")
    ...
    if selected is None:
        if status != (
            ...
            raise ValueError("EXP-056 no-selection status mismatch")
    else:
        if not isinstance(selected, Mapping):
            raise ValueError("EXP-056 selected variant malformed")
        if status != "SELECTED":
            raise ValueError("EXP-056 selected status mismatch")
        if selected.get("candidate_budget_anchor") not in (
            CANDIDATE_BUDGET_ANCHORS
        ):
            raise ValueError("EXP-056 selected budget mismatch")
...
def _validate_cell(
    ...
    if row.get("experiment_id") != (
        ...
        raise ValueError("EXP-056 cell experiment identity mismatch")
    if row.get("protocol_decision") != (
        FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_PROTOCOL_DECISION
    ):
        raise ValueError("EXP-056 cell protocol decision mismatch")
    if row.get("training_core_decision") != (
        FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_TRAINING_CORE_DECISION
    ):
        raise ValueError("EXP-056 cell training decision mismatch")
    ...
    if (
        ...
        raise ValueError("EXP-056 cell evidence malformed")
    ...
    if selection.get("selected_variant") is None:
        if validation.get("status") != "LOCKED_NO_SELECTION":
            raise ValueError("EXP-056 validation lock mismatch")
        if holdout.get("status") != "LOCKED_NO_SELECTION":
            raise ValueError("EXP-056 holdout lock mismatch")
    ...
    for field in (
        ...
        if row.get(field) is not False:
            raise ValueError(f"EXP-056 cell {field} must remain false")

    return counts


def compile_fit_temporal_residual_lower_tail_utility_model_result_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("EXP-056 model-result cell evidence incomplete")
    ...
    if totals != expected_totals:
        raise ValueError("EXP-056 aggregate evidence counts mismatch")

    payload = {
        "evidence_version": (
            FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_MODEL_RESULT_EVIDENCE_VERSION
        ),
        "experiment_id": (
            FIT_TEMPORAL_RESIDUAL_LOWER_TAIL_UTILITY_EXPERIMENT_ID
        ),
...
def write_fit_temporal_residual_lower_tail_utility_model_result_evidence(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "conflicting existing EXP-056 model-result evidence"
        )
    destination.write_text(payload, encoding="utf-8")


def run_authoritative_fit_temporal_residual_lower_tail_utility_model_bundle(
    **_: object,
) -> dict[str, object]:
    if (
        ...
        raise PermissionError(
            "DEC-211 source is non-executable for authoritative "
            "EXP-056 model fitting"
        )
    raise PermissionError("EXP-056 authoritative runner is not armed")
...
__all__ = [
    ...
    "run_authoritative_fit_temporal_residual_lower_tail_utility_model_bundle",
    "validate_fit_temporal_residual_lower_tail_utility_artifact_contract_sources",
    "validate_residual_lower_tail_cutoff",
    "write_fit_temporal_residual_lower_tail_utility_model_result_evidence",
]
