...
def _checkout_preflight() -> dict[str, object]:
    ...
    return validate_temporal_jackknife_utility_operator_checkout(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-050 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="fmp-exp050-model-result-"
    ) as tmp:
        root = Path(tmp)
        ...
        if len(matches) != 1:
            raise SystemExit(
                "EXP-050 aggregate artifact must contain exactly "
                "one model-result-evidence.json"
            )
        return load_temporal_jackknife_utility_model_result_evidence(
            matches[0],
...
def _validated_source_gate() -> dict[str, object]:
    gate = build_temporal_jackknife_utility_model_workflow_source_gate(
...
def _next_report() -> dict[str, object]:
    ...
    listing = _gh_json(temporal_jackknife_utility_model_runs_endpoint())
    run = select_temporal_jackknife_utility_manual_main_run(listing)
    ...
    exact_run = _gh_json(
        temporal_jackknife_utility_model_run_endpoint(run_id)
    )
    jobs = _gh_json(
        temporal_jackknife_utility_model_run_jobs_endpoint(run_id)
    )
    artifacts = _gh_json(
        temporal_jackknife_utility_model_run_artifacts_endpoint(run_id)
    )
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-050 run is missing head SHA"
            )
    ...
    review = validate_temporal_jackknife_utility_model_terminal_review(
        run=exact_run,
        jobs_payload=jobs,
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-147 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-147 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-050 historical "
            "model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-050 workflow state and return "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-050 operator command: {args.command}"
        )

    plan = _read_next_via_public_cli()
    command = dispatch_command_for_temporal_jackknife_utility_report(plan)
    report = {
        **plan,
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-050 live state changed between planning and "
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-050 dispatch plan changed before execution"
        )
