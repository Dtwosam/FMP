...
from fmp.market_learning.model_successor_fit_temporal_residual_regime_balance_utility_operator import (
...
def _require_gh_auth() -> None:
    try:
        ...
        raise SystemExit(
            ...
            "EXP-059 operator use"
        ) from exc


def _checkout_preflight() -> dict[str, object]:
    _run(("git", "fetch", "--quiet", "origin", "main"), capture=False)
    return validate_fit_temporal_residual_regime_balance_utility_operator_checkout(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-059 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        if len(matches) != 1:
            raise SystemExit(
                "EXP-059 aggregate artifact must contain exactly "
        ...
        try:
            ...
            raise SystemExit(
                "EXP-059 aggregate evidence is not valid JSON"
            ) from exc
        if not isinstance(raw, dict):
            raise SystemExit(
                "EXP-059 aggregate evidence must be a JSON object"
            )
...
def _validated_source_gate() -> dict[str, object]:
    ...
    if gate.get("stage") != (
        ...
        raise SystemExit(
            "EXP-059 DEC-247 source gate returned an invalid stage"
        )
    for field in (
        ...
        if gate.get(field) is not True:
            raise SystemExit(
                f"EXP-059 DEC-247 source gate {field} must be true"
            )
    for field in ("promotion_authorized", "trading_authorized"):
        if gate.get(field) is not False:
            raise SystemExit(
                f"EXP-059 DEC-247 source gate {field} must be false"
            )
    return gate
...
def _next_report() -> dict[str, object]:
    ...
    run = select_fit_temporal_residual_regime_balance_utility_manual_main_run(listing)

    report = build_fit_temporal_residual_regime_balance_utility_operator_report(
        checkout=checkout,
        run=run,
    )
    report.update(
        {
            ...
            **fit_temporal_residual_regime_balance_utility_operator_gate_metadata(gate),
        }
    )
    ...
    exact_run = _gh_json(
        fit_temporal_residual_regime_balance_utility_model_run_endpoint(run_id)
    )
    jobs = _gh_json(
        fit_temporal_residual_regime_balance_utility_model_run_jobs_endpoint(run_id)
    )
    artifacts = _gh_json(
        fit_temporal_residual_regime_balance_utility_model_run_artifacts_endpoint(run_id)
    )
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-059 run is missing head SHA"
            )
        selected = select_fit_temporal_residual_regime_balance_utility_aggregate_artifact(
            artifacts,
            head_sha=head_sha,
        )
    ...
    review = validate_fit_temporal_residual_regime_balance_utility_model_terminal_review(
        run=exact_run,
        jobs_payload=jobs,
    ...
    report["next_action"] = (
        "Review the frozen DEC-246 terminal evidence. "
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-248 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-248 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-059 historical model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-059 workflow state and return "
    ...
    advance = sub.add_parser(
        "advance",
        help=(
            "prepare or execute exactly one DEC-247-authorized dispatch"
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-059 operator command: {args.command}"
        )

    plan = _read_next_via_public_cli()
    command = dispatch_command_for_fit_temporal_residual_regime_balance_utility_report(
        plan
    )
    report = {
        **plan,
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-059 live state changed between planning and execution; "
            "rerun advance"
        )
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-059 dispatch plan changed before execution"
        )
