...
def _checkout_preflight() -> dict[str, object]:
    ...
    return validate_temporal_calibrated_utility_operator_checkout(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-051 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="fmp-exp051-model-result-"
    ) as tmp:
        root = Path(tmp)
        ...
        matches = sorted(
            extracted.rglob(
                "model-result-evidence.json"
            )
        )
        if len(matches) != 1:
            raise SystemExit(
                "EXP-051 aggregate artifact must contain exactly "
                "one model-result-evidence.json"
            )
        return load_temporal_calibrated_utility_model_result_evidence(
            matches[0],
...
def _validated_source_gate() -> dict[str, object]:
    ...
    if gate.get("stage") != (
        ...
        raise SystemExit(
            "EXP-051 DEC-155 source gate returned an invalid stage"
        )
    ...
    for field in (
        ...
        if gate.get(field) is not True:
            raise SystemExit(
                f"EXP-051 DEC-155 source gate {field} must be true"
            )
    ...
    for field in (
        ...
        if gate.get(field) is not False:
            raise SystemExit(
                f"EXP-051 DEC-155 source gate {field} must be false"
            )
    return gate
...
def _next_report() -> dict[str, object]:
    ...
    run = select_temporal_calibrated_utility_manual_main_run(
        listing
    )

    report = build_temporal_calibrated_utility_operator_report(
        checkout=checkout,
        run=run,
    )
    report.update(
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-051 run is missing head SHA"
            )
        selected = (
            select_temporal_calibrated_utility_aggregate_artifact(
                artifacts,
    ...
    report["next_action"] = (
        "Review the frozen DEC-154 terminal evidence. "
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-156 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-156 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-051 historical "
            "model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-051 workflow state and return "
    ...
    advance = sub.add_parser(
        "advance",
        help=(
            "prepare or execute exactly one DEC-155-authorized "
            "dispatch"
        ),
    )
...
def main(
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-051 operator command: {args.command}"
        )
    ...
    report = {
        **plan,
        "advance_execute_requested": bool(
            args.execute
        ),
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-051 live state changed between planning and "
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-051 dispatch plan changed before execution"
        )
