...
def _require_exact_checkout(code_commit: str) -> None:
    if _git_head() != code_commit:
        raise ValueError(
            "EXP-050 model runner code_commit must equal checkout HEAD"
        )
...
def _write_json(path: Path, value: Mapping[str, object]) -> None:
    ...
    if (
        ...
        raise ValueError(
            f"conflicting existing EXP-050 model result: {destination}"
        )
...
def _load_result_files(root: Path) -> list[Mapping[str, object]]:
    ...
    if len(paths) != 18:
        raise ValueError(
            "EXP-050 aggregate requires exactly 18 cell result files"
        )
    ...
    for path in paths:
        ...
        if not isinstance(value, dict):
            raise ValueError(
                f"EXP-050 model result must be an object: {path}"
            )
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "EXP-050 successor model workflow source. Historical "
    ...
    sub.add_parser(
        "status",
        help="report the frozen non-executable EXP-050 workflow source gate",
    )

    require = sub.add_parser(
        "require-execution",
        help="fail unless authoritative EXP-050 result execution is authorized",
    )
    require.add_argument("--code-commit", required=True)

    cell = sub.add_parser(
        "run-cell",
        help="run one EXP-050 successor cell when separately authorized",
    )
    ...
    aggregate = sub.add_parser(
        "aggregate",
        help="compile all 18 EXP-050 cell results when authorized",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        execution = require_authoritative_temporal_jackknife_utility_model_execution(
            repository_root=ROOT,
    ...
    if args.command == "run-cell":
        ...
        result = run_temporal_jackknife_utility_model_cell_core(
    ...
    if args.command == "aggregate":
        evidence = compile_temporal_jackknife_utility_model_result_evidence(
