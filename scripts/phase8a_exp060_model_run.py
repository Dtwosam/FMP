...
from fmp.market_learning.model_successor_fit_temporal_residual_regime_balance_utility_repair_artifacts import (
...
def _require_exact_checkout(code_commit: str) -> None:
    if _git_head() != code_commit:
        raise ValueError(
            "EXP-060 model runner code_commit must equal checkout HEAD"
        )
...
def _write_json(path: Path, value: Mapping[str, object]) -> None:
    ...
    if (
        ...
        raise ValueError(
            f"conflicting existing EXP-060 model result: {destination}"
        )
...
def _load_result_files(root: Path) -> list[Mapping[str, object]]:
    ...
    if len(paths) != 18:
        raise ValueError(
            "EXP-060 aggregate requires exactly 18 cell result files"
        )
    ...
    for path in paths:
        ...
        if not isinstance(value, dict):
            raise ValueError(
                f"EXP-060 model result must be an object: {path}"
            )
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "EXP-060 fit-temporal residual-regime-balance utility workflow source. "
    ...
    sub.add_parser(
        "status",
        help="report the frozen non-executable EXP-060 workflow source gate",
    )

    require = sub.add_parser(
        "require-execution",
        help="fail unless authoritative EXP-060 result execution is authorized",
    )
    require.add_argument("--code-commit", required=True)

    cell = sub.add_parser(
        "run-cell",
        help="run one EXP-060 successor cell when separately authorized",
    )
    ...
    aggregate = sub.add_parser(
        "aggregate",
        help="compile all 18 EXP-060 cell results when authorized",
    )
