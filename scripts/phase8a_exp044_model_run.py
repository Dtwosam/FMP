...
from fmp.market_learning.model_training import run_model_cell_core
...
def _require_exact_checkout(code_commit: str) -> None:
    if _git_head() != code_commit:
        raise ValueError(
            "EXP-044 model runner code_commit must equal checkout HEAD"
        )
...
def _write_json(path: Path, value: Mapping[str, object]) -> None:
    ...
    if (
        ...
        raise ValueError(
            f"conflicting existing EXP-044 model result: {destination}"
        )
...
def _load_result_files(root: Path) -> list[Mapping[str, object]]:
    ...
    if len(paths) != 18:
        raise ValueError(
            "EXP-044 aggregate requires exactly 18 cell result files"
        )
    ...
    for path in paths:
        ...
        if not isinstance(value, dict):
            raise ValueError(
                f"EXP-044 model result must be an object: {path}"
            )
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "EXP-044 model workflow source. Authoritative execution remains "
            "fail-closed until a later decision."
        )
    )
    ...
    sub.add_parser(
        "status",
        help="report the frozen non-executable model-workflow source gate",
    )

    require = sub.add_parser(
        "require-execution",
        help="fail unless authoritative model-result execution is authorized",
    )
    require.add_argument("--code-commit", required=True)

    cell = sub.add_parser(
        "run-cell",
        help="run one frozen pair/timeframe/horizon cell when authorized",
    )
    ...
    aggregate = sub.add_parser(
        "aggregate",
        help="compile all 18 cell results when execution is authorized",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        execution = require_authoritative_model_execution(
            repository_root=ROOT,
    ...
    if args.command == "aggregate":
        evidence = compile_model_result_evidence(
