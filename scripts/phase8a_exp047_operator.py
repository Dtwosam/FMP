...
def _checkout_preflight() -> dict[str, object]:
    ...
    return validate_density_operator_checkout(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-047 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="fmp-exp047-model-result-"
    ) as tmp:
        root = Path(tmp)
        ...
        if len(matches) != 1:
            raise SystemExit(
                "EXP-047 aggregate artifact must contain exactly "
                "one model-result-evidence.json"
            )
        return load_density_model_result_evidence(
            matches[0],
...
def _validated_source_gate() -> dict[str, object]:
    ...
    if (
        ...
        raise SystemExit(
            "EXP-047 DEC-118 source gate returned an invalid stage"
        )
    ...
    for field in (
        ...
        if gate.get(field) is not True:
            raise SystemExit(
                f"EXP-047 DEC-118 source gate {field} must be true"
            )
    ...
    for field in (
        ...
        if gate.get(field) is not False:
            raise SystemExit(
                f"EXP-047 DEC-118 source gate {field} must be false"
            )
    return gate
...
def _next_report() -> dict[str, object]:
    ...
    listing = _gh_json(density_model_runs_endpoint())
    run = select_density_manual_main_run(listing)
    ...
    exact_run = _gh_json(
        density_model_run_endpoint(run_id)
    )
    jobs = _gh_json(
        density_model_run_jobs_endpoint(run_id)
    )
    artifacts = _gh_json(
        density_model_run_artifacts_endpoint(run_id)
    )
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-047 run is missing head SHA"
            )
    ...
    report["next_action"] = (
        "Review the frozen DEC-117 terminal evidence. "
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-119 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-119 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-047 historical "
            "model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-047 workflow state and return "
    ...
    advance = sub.add_parser(
        "advance",
        help=(
            "prepare or execute exactly one DEC-118-authorized "
            "dispatch"
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-047 operator command: {args.command}"
        )

    plan = _read_next_via_public_cli()
    command = dispatch_command_for_density_report(plan)
    report = {
        **plan,
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-047 live state changed between planning and "
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-047 dispatch plan changed before execution"
        )
