...
from fmp.market_learning.model_successor_fit_temporal_residual_bound_utility_operator import (
...
from fmp.market_learning.model_successor_fit_temporal_residual_bound_utility_result_review import (
...
def _require_gh_auth() -> None:
    try:
        ...
        raise SystemExit(
            ...
            "EXP-054 operator use"
        ) from exc


def _checkout_preflight() -> dict[str, object]:
    _run(("git", "fetch", "--quiet", "origin", "main"), capture=False)
    return validate_fit_temporal_residual_bound_utility_operator_checkout(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-054 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="fmp-exp054-model-result-"
    ) as tmp:
        root = Path(tmp)
        ...
        try:
            ...
            raise SystemExit(
                "EXP-054 aggregate model-result evidence is unreadable"
            ) from exc
        if not isinstance(value, dict):
            raise SystemExit(
                "EXP-054 aggregate model-result evidence must be an object"
            )
        if value.get("code_commit") != head_sha:
            raise SystemExit(
                "EXP-054 aggregate evidence code_commit mismatch"
            )
        return value
...
def _validated_source_gate() -> dict[str, object]:
    ...
    if gate.get("stage") != (
        ...
        raise SystemExit(
            "EXP-054 DEC-191 source gate returned an invalid stage"
        )
    for field in (
        ...
        if gate.get(field) is not True:
            raise SystemExit(
                f"EXP-054 DEC-191 source gate {field} must be true"
            )
    for field in ("promotion_authorized", "trading_authorized"):
        if gate.get(field) is not False:
            raise SystemExit(
                f"EXP-054 DEC-191 source gate {field} must be false"
            )
    return gate
...
def _next_report() -> dict[str, object]:
    ...
    run = select_fit_temporal_residual_bound_utility_manual_main_run(listing)

    report = build_fit_temporal_residual_bound_utility_operator_report(
        checkout=checkout,
        run=run,
    )
    report.update(
        {
            ...
            **fit_temporal_residual_bound_utility_operator_gate_metadata(gate),
        }
    )
    ...
    exact_run = _gh_json(
        fit_temporal_residual_bound_utility_model_run_endpoint(run_id)
    )
    jobs = _gh_json(
        fit_temporal_residual_bound_utility_model_run_jobs_endpoint(run_id)
    )
    artifacts = _gh_json(
        fit_temporal_residual_bound_utility_model_run_artifacts_endpoint(run_id)
    )
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-054 run is missing head SHA"
            )
    ...
    review = validate_fit_temporal_residual_bound_utility_model_terminal_review(
        run=exact_run,
        jobs_payload=jobs,
    ...
    report["next_action"] = (
        "Review the frozen DEC-190 terminal evidence. "
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-192 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-192 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-054 historical model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-054 workflow state and return "
    ...
    advance = sub.add_parser(
        "advance",
        help=(
            "prepare or execute exactly one DEC-191-authorized dispatch"
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-054 operator command: {args.command}"
        )

    plan = _read_next_via_public_cli()
    command = dispatch_command_for_fit_temporal_residual_bound_utility_report(
        plan
    )
    report = {
        **plan,
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-054 live state changed between planning and execution; "
            "rerun advance"
        )
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-054 dispatch plan changed before execution"
        )
