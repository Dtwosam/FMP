...
from fmp.market_learning.model_successor_stability_result_review import (
    validate_stability_model_terminal_review,
)


ROOT = Path(__file__).resolve().parents[1]
OPERATOR_DECISION = "DEC-110"
...
def _checkout_preflight() -> dict[str, object]:
    ...
    return validate_stability_operator_checkout(
        branch=_run(("git", "branch", "--show-current")),
        head_sha=_run(("git", "rev-parse", "HEAD")),
        origin_main_sha=_run(
            ("git", "rev-parse", "origin/main")
        ),
        porcelain_status=_run(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-046 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="fmp-exp046-model-result-"
    ) as tmp:
        root = Path(tmp)
        ...
        matches = sorted(
            extracted.rglob("model-result-evidence.json")
        )
        if len(matches) != 1:
            raise SystemExit(
                "EXP-046 aggregate artifact must contain exactly "
                "one model-result-evidence.json"
            )
        return load_stability_model_result_evidence(
            matches[0],
...
def _validated_source_gate() -> dict[str, object]:
    ...
    if (
        ...
        raise SystemExit(
            "EXP-046 DEC-109 source gate returned an invalid stage"
        )
    ...
    for field in (
        ...
        if gate.get(field) is not True:
            raise SystemExit(
                f"EXP-046 DEC-109 source gate {field} must be true"
            )
    ...
    for field in (
        ...
        if gate.get(field) is not False:
            raise SystemExit(
                f"EXP-046 DEC-109 source gate {field} must be false"
            )
    return gate
...
def _next_report() -> dict[str, object]:
    ...
    listing = _gh_json(stability_model_runs_endpoint())
    run = select_stability_manual_main_run(listing)
    ...
    exact_run = _gh_json(
        stability_model_run_endpoint(run_id)
    )
    jobs = _gh_json(
        stability_model_run_jobs_endpoint(run_id)
    )
    artifacts = _gh_json(
        stability_model_run_artifacts_endpoint(run_id)
    )
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-046 run is missing head SHA"
            )
    ...
    report["next_action"] = (
        "Review the frozen DEC-108 terminal evidence. "
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-110 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-110 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-046 historical "
            "model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-046 workflow state and return "
    ...
    advance = sub.add_parser(
        "advance",
        help=(
            "prepare or execute exactly one DEC-109-authorized "
            "dispatch"
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-046 operator command: {args.command}"
        )

    plan = _read_next_via_public_cli()
    command = dispatch_command_for_stability_report(plan)
    report = {
        **plan,
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-046 live state changed between planning and "
            "execution; rerun advance"
        )
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-046 dispatch plan changed before execution"
        )
