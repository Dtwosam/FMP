...
from fmp.market_learning.model_successor_fit_temporal_feature_support_utility_result_review import (
...
def _require_gh_auth() -> None:
    try:
        ...
        raise SystemExit(
            ...
            "EXP-053 operator use"
        ) from exc


def _checkout_preflight() -> dict[str, object]:
    _run(("git", "fetch", "--quiet", "origin", "main"), capture=False)
    return validate_fit_temporal_feature_support_utility_operator_checkout(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise SystemExit(
                    "EXP-053 aggregate ZIP contains an unsafe path"
                ) from exc
...
def _download_aggregate_evidence(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="fmp-exp053-model-result-"
    ) as tmp:
        root = Path(tmp)
        ...
        if len(matches) != 1:
            raise SystemExit(
                "EXP-053 aggregate artifact must contain exactly "
                "one model-result-evidence.json"
            )
        return load_fit_temporal_feature_support_utility_model_result_evidence(
            matches[0],
...
def _validated_source_gate() -> dict[str, object]:
    gate = build_fit_temporal_feature_support_utility_model_workflow_source_gate(
...
def _next_report() -> dict[str, object]:
    ...
    run = select_fit_temporal_feature_support_utility_manual_main_run(listing)
    ...
    report.update(
        {
            ...
            **fit_temporal_feature_support_utility_operator_gate_metadata(gate),
        }
    )
    ...
    exact_run = _gh_json(
        fit_temporal_feature_support_utility_model_run_endpoint(run_id)
    )
    jobs = _gh_json(
        fit_temporal_feature_support_utility_model_run_jobs_endpoint(run_id)
    )
    artifacts = _gh_json(
        fit_temporal_feature_support_utility_model_run_artifacts_endpoint(run_id)
    )
    ...
    if exact_run.get("conclusion") == "success":
        ...
        if not isinstance(head_sha, str):
            raise SystemExit(
                "successful EXP-053 run is missing head SHA"
            )
    ...
    review = validate_fit_temporal_feature_support_utility_model_terminal_review(
        run=exact_run,
        jobs_payload=jobs,
    ...
    report["next_action"] = (
        "Review the frozen DEC-178 terminal evidence. "
...
def _read_next_via_public_cli() -> dict[str, object]:
    ...
    try:
        ...
        raise SystemExit(
            "DEC-180 next planner did not return valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise SystemExit(
            "DEC-180 next planner must return a JSON object"
        )
    return value
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "Fail-closed single-step EXP-053 historical model-run operator"
        )
    )
    ...
    sub.add_parser(
        "next",
        help=(
            "inspect live EXP-053 workflow state and return "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command != "advance":
        raise SystemExit(
            f"unsupported EXP-053 operator command: {args.command}"
        )

    plan = _read_next_via_public_cli()
    command = dispatch_command_for_fit_temporal_feature_support_utility_report(
        plan
    )
    report = {
        **plan,
    ...
    if confirmed != plan:
        raise SystemExit(
            "EXP-053 live state changed between planning and execution; "
            "rerun advance"
        )
    ...
    if confirmed_command != command:
        raise SystemExit(
            "EXP-053 dispatch plan changed before execution"
        )
