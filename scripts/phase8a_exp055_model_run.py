...
def _require_exact_checkout(code_commit: str) -> None:
    if _git_head() != code_commit:
        raise ValueError(
            "EXP-055 model runner code_commit must equal checkout HEAD"
        )
...
def _write_json(path: Path, value: Mapping[str, object]) -> None:
    ...
    if (
        ...
        raise ValueError(
            f"conflicting existing EXP-055 model result: {destination}"
        )
...
def _load_result_files(root: Path) -> list[Mapping[str, object]]:
    ...
    if len(paths) != 18:
        raise ValueError(
            "EXP-055 aggregate requires exactly 18 cell result files"
        )
    ...
    for path in paths:
        ...
        if not isinstance(value, dict):
            raise ValueError(
                f"EXP-055 model result must be an object: {path}"
            )
...
def parser() -> argparse.ArgumentParser:
    out = argparse.ArgumentParser(
        description=(
            "EXP-055 fit-temporal residual-breadth utility workflow source. "
    ...
    sub.add_parser(
        "status",
        help="report the frozen non-executable EXP-055 workflow source gate",
    )

    require = sub.add_parser(
        "require-execution",
        help="fail unless authoritative EXP-055 result execution is authorized",
    )
    require.add_argument("--code-commit", required=True)

    cell = sub.add_parser(
        "run-cell",
        help="run one EXP-055 successor cell when separately authorized",
    )
    ...
    aggregate = sub.add_parser(
        "aggregate",
        help="compile all 18 EXP-055 cell results when authorized",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run-cell":
        ...
        result = run_fit_temporal_residual_breadth_utility_model_cell_core(
